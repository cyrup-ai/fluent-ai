...
use crate::{
    completion::{
        CompletionError, CompletionModel, CompletionRequest, CompletionRequestBuilder, PromptError,
        ToolDefinition,
    },
    message::Message,
...
/* --------------------------------------------------------------------- */
impl<M: CompletionModel> CompletionBuilder<M, NeedsPrompt> {
    #[inline(always)]
...
pub fn default_builder<M: CompletionModel>(model: M) -> CompletionBuilder<M, NeedsPrompt> {
...
/* --------------------------------------------------------------------- */
impl<M: CompletionModel> CompletionBuilder<M, HasPrompt> {
    ...
    pub fn send(
        ...
        Result<
            crate::completion::CompletionResponse<<M as CompletionModel>::Response>,
            CompletionError,
        >,
    > {
...
impl<S> AnthropicBuilderExt
    for CompletionBuilder<crate::providers::anthropic::completion::CompletionModel, S>
{
    #[inline(always)]
    fn beta(self, tag: &'static str) -> Self {
