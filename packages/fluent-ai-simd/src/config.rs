...
impl ProcessorConfig {
    ...
    pub fn validate(&self) -> Result<(), ConfigError> {
        ...
        if self.repetition_penalty < 1.0 {
            return Err(ConfigError::InvalidRepetitionPenalty(self.repetition_penalty));
        }
...
mod tests {
    ...
    fn test_validation() {
        ...
        assert!(matches!(config.validate(), Err(ConfigError::InvalidRepetitionPenalty(_))));
        config.repetition_penalty = 1.2;
        
        // Test invalid frequency penalty
        config.frequency_penalty = -0.1;
        assert!(matches!(config.validate(), Err(ConfigError::InvalidFrequencyPenalty(_))));
        config.frequency_penalty = 0.1;
        
        // Test invalid presence penalty
        config.presence_penalty = -0.1;
        assert!(matches!(config.validate(), Err(ConfigError::InvalidPresencePenalty(_))));
    }
}
