...
impl GeneratedToken {
    ...
    #[inline(always)]
    pub fn new(id: u32, text: &str, log_prob: f32, cumulative_log_prob: f32, step: u32, is_special: bool) -> CandleResult<Self> {
...
impl CandleGenerator {
    ...
    #[inline(always)]
    pub async fn generate(&self, request: &CompletionRequest<'_>) -> CandleResult<CompletionResponse> {
        let start_time = std::time::Instant::now();
        
        // Tokenize input
        let input_tokens = self.tokenizer.encode(
            std::str::from_utf8(request.prompt()).map_err(|_| CandleError::tokenizer("Invalid UTF-8 in prompt"))?,
            true
        )?;
        
        // Initialize generation state
        let mut state = GenerationState::new();
        state.tokens.try_extend_from_slice(&input_tokens)
        ...
        state.stats.generation_time_us.store(elapsed.as_micros() as u64, Ordering::Relaxed);
        state.stats.tokens_per_second.store(tokens_per_second, Ordering::Relaxed);
    ...
    #[inline(always)]
    pub async fn generate_stream(&self, request: &CompletionRequest<'_>) -> CandleResult<StreamingResponse> {
    ...
    async fn generate_stream_internal(
        ...
        tx: mpsc::UnboundedSender<Result<CompletionResponse, fluent_ai_core::completion::error::CompletionError>>,
    ) -> CandleResult<()> {
        // Tokenize input
        let input_tokens = self.tokenizer.encode(
            std::str::from_utf8(request.prompt()).map_err(|_| CandleError::tokenizer("Invalid UTF-8 in prompt"))?,
    ...
    async fn generate_next_token(&self, tokens: &[u32], step: u32) -> CandleResult<GeneratedToken> {
        ...
        // Decode token to text
        let token_text = self.tokenizer.id_to_token(next_token_id)
            .unwrap_or_else(|| format!("<unk:{}>", next_token_id));
        
        // Calculate actual log probability from logits
        let log_prob = self.calculate_token_log_probability(&logits, next_token_id).await?;
    ...
    async fn sample_token(&self, logits: &Tensor, _step: u32) -> CandleResult<u32> {
        ...
        let final_logits = if self.config.top_p < 1.0 {
            self.apply_top_p(&filtered_logits, self.config.top_p).await?
        } else {
            filtered_logits
        };
    ...
    async fn apply_top_k(&self, logits: &Tensor, k: u32) -> CandleResult<Tensor> {
        ...
        let (top_k_values, top_k_indices) = logits.topk(k as usize)
            .map_err(|e| CandleError::from(e))?;
        ...
        filtered_logits = filtered_logits.scatter_add(&top_k_indices, &top_k_values, 0)
            .map_err(|e| CandleError::from(e))?;
    ...
    async fn apply_top_p(&self, logits: &Tensor, p: f32) -> CandleResult<Tensor> {
        ...
        let sorted_probs = probs.sort_by(candle_core::D::Minus1, false)
            .map_err(|e| CandleError::from(e))?;
        
        // Calculate cumulative probabilities
        let cumulative_probs = sorted_probs.cumsum(candle_core::D::Minus1)
            .map_err(|e| CandleError::from(e))?;
        
        // Find tokens where cumulative probability exceeds p
        let mask = cumulative_probs.le(&Tensor::full(p, cumulative_probs.shape(), &self.device)
            .map_err(|e| CandleError::from(e))?)
            .map_err(|e| CandleError::from(e))?;
        
        // Apply mask to filter out tokens beyond nucleus
        let filtered_logits = logits.where_cond(
            &mask,
    ...
    async fn sample_from_logits(&self, logits: &Tensor) -> CandleResult<u32> {
        ...
        let prob_values: Vec<f32> = probs_cpu.to_vec1()
            .map_err(|e| CandleError::from(e))?;
    ...
    async fn greedy_sample(&self, logits: &Tensor) -> CandleResult<u32> {
        let argmax = logits.argmax_keepdim(candle_core::D::Minus1)
            .map_err(|e| CandleError::from(e))?;
        
        let token_id = argmax.to_scalar::<u32>()
            .map_err(|e| CandleError::from(e))?;
    ...
    async fn calculate_token_log_probability(&self, logits: &Tensor, token_id: u32) -> CandleResult<f64> {
        ...
        let token_log_prob = log_probs.i(token_id as usize)
            .map_err(|e| CandleError::from(e))?
            .to_scalar::<f64>()
            .map_err(|e| CandleError::from(e))?;
...
pub struct CandleTokenStream {
    receiver: mpsc::UnboundedReceiver<Result<CompletionResponse, fluent_ai_core::completion::error::CompletionError>>,
}

impl CandleTokenStream {
    /// Create a new token stream
    #[inline(always)]
    pub fn new(receiver: mpsc::UnboundedReceiver<Result<CompletionResponse, fluent_ai_core::completion::error::CompletionError>>) -> Self {
        Self { receiver }
    }
}
