//! ImmutableChatCommand enum definition
//!
//! Provides the main command enum with all command variants and their implementations
//! using zero-allocation patterns and owned strings for blazing-fast performance.
...
impl ImmutableChatCommand {
    /// Create a simple custom command from a string (for macro compatibility)
    pub fn new(command_text: String) -> Self {
        // Simple parsing - treat as a custom command
        let parts: Vec<&str> = command_text.trim().split_whitespace().collect();
        let name = parts.first().unwrap_or(&"unknown").to_string();
        let args = HashMap::new(); // For simplicity, no argument parsing
