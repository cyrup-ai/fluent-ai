...
//!
//! Provides comprehensive built-in command definitions with parameter specifications,
//! usage examples, and category organization using zero-allocation patterns.

use crate::types::candle_chat::chat::commands::types::{
    CommandInfo, ParameterInfo, ParameterType};

/// Built-in command registry helper
pub struct BuiltinCommands;

impl BuiltinCommands {
    /// Get all built-in command definitions
    pub fn get_all() -> Vec<CommandInfo> {
        vec![
            Self::help_command(),
            Self::clear_command(),
            Self::export_command(),
    ...
    pub fn help_command() -> CommandInfo {
        CommandInfo {
            name: "help".to_string(),
    ...
    pub fn clear_command() -> CommandInfo {
        CommandInfo {
    ...
    pub fn export_command() -> CommandInfo {
        CommandInfo {
    ...
    pub fn config_command() -> CommandInfo {
        CommandInfo {
    ...
    pub fn search_command() -> CommandInfo {
        CommandInfo {
            name: "search".to_string(),
            description: "Search conversation history".to_string(),
            usage: "/search QUERY [--scope SCOPE] [--limit N] [--context]".to_string(),
            parameters: vec![
                ParameterInfo {
                ...
                ParameterInfo {
                    ...
                    description: "Search scope (all, current, recent, bookmarked)".to_string(),
                    parameter_type: ParameterType::Enum,
                    required: false,
            ...
            examples: vec![
                ...
                "/search \"machine learning\" --scope recent".to_string(),
                "/search error --limit 5 --context".to_string(),
            ]}
    }
}