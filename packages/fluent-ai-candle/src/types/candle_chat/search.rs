...
/// Search query with advanced filtering options - zero-allocation design
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct SearchQuery {
    /// Search terms - stack-allocated for typical queries (up to 8 terms)
    pub terms: SmallVec<Arc<str>, 8>,
    ...
    /// Tag filter - stack-allocated for typical tag lists (up to 4 tags)
    pub tag_filter: Option<SmallVec<Arc<str>, 4>>,
    /// Content type filter
...
/// Search result with relevance scoring - zero-allocation design
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct SearchResult {
    /// Message that matched
    pub message: crate::types::CandleSearchChatMessage,
    /// Relevance score (0.0-1.0)
    pub relevance_score: f32,
    /// Matching terms - stack-allocated for typical matches (up to 8 terms)
    pub matching_terms: SmallVec<Arc<str>, 8>,
    /// Highlighted content
    pub highlighted_content: Option<Arc<str>>,
    /// Associated tags - stack-allocated for typical tag lists (up to 4 tags)
    pub tags: SmallVec<Arc<str>, 4>,
    /// Context messages (before/after) - stack-allocated for typical context (up to 6 messages)
    pub context: SmallVec<crate::types::CandleSearchChatMessage, 6>,
    /// Match positions in the content - stack-allocated for typical matches (up to 16 positions)
    pub match_positions: SmallVec<MatchPosition, 16>,
    /// Search metadata
...
/// Inverted index entry - zero-allocation design
#[derive(Debug, Clone)]
pub struct IndexEntry {
    ...
    /// Positions of term in document - stack-allocated for typical term occurrences (up to 32 positions)
    pub positions: SmallVec<usize, 32>,
}

/// Chat search index with SIMD optimization - zero-allocation, lock-free design
pub struct ChatSearchIndex {
    /// Inverted index: term -> documents containing term - lock-free concurrent access
    inverted_index: Arc<SkipMap<Arc<str>, SmallVec<IndexEntry, 16>>>,
    /// Document store: doc_id -> message - lock-free concurrent access
    document_store: Arc<SkipMap<Arc<str>, crate::types::CandleSearchChatMessage>>,
    /// Term frequencies for TF-IDF calculation - lock-free concurrent access
    term_frequencies: Arc<SkipMap<Arc<str>, TermFrequency>>,
    /// Document count - cache-padded for performance
    document_count: CachePadded<AtomicUsize>,
    /// Query counter - cache-padded for performance
    query_counter: CachePadded<AtomicU64>,
    /// Index update counter - cache-padded for performance
    index_update_counter: CachePadded<AtomicU64>,
    /// Search statistics - lock-free atomic operations
    total_messages: CachePadded<AtomicUsize>,
    total_terms: CachePadded<AtomicUsize>, 
...
impl ChatSearchIndex {
    ...
    pub fn new() -> Self {
        Self {
            inverted_index: Arc::new(SkipMap::new()),
            document_store: Arc::new(SkipMap::new()),
            term_frequencies: Arc::new(SkipMap::new()),
            document_count: CachePadded::new(AtomicUsize::new(0)),
            query_counter: CachePadded::new(AtomicU64::new(0)),
            index_update_counter: CachePadded::new(AtomicU64::new(0)),
            total_messages: CachePadded::new(AtomicUsize::new(0)),
            total_terms: CachePadded::new(AtomicUsize::new(0)),
    ...
    pub fn add_message_stream(&self, message: crate::types::CandleSearchChatMessage) -> AsyncStream<()> {
    ...
    pub async fn add_message(&self, message: crate::types::CandleSearchChatMessage) -> Result<(), SearchError> {
            ...
            for result in results {
                ...
                if keep_result && let Some(user_filter) = &query_clone.user_filter {
                    let role_filter = match user_filter.as_ref() {
                        "system" => crate::types::CandleMessageRole::System,
                        "user" => crate::types::CandleMessageRole::User,
                        "assistant" => crate::types::CandleMessageRole::Assistant,
                        "tool" => crate::types::CandleMessageRole::Tool,
                        _ => crate::types::CandleMessageRole::User, // Default fallback
                    };
...
impl ConversationTagger {
    ...
    pub fn auto_tag_message_stream(&self, message: crate::types::CandleSearchChatMessage) -> AsyncStream<Arc<str>> {
    ...
    pub fn auto_tag_message(&self, message: &crate::types::CandleSearchChatMessage) -> AsyncStream<Vec<Arc<str>>> {
...
pub struct HistoryExporter {
    ...
    /// Export statistics - lock-free atomic operations
    total_exports: CachePadded<AtomicUsize>,
...
impl HistoryExporter {
    ...
    pub fn new() -> Self {
        Self {
            ...
            total_exports: CachePadded::new(AtomicUsize::new(0)),
                ...
                ExportFormat::Csv => {
                    ...
                    for message in &filtered_messages {
                        ...
                        let role = match message.message.role {
                            crate::types::CandleMessageRole::User => "user",
                            crate::types::CandleMessageRole::Assistant => "assistant",
                            crate::types::CandleMessageRole::System => "system",
                            crate::types::CandleMessageRole::Tool => "tool",
                        };
    ...
    fn filter_messages_stream(
        ...
        messages: Vec<crate::types::CandleSearchChatMessage>,
        options: &ExportOptions,
    ) -> AsyncStream<crate::types::CandleSearchChatMessage> {
    ...
    async fn filter_messages(
        ...
        messages: Vec<crate::types::CandleSearchChatMessage>,
        options: &ExportOptions,
    ) -> Result<Vec<crate::types::CandleSearchChatMessage>, SearchError> {
    ...
    async fn export_json(
        &self,
        messages: &[crate::types::CandleSearchChatMessage],
        options: &ExportOptions,
    ) -> Result<String, SearchError> {
...
impl EnhancedHistoryManager {
    ...
    pub fn new() -> Self {
        Self {
            ...
            total_operations: CachePadded::new(AtomicUsize::new(0)),
            system_uptime: CachePadded::new(AtomicU64::new(0)),
        }
    }
