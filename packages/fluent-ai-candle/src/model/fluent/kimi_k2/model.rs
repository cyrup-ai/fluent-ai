...
impl RotaryEmbedding {
    ...
    pub fn apply_rotary_emb(&self, q: &Tensor, k: &Tensor, position: usize) -> Result<(Tensor, Tensor)> {
        let seq_len = q.dim(1)?;
...
impl KimiK2Attention {
    ...
    pub fn forward(&self, x: &Tensor, position: usize) -> Result<Tensor> {
        ...
        let k = self.k_proj.forward(x)?
            .reshape((batch_size, seq_len, self.num_kv_heads, self.kv_lora_rank / self.num_kv_heads))?
            .transpose(1, 2)?;
    ...
    pub fn forward(&self, x: &Tensor) -> Result<Tensor> {
        ...
        for expert_idx in 0..self.experts.len() {
            ...
            if expert_mask.sum_all()?.to_scalar::<f32>()? > 0.0 {
                ...
                let weighted_output = expert_output.broadcast_mul(&expert_mask.to_dtype(x_flat.dtype())?)?;
...
impl KimiK2Block {
    pub fn new(
        ...
        let post_attention_layernorm = rms_norm(hidden_size, config.rms_norm_eps, vb.pp("post_attention_layernorm"))?;
        
        let (mlp, moe) = if is_moe_layer {
            (None, Some(KimiK2MoE::new(config, vb.pp("mlp"))?))
        } else {
            (Some(linear(hidden_size, config.intermediate_size, vb.pp("mlp"))?), None)
        };
