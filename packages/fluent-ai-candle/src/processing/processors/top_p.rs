    ...
    pub fn new(p: f32) -> ProcessingResult<Self> {
        if p <= 0.0 || p > 1.0 {
            return Err(ProcessingError::InvalidConfiguration(
                format!("Top-P value must be in range (0.0, 1.0], got {}", p)
            ));
        }
    ...
    fn apply_nucleus_filtering(&mut self, logits: &mut [f32]) -> ProcessingResult<()> {
        ...
        if total_prob <= 0.0 || !total_prob.is_finite() {
            return Err(ProcessingError::NumericalError(
                "Invalid probability distribution - total probability is zero or infinite".to_string()
            ));
        }
...
impl LogitsProcessor for TopPProcessor {
    ...
    fn validate(&self) -> ProcessingResult<()> {
        if self.p <= 0.0 || self.p > 1.0 {
            return Err(ProcessingError::InvalidConfiguration(
                format!("Top-P value must be in range (0.0, 1.0], got {}", self.p)
            ));
        }
        Ok(())
    }
...
mod tests {
    ...
    fn test_nucleus_filtering() {
        ...
        identity_processor.process_logits(&mut identity_logits, &context).unwrap();
    ...
    fn test_numerical_stability() {
        ...
        assert!(processor.process_logits(&mut large_logits, &context).is_ok());

        // Test with small logit values
        let mut small_logits = vec![-100.0, -101.0, -102.0, -103.0, -104.0];
        assert!(processor.process_logits(&mut small_logits, &context).is_ok());
    }
