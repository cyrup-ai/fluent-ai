...
use crate::processing::traits::{
    ...
    utils::{validate_logits, clamp_for_stability, find_max_logit}
};
    ...
    pub fn new(temperature: f32) -> ProcessingResult<Self> {
        ...
        if !temperature.is_finite() {
            return Err(ProcessingError::configuration(
                format!("Temperature must be finite, got: {}", temperature)
            ));
        }
        ...
        if temperature < Self::MIN_TEMPERATURE {
            return Err(ProcessingError::configuration(
                format!("Temperature {} is below minimum {}", temperature, Self::MIN_TEMPERATURE)
            ));
        }
        
        if temperature > Self::MAX_TEMPERATURE {
            return Err(ProcessingError::configuration(
                format!("Temperature {} exceeds maximum {}", temperature, Self::MAX_TEMPERATURE)
            ));
        }
        ...
        for (i, &logit) in logits.iter().enumerate() {
            if !logit.is_finite() {
                return Err(ProcessingError::numerical(
                    format!("Temperature scaling produced non-finite logit at index {}: {}", i, logit)
                ));
            }
        }
...
impl LogitsProcessor for TemperatureProcessor {
    ...
    fn validate(&self) -> ProcessingResult<()> {
        ...
        if !self.temperature.is_finite() || self.temperature <= 0.0 {
            return Err(ProcessingError::configuration(
                format!("Invalid temperature in processor: {}", self.temperature)
            ));
        }
        
        if self.temperature < Self::MIN_TEMPERATURE || self.temperature > Self::MAX_TEMPERATURE {
            return Err(ProcessingError::configuration(
                format!("Temperature {} is outside valid range [{}, {}]", 
...
pub mod utils {
    ...
    pub fn adaptive_temperature(
        ...
        adjusted = adjusted.clamp(TemperatureProcessor::MIN_TEMPERATURE, TemperatureProcessor::MAX_TEMPERATURE);
    ...
    pub fn temperature_from_entropy(desired_entropy: f32) -> ProcessingResult<f32> {
        if !desired_entropy.is_finite() {
            return Err(ProcessingError::configuration("Desired entropy must be finite"));
        }
        
        if !(0.0..=1.0).contains(&desired_entropy) {
            return Err(ProcessingError::configuration(
                format!("Desired entropy {} must be between 0.0 and 1.0", desired_entropy)
            ));
        }
    ...
    pub fn find_temperature_for_perplexity(
        ...
        if target_perplexity <= 1.0 {
            return Err(ProcessingError::configuration("Target perplexity must be > 1.0"));
        }
