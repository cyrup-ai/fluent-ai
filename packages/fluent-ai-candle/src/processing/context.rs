...
impl ProcessingContext {
    ...
    pub fn new(vocab_size: usize, context_size: usize) -> ProcessingResult<Self> {
        if vocab_size == 0 {
            return Err(ProcessingError::configuration("Vocabulary size cannot be zero"));
        }
        
        if vocab_size > MAX_VOCAB_SIZE {
            return Err(ProcessingError::configuration(
                format!("Vocabulary size {} exceeds maximum {}", vocab_size, MAX_VOCAB_SIZE)
            ));
        }
        
        if context_size == 0 {
            return Err(ProcessingError::configuration("Context size cannot be zero"));
        }
        
        if context_size > MAX_CONTEXT_SIZE {
            return Err(ProcessingError::configuration(
                format!("Context size {} exceeds maximum {}", context_size, MAX_CONTEXT_SIZE)
            ));
        }
    ...
    pub fn unique_token_count(&self) -> usize {
        self.token_frequencies.iter().filter(|&&freq| freq > 0).count()
    }
...
pub mod utils {
    ...
    pub fn validate_context_config(
        ...
        if vocab_size == 0 || vocab_size > MAX_VOCAB_SIZE {
            return Err(ProcessingError::configuration(
                format!("Invalid vocabulary size: {}", vocab_size)
            ));
        }

        if context_size == 0 || context_size > MAX_CONTEXT_SIZE {
            return Err(ProcessingError::configuration(
                format!("Invalid context size: {}", context_size)
            ));
        }
