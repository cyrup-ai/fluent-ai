...
impl CandleModel {
    ...
    pub async fn load_from_file<P: AsRef<Path>>(&self, path: P) -> CandleResult<()> {
        ...
        let model_type = ModelType::from_str(
            path.file_name()
                .and_then(|n| n.to_str())
                .unwrap_or("")
        );
        ...
        let (model, config) = match model_type {
            ...
            ModelType::Custom => return Err(CandleError::InvalidModelFormat("Unsupported model type")),
        };
    ...
    async fn load_llama_model(&self, mmap: &Mmap) -> CandleResult<(Box<dyn Module + Send + Sync>, ModelConfig)> {
        use candle_transformers::models::llama as llama_models;
        use safetensors::SafeTensors;
        
        // Parse safetensors file to get model weights
        let safetensors = SafeTensors::deserialize(mmap)
            .map_err(|e| CandleError::ModelLoadError(format!("Failed to parse safetensors: {}", e)))?;
        
        // Extract model configuration from tensor metadata or use defaults
        let config_json = safetensors.metadata()
            .and_then(|m| m.get("model_config"))
            .unwrap_or("{}");
        
        // Parse LLaMA config or use sensible defaults
        let llama_config = if let Ok(parsed_config) = serde_json::from_str::<serde_json::Value>(config_json) {
            llama_models::Config {
                hidden_size: parsed_config.get("hidden_size")
                    .and_then(|v| v.as_u64()).unwrap_or(4096) as usize,
                intermediate_size: parsed_config.get("intermediate_size")
                    .and_then(|v| v.as_u64()).unwrap_or(11008) as usize,
                vocab_size: parsed_config.get("vocab_size")
                    .and_then(|v| v.as_u64()).unwrap_or(32000) as usize,
                num_hidden_layers: parsed_config.get("num_hidden_layers")
                    .and_then(|v| v.as_u64()).unwrap_or(32) as usize,
                num_attention_heads: parsed_config.get("num_attention_heads")
                    .and_then(|v| v.as_u64()).unwrap_or(32) as usize,
                num_key_value_heads: parsed_config.get("num_key_value_heads")
                    .and_then(|v| v.as_u64()).unwrap_or(32) as usize,
                rms_norm_eps: parsed_config.get("rms_norm_eps")
                    .and_then(|v| v.as_f64()).unwrap_or(1e-6) as f64,
                rope_theta: parsed_config.get("rope_theta")
                    .and_then(|v| v.as_f64()).unwrap_or(10000.0) as f32,
                max_position_embeddings: parsed_config.get("max_position_embeddings")
                    .and_then(|v| v.as_u64()).unwrap_or(4096) as usize,
        ...
        let vs = candle_nn::VarBuilder::from_safetensors(vec![&safetensors], candle_core::DType::F16, &self.device)
            .map_err(|e| CandleError::ModelLoadError(format!("Failed to create variable store: {}", e)))?;
        
        // Load LLaMA model
        let llama_model = llama_models::Llama::load(&vs, &llama_config)
            .map_err(|e| CandleError::ModelLoadError(format!("Failed to load LLaMA model: {}", e)))?;
    ...
    async fn load_mistral_model(&self, mmap: &Mmap) -> CandleResult<(Box<dyn Module + Send + Sync>, ModelConfig)> {
        ...
        let mistral_config = if let Ok(parsed_config) = serde_json::from_str::<serde_json::Value>(config_json) {
            mistral_models::Config {
                ...
                hidden_size: parsed_config.get("hidden_size")
                    .and_then(|v| v.as_u64()).unwrap_or(4096) as usize,
                intermediate_size: parsed_config.get("intermediate_size")
                    .and_then(|v| v.as_u64()).unwrap_or(14336) as usize,
                num_hidden_layers: parsed_config.get("num_hidden_layers")
                    .and_then(|v| v.as_u64()).unwrap_or(32) as usize,
                num_attention_heads: parsed_config.get("num_attention_heads")
                    .and_then(|v| v.as_u64()).unwrap_or(32) as usize,
                ...
                rms_norm_eps: parsed_config.get("rms_norm_eps")
                    .and_then(|v| v.as_f64()).unwrap_or(1e-5) as f64,
                rope_theta: parsed_config.get("rope_theta")
                    .and_then(|v| v.as_f64()).unwrap_or(10000.0) as f32,
                sliding_window: parsed_config.get("sliding_window")
        ...
        // Load Mistral model
        let mistral_model = mistral_models::Model::load(&vs, &mistral_config)
            .map_err(|e| CandleError::ModelLoadError(format!("Failed to load Mistral model: {}", e)))?;
    ...
    async fn load_gemma_model(&self, mmap: &Mmap) -> CandleResult<(Box<dyn Module + Send + Sync>, ModelConfig)> {
        ...
        let gemma_config = if let Ok(parsed_config) = serde_json::from_str::<serde_json::Value>(config_json) {
            gemma_models::Config {
                vocab_size: parsed_config.get("vocab_size")
                    .and_then(|v| v.as_u64()).unwrap_or(256000) as usize,
                hidden_size: parsed_config.get("hidden_size")
                    .and_then(|v| v.as_u64()).unwrap_or(3072) as usize,
                intermediate_size: parsed_config.get("intermediate_size")
                    .and_then(|v| v.as_u64()).unwrap_or(24576) as usize,
                num_hidden_layers: parsed_config.get("num_hidden_layers")
                    .and_then(|v| v.as_u64()).unwrap_or(28) as usize,
                num_attention_heads: parsed_config.get("num_attention_heads")
                    .and_then(|v| v.as_u64()).unwrap_or(16) as usize,
                num_key_value_heads: parsed_config.get("num_key_value_heads")
                    .and_then(|v| v.as_u64()).unwrap_or(16) as usize,
                head_dim: parsed_config.get("head_dim")
                    .and_then(|v| v.as_u64()).unwrap_or(256) as usize,
                max_position_embeddings: parsed_config.get("max_position_embeddings")
                    .and_then(|v| v.as_u64()).unwrap_or(8192) as usize,
                rms_norm_eps: parsed_config.get("rms_norm_eps")
                    .and_then(|v| v.as_f64()).unwrap_or(1e-6) as f64,
                rope_theta: parsed_config.get("rope_theta")
                    .and_then(|v| v.as_f64()).unwrap_or(10000.0) as f32,
                attention_bias: parsed_config.get("attention_bias")
                    .and_then(|v| v.as_bool()).unwrap_or(false),
                mlp_bias: parsed_config.get("mlp_bias")
                    .and_then(|v| v.as_bool()).unwrap_or(false),
            }
        } else {
        ...
        // Load Gemma model
        let gemma_model = gemma_models::Model::load(&vs, &gemma_config)
            .map_err(|e| CandleError::ModelLoadError(format!("Failed to load Gemma model: {}", e)))?;
    ...
    async fn load_phi_model(&self, mmap: &Mmap) -> CandleResult<(Box<dyn Module + Send + Sync>, ModelConfig)> {
        ...
        let phi_config = if let Ok(parsed_config) = serde_json::from_str::<serde_json::Value>(config_json) {
            phi_models::Config {
                vocab_size: parsed_config.get("vocab_size")
                    .and_then(|v| v.as_u64()).unwrap_or(51200) as usize,
                hidden_size: parsed_config.get("hidden_size")
                    .and_then(|v| v.as_u64()).unwrap_or(2560) as usize,
                intermediate_size: parsed_config.get("intermediate_size")
                ...
                layer_norm_eps: parsed_config.get("layer_norm_eps")
                    .and_then(|v| v.as_f64()).unwrap_or(1e-5) as f64,
                rope_theta: parsed_config.get("rope_theta")
                    .and_then(|v| v.as_f64()).unwrap_or(10000.0) as f32,
                partial_rotary_factor: parsed_config.get("partial_rotary_factor")
                    .and_then(|v| v.as_f64()).unwrap_or(0.5) as f64,
                qk_layernorm: parsed_config.get("qk_layernorm")
                    .and_then(|v| v.as_bool()).unwrap_or(false),
            }
        } else {
    ...
    async fn load_qwen_model(&self, mmap: &Mmap) -> CandleResult<(Box<dyn Module + Send + Sync>, ModelConfig)> {
        ...
        let qwen_config = if let Ok(parsed_config) = serde_json::from_str::<serde_json::Value>(config_json) {
            qwen_models::Config {
                vocab_size: parsed_config.get("vocab_size")
                    .and_then(|v| v.as_u64()).unwrap_or(151936) as usize,
                hidden_size: parsed_config.get("hidden_size")
                    .and_then(|v| v.as_u64()).unwrap_or(3584) as usize,
                intermediate_size: parsed_config.get("intermediate_size")
                    .and_then(|v| v.as_u64()).unwrap_or(18944) as usize,
                num_hidden_layers: parsed_config.get("num_hidden_layers")
                    .and_then(|v| v.as_u64()).unwrap_or(28) as usize,
                num_attention_heads: parsed_config.get("num_attention_heads")
                    .and_then(|v| v.as_u64()).unwrap_or(28) as usize,
                ...
                max_position_embeddings: parsed_config.get("max_position_embeddings")
                    .and_then(|v| v.as_u64()).unwrap_or(32768) as usize,
                sliding_window: parsed_config.get("sliding_window")
                    .and_then(|v| v.as_u64()).map(|v| v as usize).unwrap_or(32768),
                max_window_layers: parsed_config.get("max_window_layers")
                    .and_then(|v| v.as_u64()).unwrap_or(28) as usize,
                tie_word_embeddings: parsed_config.get("tie_word_embeddings")
                    .and_then(|v| v.as_bool()).unwrap_or(false),
                rope_theta: parsed_config.get("rope_theta")
                    .and_then(|v| v.as_f64()).unwrap_or(1000000.0) as f64,
                rms_norm_eps: parsed_config.get("rms_norm_eps")
                    .and_then(|v| v.as_f64()).unwrap_or(1e-6) as f64,
                use_sliding_window: parsed_config.get("use_sliding_window")
                    .and_then(|v| v.as_bool()).unwrap_or(false),
            }
        } else {
        ...
        // Load Qwen model
        let qwen_model = qwen_models::Model::load(&vs, &qwen_config)
            .map_err(|e| CandleError::ModelLoadError(format!("Failed to load Qwen model: {}", e)))?;
