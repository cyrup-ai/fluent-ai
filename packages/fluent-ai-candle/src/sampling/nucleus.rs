...
impl TopPProcessor {
    ...
    fn apply_nucleus_sampling(&self, logits: &Tensor) -> Result<Tensor, SamplingError> {
        ...
        indices.sort_by(|&a, &b| {
            prob_vec[b].partial_cmp(&prob_vec[a]).unwrap_or(std::cmp::Ordering::Equal)
        });
        ...
        let masked_logits: Vec<f32> = logits_vec.iter()
            .zip(mask.iter())
        ...
        let output_vec = masked_tensor.to_vec1::<f32>().map_err(SamplingError::from)?;
...
pub mod utils {
    ...
    pub fn estimate_diversity_score(
        logits: &Tensor,
        top_p: f32
