...
impl TemperatureProcessor {
    ...
    fn apply_temperature_scaling(&self, logits: &Tensor) -> Result<Tensor, SamplingError> {
        ...
        let processed_logits = if has_inf {
            ...
            let clamped: Vec<f32> = logits_vec.iter().map(|&x| {
        ...
        let scaled = processed_logits.affine(self.inv_temperature as f64, 0.0)
            .map_err(SamplingError::from)?;
...
pub mod utils {
    ...
    pub fn adaptive_temperature(
        ...
        adjusted.clamp(TemperatureProcessor::MIN_TEMPERATURE, TemperatureProcessor::MAX_TEMPERATURE)
    }
...
mod tests {
    ...
    fn test_temperature_builder() -> Result<(), Box<dyn std::error::Error>> {
        let processor = TemperatureBuilder::new()
            .creative()
            .build()?;
