...
impl EvolutionManager {
    ...
    pub async fn process_decision(
        ...
        if decision.is_positive() && decision.confidence > 0.5 {
            // Update best state
            *self.current_best.write().await = candidate_state;
            ...
            info!(
                ...
                action, decision.decision_score, decision.confidence
            );
            ...
            failed.push(FailedAttempt {
                ...
                reasons: if decision.is_positive() {
                    vec![format!("Low confidence: {:.2}", decision.confidence)]
                } else {
                    decision.dissenting_opinions.iter().map(|s| s.to_string()).collect()
                },
                suggestions: Vec::new(), // improvement_suggestions not available in new ConsensusDecision
            });
    ...
    pub async fn generate_enhanced_prompt(
        ...
        let mut all_suggestions: Vec<_> = failed
            .iter()
