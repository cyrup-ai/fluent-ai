...
pub use committee_types::{
    ModelType, Model, EvaluationConfig, CommitteeEvaluation, ConsensusDecision,
    EvaluationResult, CommitteeMetrics, CacheEntry, CacheMetrics
};
...
pub use committee_orchestrator::{
    CommitteeEvaluator, EvaluationWorkflow, CommitteeCoordinator
};
...
pub mod api {
    ...
    pub async fn comprehensive_evaluate(
        ...
        committee.evaluate_optimization(
            optimization_spec,
            current_state,
            proposed_state,
        ).await
...
pub mod utils {
    ...
    pub fn extract_key_insights(evaluations: &[CommitteeEvaluation]) -> Vec<String> {
        ...
        let reasoning_terms: Vec<_> = evaluations
            .iter()
            .flat_map(|e| e.reasoning.split_whitespace())
...
mod tests {
    ...
    fn test_committee_config_validation() {
        let valid_config = EvaluationConfig {
            models: vec![ModelType::Gpt4O, ModelType::Claude3Sonnet, ModelType::Claude3Haiku],
            timeout: Duration::from_secs(30),
            consensus_threshold: 0.7,
        };
