...
impl ComponentChecker for DatabaseHealthChecker {
    ...
    fn check(&self) -> PendingComponentHealth {
        ...
        tokio::spawn(async move {
            ...
            let (connection_result, query_duration, (active_conns, max_conns)) = tokio::join!(
                connection_test,
            ...
            match connection_result {
                Ok(Ok(_)) => {
                    ...
                    health.details.insert("connection_time_ms".to_string(), 
                        serde_json::Value::Number(serde_json::Number::from(total_duration.as_millis() as u64)));
                    health.details.insert("query_time_ms".to_string(), 
                        serde_json::Value::Number(serde_json::Number::from(query_duration.as_millis() as u64)));
                    health.details.insert("active_connections".to_string(), 
                        serde_json::Value::Number(serde_json::Number::from(active_conns)));
                    health.details.insert("max_connections".to_string(), 
                        serde_json::Value::Number(serde_json::Number::from(max_conns)));
                    health.details.insert("connection_utilization".to_string(), 
                        serde_json::Value::String(format!("{:.1}%", (active_conns as f64 / max_conns as f64) * 100.0)));
                ...
                Ok(Err(e)) => {
                    ...
                    health.details.insert("error_type".to_string(), serde_json::Value::String("query_error".to_string()));
                }
                Err(_) => {
                    health.status = HealthStatus::Unhealthy;
                    health.message = Some("Database connection timeout".to_string());
                    health.details.insert("error_type".to_string(), serde_json::Value::String("timeout".to_string()));
                    health.details.insert("timeout_ms".to_string(), serde_json::Value::Number(serde_json::Number::from(5000)));
                }
            }
        ...
        tokio::spawn(async move {
            ...
            match connectivity_result {
                Ok(Ok(_)) => {
                    ...
                    health.details.insert("health_check_time_ms".to_string(), 
                        serde_json::Value::Number(serde_json::Number::from(total_check_duration.as_millis() as u64)));
                    health.details.insert("vector_count".to_string(), 
                        serde_json::Value::Number(serde_json::Number::from(vector_count)));
                    health.details.insert("vector_dimensions".to_string(), 
                        serde_json::Value::Number(serde_json::Number::from(dimensions)));
                    health.details.insert("index_quality_percent".to_string(), 
                        serde_json::Value::String(format!("{:.1}", index_quality)));
                    health.details.insert("search_latency_ms".to_string(), 
                        serde_json::Value::Number(serde_json::Number::from(search_duration.as_millis() as u64)));
                    health.details.insert("search_results_found".to_string(), 
                        serde_json::Value::Number(serde_json::Number::from(search_results)));
                    health.details.insert("memory_usage_mb".to_string(), 
                        serde_json::Value::Number(serde_json::Number::from(used_memory)));
                    health.details.insert("memory_total_mb".to_string(), 
                        serde_json::Value::Number(serde_json::Number::from(total_memory)));
                    health.details.insert("memory_utilization_percent".to_string(), 
                        serde_json::Value::String(format!("{:.1}", (used_memory as f64 / total_memory as f64) * 100.0)));
                    ...
                    if index_quality < 90.0 {
                        health.status = HealthStatus::Degraded;
                        health.message = Some("Vector index quality below optimal threshold".to_string());
                    }
                ...
                Ok(Err(e)) => {
                    ...
                    health.details.insert("error_type".to_string(), serde_json::Value::String("operation_error".to_string()));
                }
                Err(_) => {
                    health.status = HealthStatus::Unhealthy;
                    health.message = Some("Vector store connection timeout".to_string());
                    health.details.insert("error_type".to_string(), serde_json::Value::String("connectivity_timeout".to_string()));
                    health.details.insert("timeout_ms".to_string(), serde_json::Value::Number(serde_json::Number::from(3000)));
                }
            }
