...
#[command(long_about = "Downloads and parses AI model information from the aichat repository using yyaml exclusively. Features intelligent caching and zero fallback logic for maximum reliability.")]
struct Cli {
    ...
    match cli.format {
        OutputFormat::Table => {
            ...
            for provider in &providers {
                let vision_count = provider.models.iter()
    ...
    match cli.format {
        OutputFormat::Table => {
            ...
            for model in &provider.models {
                println!("│ {:23} │ {:11} │ {:11} │ {:11} │",
                    model.name,
                    model.max_input_tokens.map(|t| t.to_string()).unwrap_or_else(|| "N/A".to_string()),
                    model.max_output_tokens.map(|t| t.to_string()).unwrap_or_else(|| "N/A".to_string()),
                    if model.supports_vision.unwrap_or(false) { "Yes" } else { "No" }
                );
            }
...
#[inline(always)]
async fn show_model(cli: &Cli, provider_name: &str, model_name: &str) -> Result<(), Box<dyn std::error::Error>> {
    let yaml_content = download::download_yaml_with_cache(&cli.cache_dir).await?;
    let providers: Vec<YamlProvider> = yyaml::from_str(&yaml_content)?;
    ...
    let model = provider.models.iter()
        ...
        .ok_or_else(|| format!("Model '{}' not found in provider '{}'", model_name, provider_name))?;
    
    match cli.format {
        OutputFormat::Table => {
            println!("Model: {}", model.identifier(provider.identifier()));
            println!("Provider: {}", provider.identifier());
            println!("Max Input Tokens: {}", model.max_input_tokens.map(|t| t.to_string()).unwrap_or_else(|| "N/A".to_string()));
            println!("Max Output Tokens: {}", model.max_output_tokens.map(|t| t.to_string()).unwrap_or_else(|| "N/A".to_string()));
            println!("Input Price: {}", model.input_price.map(|p| p.to_string()).unwrap_or_else(|| "N/A".to_string()));
            println!("Output Price: {}", model.output_price.map(|p| p.to_string()).unwrap_or_else(|| "N/A".to_string()));
            println!("Supports Vision: {}", if model.supports_vision.unwrap_or(false) { "Yes" } else { "No" });
            println!("Supports Function Calling: {}", if model.supports_function_calling.unwrap_or(false) { "Yes" } else { "No" });
        }
        OutputFormat::Json => {
...
async fn refresh_cache(cli: &Cli) -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("✅ Successfully downloaded and parsed {} providers", providers.len());
