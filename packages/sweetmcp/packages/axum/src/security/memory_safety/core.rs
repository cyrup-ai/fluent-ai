...
impl MemorySafetyResult {
    ...
    pub fn most_critical_violation(&self) -> Option<&MemorySafetyViolation> {
        self.violations.iter().max_by_key(|v| v.priority_score())
    }

    /// Sort violations by priority
    pub fn sort_by_priority(&mut self) {
        self.violations
            .sort_by_key(|v| std::cmp::Reverse(v.priority_score()));
    }
...
impl MemorySafetyMetrics {
    ...
    pub fn record_validation(&self, result: &MemorySafetyResult) {
        ...
        self.total_operations_validated
            .fetch_add(result.operations_validated as u64, Ordering::Relaxed);
        ...
        for violation in &result.violations {
            match violation.violation_type {
                SafetyViolationType::BufferOverflow => {
                    self.buffer_overflow_violations
                        .fetch_add(1, Ordering::Relaxed);
                }
                SafetyViolationType::UseAfterFree => {
                    self.use_after_free_violations
                        .fetch_add(1, Ordering::Relaxed);
                }
