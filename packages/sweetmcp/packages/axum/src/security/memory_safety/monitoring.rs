...
impl MemorySafetyMonitor {
    ...
    pub fn check_alerts(&self) -> Vec<AlertType> {
        ...
        if metrics.metrics_snapshot.buffer_overflow_violations
            > self.alert_thresholds.max_buffer_overflows
        {
            alerts.push(AlertType::BufferOverflow);
        }

        // Check use-after-free threshold
        if metrics.metrics_snapshot.use_after_free_violations
            > self.alert_thresholds.max_use_after_free
        {
        ...
        if metrics.metrics_snapshot.avg_validation_time_us
            > self.alert_thresholds.max_validation_time_us
        {
    ...
    pub fn generate_report(&self) -> MonitoringReport {
        ...
        let critical_violations =
            self.get_violations_by_severity(SafetyViolationSeverity::Critical);
    ...
    fn generate_recommendations(&self) -> Vec<String> {
        ...
        if alerts.contains(&AlertType::BufferOverflow) {
            recommendations.push(
                "Review buffer bounds checking and consider using safer memory operations"
                    .to_string(),
            );
        }
        ...
        if alerts.contains(&AlertType::UseAfterFree) {
            recommendations.push(
                "Implement stricter lifetime management and consider using smart pointers"
                    .to_string(),
            );
        }
        ...
        if alerts.contains(&AlertType::HighValidationTime) {
            recommendations
                .push("Consider optimizing validation rules or increasing cache size".to_string());
        }

        if alerts.contains(&AlertType::HighCacheMissRate) {
            recommendations.push(
                "Consider increasing cache size or reviewing cache key generation".to_string(),
            );
        }

        if metrics.cache_hit_rate() < 0.5 {
            recommendations.push(
                "Cache hit rate is very low - consider reviewing caching strategy".to_string(),
            );
        }

        if metrics.tracked_allocations > 10000 {
            recommendations
                .push("High number of tracked allocations - consider periodic cleanup".to_string());
        }
...
impl AlertType {
    ...
    pub fn severity(&self) -> SafetyViolationSeverity {
        match self {
            AlertType::BufferOverflow | AlertType::UseAfterFree => {
                SafetyViolationSeverity::Critical
            }
            AlertType::MemoryLeak => SafetyViolationSeverity::High,
            AlertType::LowSuccessRate | AlertType::HighValidationTime => {
