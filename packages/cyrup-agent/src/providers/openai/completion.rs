...
use crate::{completion, json_util, message, OneOrMany};
                        ...
                        Ok(response) => {
                            if response.status().is_success() {
                                match response.text().await {
                                    Ok(t) => {
                                        tracing::debug!(target: "rig", "OpenAI completion response: {}", t);
        ...
        match self.create_completion_request(request) {
            Ok(mut request) => {
                ...
                crate::runtime::spawn_async(async move {
                    ...
                    let result = super::streaming::send_compatible_streaming_request(builder).await;
                    tx.finish(result);
                });
            }
            Err(e) => {
