...
impl ProviderTrait for OpenAiProvider {
    ...
    fn list_models(&self) -> AsyncStream<ModelInfo> {
        ...
        impl From<fluent_ai_http3::BadChunk> for OpenAiModelsResponse {
        ...
        AsyncStream::with_channel(move |sender| {
            let response = if let Ok(api_key) = env::var("OPENAI_API_KEY") {
                Http3::json()
                    ...
                    .collect_one::<OpenAiModelsResponse>()
            } else {
                Http3::json()
                    .get("https://api.openai.com/v1/models")
                    .collect_one::<OpenAiModelsResponse>()
...
fn adapt_openai_to_model_info(model: &str) -> ModelInfo {
    ...
    ModelInfo {
        ...
        real_name: None,
        model_type: if model.contains("embedding") {
...
impl OpenAiModelExt for OpenAi {
    ...
    fn supports_function_calling(&self) -> bool {
        matches!(
            self,
            OpenAi::Gpt41 | OpenAi::Gpt41Mini | OpenAi::Gpt4o | OpenAi::Gpt4oMini
        )
    }
    ...
    pub fn cost_tier(model: &str) -> &'static str {
        OpenAi::from_model_str(model)
            ...
            .unwrap_or_else(|| match model {
                ...
                _ if model.contains("o1")
                    || model.contains("o3")
                    || (model.contains("gpt-4") && !model.contains("mini")) =>
                {
                    "premium"
                }
                _ if model.contains("mini") || model.contains("3.5") || model.contains("nano") => {
                    "budget"
                }
                _ => "standard",
            })
    ...
    pub fn model_tier(model: &str) -> Option<&'static str> {
        OpenAi::from_model_str(model)
            ...
            .unwrap_or_else(|| match model {
                ...
                GPT_4_1_MINI | GPT_4O_MINI | GPT_4_TURBO | TEXT_EMBEDDING_3_LARGE => {
                    Some("standard")
                }
