...
impl ProviderTrait for TogetherProvider {
    ...
    fn list_models(&self) -> AsyncStream<ModelInfo> {
        ...
        impl From<fluent_ai_http3::BadChunk> for TogetherModelsResponse {
        ...
        AsyncStream::with_channel(move |sender| {
            let response = if let Ok(api_key) = env::var("TOGETHER_API_KEY") {
                Http3::json()
                    ...
                    .collect_one::<TogetherModelsResponse>()
            } else {
                Http3::json()
                    .get("https://api.together.xyz/v1/models")
                    .collect_one::<TogetherModelsResponse>()
...
fn adapt_together_to_model_info(model: &str) -> ModelInfo {
    ...
    let map = MAP.get_or_init(|| {
        ...
        m.insert(
            ...
            (8192, 2048, 0.2, 0.2, false, false, true, false, false),
        );
        m.insert(
            "mistralai/Mixtral-8x7B-Instruct-v0.1",
            (32768, 8192, 0.27, 0.27, false, true, true, false, false),
        );
        m.insert(
            "togethercomputer/CodeLlama-34b-Instruct",
            (16384, 4096, 0.5, 0.5, false, false, true, false, false),
        );
        m
    });
