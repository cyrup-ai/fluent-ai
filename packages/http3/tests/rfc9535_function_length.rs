...
use fluent_ai_http3::json_path::{JsonPathParser, JsonArrayStream};
...
mod length_function_tests {
    ...
    fn test_length_performance_large_data() {
        ...
        let large_object: serde_json::Map<String, serde_json::Value> = 
            (0..100).map(|i| (format!("key_{}", i), serde_json::Value::Number(i.into())))
                   .collect();
        ...
        for (expr, expected_count) in test_cases {
            ...
            assert!(duration.as_millis() < 1000, 
                "length() performance test '{}' should complete in <1000ms", expr);
        }
    }
...
mod length_error_tests {
    ...
    fn test_length_invalid_syntax() {
        ...
        for expr in invalid_expressions {
            ...
            assert!(result.is_err(), "Invalid length() syntax '{}' should fail", expr);
        }
    }
    ...
    fn test_length_argument_validation() {
        ...
        for expr in validation_cases {
            ...
            assert!(result.is_ok() || result.is_err(), 
                "length() validation should handle expression properly: {}", expr);
        }
    }
            ...
            match result {
                Ok(_) => {
                    ...
                    assert!(results.len() >= 0, 
                        "Complex length() expression '{}' should return valid results", expr);
                }
                Err(_) => {
