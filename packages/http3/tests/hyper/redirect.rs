...
async fn test_redirect_301_and_302_and_303_changes_post_to_get() {
    ...
    for &code in &codes {
        ...
        assert_eq!(res.status(), crate::hyper::StatusCode::OK);
        assert_eq!(
            res.headers().get(crate::hyper::header::SERVER).unwrap(),
            &"test-dst"
        );
    }
}
...
fn test_redirect_307_does_not_try_if_reader_cannot_reset() {
    let client = crate::hyper::blocking::Client::new();
    let codes = [307u16, 308];
    for &code in &codes {
        let redirect = server::http(move |mut req| async move {
            assert_eq!(req.method(), "POST");
        ...
        let res = client
            .post(&url)
            .body(crate::hyper::blocking::Body::new(&b"Hello"[..]))
            .send()
            .unwrap();
...
async fn test_redirect_removes_sensitive_headers() {
    ...
    crate::hyper::Client::builder()
        .build()
        .unwrap()
        .get(&format!("http://{}/sensitive", mid_server.addr()))
        .header(
            crate::hyper::header::COOKIE,
            crate::hyper::header::HeaderValue::from_static("foo=bar"),
        )
        .send()
        .await
        .unwrap();
...
async fn test_redirect_policy_can_return_errors() {
    ...
    let err = crate::hyper::get(&url).await.unwrap_err();
...
async fn test_redirect_policy_can_stop_redirects_without_an_error() {
    ...
    let res = crate::hyper::Client::builder()
        .redirect(crate::hyper::redirect::Policy::none())
        .build()
        .unwrap()
        .get(&url)
        .send()
    ...
    assert_eq!(res.url().as_str(), url);
    assert_eq!(res.status(), crate::hyper::StatusCode::FOUND);
}
...
async fn test_invalid_location_stops_redirect_gh484() {
    ...
    let res = crate::hyper::get(&url).await.unwrap();
...
async fn test_redirect_302_with_set_cookies() {
    ...
    let client = crate::hyper::ClientBuilder::new()
        .cookie_store(true)
        .build()
        .unwrap();
    let res = client.get(&url).send().await.unwrap();
...
async fn test_redirect_limit_to_1() {
    ...
    let client = crate::hyper::Client::builder()
        .redirect(crate::hyper::redirect::Policy::limited(1))
        .build()
        .unwrap();
...
async fn test_redirect_custom() {
    ...
    let res = crate::hyper::Client::builder()
        .redirect(crate::hyper::redirect::Policy::custom(|attempt| {
...
async fn test_scheme_only_check_after_policy_return_follow() {
    ...
    let url = format!("http://{}/yikes", server.addr());
    let res = crate::hyper::Client::builder()
        .redirect(crate::hyper::redirect::Policy::custom(|attempt| attempt.stop()))
        .build()
        .unwrap()
        .get(&url)
        .send()
    ...
    assert!(res.is_ok());
    assert_eq!(res.unwrap().status(), crate::hyper::StatusCode::FOUND);
    ...
    for &code in &codes {
        let redirect = server::http(move |mut req| async move {
            if req.method() == "POST" {
                ...
                assert_eq!(&*data, b"Hello");
                if req.headers().get(crate::hyper::header::CONTENT_LENGTH).is_some() {
                    assert_eq!(req.headers()[crate::hyper::header::CONTENT_LENGTH], "5");
                }
                assert_eq!(req.uri(), &*format!("/{code}"));
                ...
                assert_eq!(req.method(), "GET");
                assert!(req.headers().get(crate::hyper::header::CONTENT_TYPE).is_none());
                assert!(req.headers().get(crate::hyper::header::CONTENT_LENGTH).is_none());
                assert!(
                    req.headers()
                        .get(crate::hyper::header::CONTENT_ENCODING)
                        .is_none()
                );
                http::Response::builder()
                    .header("server", "test-dst")
        ...
        let res = client
            .post(&url)
            .body("Hello")
            .header(crate::hyper::header::CONTENT_TYPE, "text/plain")
            .header(crate::hyper::header::CONTENT_LENGTH, "5")
            .header(crate::hyper::header::CONTENT_ENCODING, "identity")
            .send()
            .await
            .unwrap();
        assert_eq!(res.url().as_str(), dst);
