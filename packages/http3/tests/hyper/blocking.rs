...
fn test_response_text() {
    ...
    let res = crate::hyper::blocking::get(&url).unwrap();
...
fn donot_set_content_length_0_if_have_no_body() {
    ...
    let res = crate::hyper::blocking::Client::builder()
        .no_proxy()
        .build()
...
fn test_response_copy_to() {
    ...
    let mut res = crate::hyper::blocking::get(&url).unwrap();
    assert_eq!(res.url().as_str(), &url);
    assert_eq!(res.status(), crate::hyper::StatusCode::OK);
...
fn test_post() {
    ...
    let res = crate::hyper::blocking::Client::new()
        .post(&url)
        .body("Hello")
        .send()
        .unwrap();
...
fn test_error_for_status_4xx() {
    ...
    assert_eq!(err.status(), Some(crate::hyper::StatusCode::BAD_REQUEST));
}
...
fn test_error_for_status_5xx() {
    ...
    assert_eq!(
        err.status(),
        Some(crate::hyper::StatusCode::INTERNAL_SERVER_ERROR)
    );
}
...
fn test_default_headers() {
    let server = server::http(move |req| async move {
        assert_eq!(req.headers()["http3-test"], "orly");
        http::Response::default()
    });

    let mut headers = http::HeaderMap::with_capacity(1);
    headers.insert("http3-test", "orly".parse().unwrap());
    let client = crate::hyper::blocking::Client::builder()
...
fn test_blocking_inside_a_runtime() {
    ...
    rt.block_on(async move {
        let _should_panic = crate::hyper::blocking::get(&url);
    });
}
...
fn test_allowed_methods_blocking() {
    let resp = crate::hyper::blocking::Client::builder()
        .https_only(true)
        .build()
        .expect("client builder")
...
/// Test that a [`crate::hyper::blocking::Body`] can be created from [`bytes::Bytes`].
#[test]
fn test_body_from_bytes() {
    let body = "abc";
    // No external calls are needed. Only the request building is tested.
    let request = crate::hyper::blocking::Client::builder()
        .build()
...
fn blocking_add_json_default_content_type_if_not_set_manually() {
    ...
    let req = crate::hyper::blocking::Client::new()
...
fn test_response_no_tls_info_for_http() {
    ...
    assert_eq!(res.status(), crate::hyper::StatusCode::OK);
    assert_eq!(res.content_length(), Some(5));
    let tls_info = res.extensions().get::<crate::hyper::tls::TlsInfo>();
