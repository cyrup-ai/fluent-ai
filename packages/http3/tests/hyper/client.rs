...
async fn user_agent() {
    let server = server::http(move |req| async move {
        assert_eq!(req.headers()["user-agent"], "http3-test-agent");
        http::Response::default()
    });

    let url = format!("http://{}/ua", server.addr());
    let res = crate::hyper::Client::builder()
        .user_agent("http3-test-agent")
        .build()
        .expect("client builder")
        .get(&url)
        .send()
...
async fn body_pipe_response() {
    ...
    assert_eq!(res1.status(), crate::hyper::StatusCode::OK);
    ...
    assert_eq!(res2.status(), crate::hyper::StatusCode::OK);
}
...
async fn http2_upgrade() {
    ...
    assert_eq!(res.status(), crate::hyper::StatusCode::OK);
    assert_eq!(res.version(), crate::hyper::Version::HTTP_2);
}
...
async fn test_allowed_methods() {
    let resp = crate::hyper::Client::builder()
        .https_only(true)
        .build()
        .expect("client builder")
...
async fn test_tls_info() {
    let resp = crate::hyper::Client::builder()
        ...
        .await
        .expect("response");
    let tls_info = resp.extensions().get::<crate::hyper::tls::TlsInfo>();
...
async fn error_has_url() {
    ...
    let err = crate::hyper::get(u).await.unwrap_err();
