...
async fn http3_request_full() {
    ...
    let res_fut = crate::hyper::Client::builder()
...
async fn http3_test_concurrent_request() {
    let server = server::Http3::new().build(|req| async move {
        ...
        *res.body_mut() = crate::hyper::Body::from(format!("hello {}", req.uri().path()));
        res
    });
    ...
    for (i, handler) in handlers.into_iter().enumerate() {
        ...
        assert_eq!(result.status(), crate::hyper::StatusCode::OK);
...
async fn http3_request_stream() {
    ...
    let body = crate::hyper::Body::wrap_stream(futures_util::stream::iter(vec![
...
async fn http3_request_stream_error() {
    ...
    let err = err
        ...
        .source()
        .unwrap()
        .downcast_ref::<crate::hyper::Error>()
        .unwrap();
    assert!(err.is_body());
}
