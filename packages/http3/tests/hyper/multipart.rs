...
async fn text_part() {
    ...
    let form = crate::hyper::multipart::Form::new().text("foo", "bar");
    ...
    let res = crate::hyper::Client::new()
        .post(&url)
        .multipart(form)
        .send()
        .await
...
async fn stream_part() {
    ...
    let stream = crate::hyper::Body::wrap_stream(stream::once(future::ready(Ok::<_, crate::hyper::Error>(
        "part1 part2".to_owned(),
    ))));
    let part = crate::hyper::multipart::Part::stream(stream);
...
fn blocking_file_part() {
    ...
    let form = crate::hyper::blocking::multipart::Form::new()
        .file("foo", "Cargo.lock")
        .unwrap();
