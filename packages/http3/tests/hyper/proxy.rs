...
async fn http_proxy() {
    ...
    let res = crate::hyper::Client::builder()
        .proxy(crate::hyper::Proxy::http(&proxy).unwrap())
        .build()
        .unwrap()
        .get(url)
        .send()
...
async fn test_custom_headers() {
    ...
    let proxy = format!("http://{}", server.addr());
    let mut headers = crate::hyper::header::HeaderMap::new();
    headers.insert(
        // crate::hyper::header::HeaderName::from_static("Proxy-Authorization"),
        crate::hyper::header::PROXY_AUTHORIZATION,
        "Basic QWxhZGRpbjpvcGVuIHNlc2FtZQ==".parse().unwrap(),
    );

    let res = crate::hyper::Client::builder()
        .proxy(crate::hyper::Proxy::http(&proxy).unwrap().headers(headers))
        .build()
        .unwrap()
        .get(url)
        .send()
...
async fn tunnel_detects_auth_required() {
    ...
    let err = crate::hyper::Client::builder()
        .proxy(crate::hyper::Proxy::https(&proxy).unwrap())
        .build()
        .unwrap()
        .get(url)
        .send()
...
async fn tunnel_includes_user_agent() {
    ...
    let server = server::http(move |req| {
        assert_eq!(req.method(), "CONNECT");
        assert_eq!(req.uri(), "hyper.rs.local:443");
        assert_eq!(req.headers()["user-agent"], "http3-test");
...
async fn tunnel_includes_proxy_auth_with_multiple_proxies() {
    ...
    let mut headers1 = crate::hyper::header::HeaderMap::new();
    headers1.insert("proxy-header", "proxy1".parse().unwrap());

    let mut headers2 = crate::hyper::header::HeaderMap::new();
    ...
    let client = crate::hyper::Client::builder()
        ...
        .proxy(
            crate::hyper::Proxy::https(&proxy_url)
                .unwrap()
        ...
        .proxy(
            crate::hyper::Proxy::http(&proxy_url)
                .unwrap()
    ...
    let client = crate::hyper::Client::builder()
        // When processing proxy headers, the first one is iterated,
        // and for the current URL matching, the proxy will be used
        .proxy(crate::hyper::Proxy::http(&proxy_url).unwrap().headers(headers2))
        // When processing proxy headers, the second one is iterated,
        // and if the current URL does not match, the proxy is skipped
        .proxy(crate::hyper::Proxy::https(&proxy_url).unwrap().headers(headers1))
        .build()
        .unwrap();
