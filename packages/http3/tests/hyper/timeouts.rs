...
async fn request_timeout() {
    ...
    let client = crate::hyper::Client::builder().no_proxy().build().unwrap();
...
async fn response_timeout() {
    ...
    let server = server::http(move |_req| {
        async {
            ...
            let body = crate::hyper::Body::wrap_stream(futures_util::stream::once(async {
...
async fn read_timeout_allows_slow_response_body() {
    ...
    let server = server::http(move |_req| {
        async {
            ...
            let body = crate::hyper::Body::wrap_stream(slow);
...
fn timeout_blocking_request() {
    ...
    // Make Client drop *after* the Server, so the background doesn't
    // close too early.
    let client = crate::hyper::blocking::Client::builder().build().unwrap();
...
fn write_timeout_large_body() {
    ...
    let err = client
        .post(&url)
        .body(crate::hyper::blocking::Body::sized(cursor, len as u64))
        .send()
        .unwrap_err();
