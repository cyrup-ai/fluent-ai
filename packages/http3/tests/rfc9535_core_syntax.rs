...
mod wellformedness_validity_tests {
    ...
    fn test_wellformed_syntactically_valid_paths() {
        ...
        let wellformed_tests = vec![
            ...
            ("$.store.book[*].nonexistent", "Valid structure with non-existent property"),
            ("$['']", "Empty string property name"),
            ("$[-999]", "Large negative index"),
            ("$.a.b.c.d.e.f", "Deep property chain"),
        ];

        for (expr, description) in wellformed_tests {
            let result = JsonPathParser::compile(expr);
            assert!(result.is_ok(), 
                "Well-formed expression '{}' should compile: {}", expr, description);
        }
    }
    ...
    fn test_malformed_syntactically_invalid_paths() {
        ...
        for (expr, description) in malformed_tests {
            let result = JsonPathParser::compile(expr);
            assert!(result.is_err(), 
                "Malformed expression '{}' should fail: {}", expr, description);
        }
    }
    ...
    fn test_semantic_validity_vs_syntax_validity() {
        ...
        let semantic_tests = vec![
            ...
            ("$.store.nonexistent.title", 0, "Valid syntax, non-existent property"),
            ("$.store.book[*].nonexistent", 0, "Valid syntax, non-existent nested property"),
            ("$.store.book[-1].title", 1, "Valid syntax, negative index semantics"),
            ("$.store.book[*].title", 2, "Valid syntax, wildcard semantics"),
        ];

        for (expr, expected_count, description) in semantic_tests {
            let mut stream = JsonArrayStream::<serde_json::Value>::new(expr);
            let chunk = Bytes::from(json_data);
            let results: Vec<_> = stream.process_chunk(chunk).collect();
            
            assert_eq!(results.len(), expected_count,
                "Semantic test '{}' should return {} results: {}", expr, expected_count, description);
        }
    }
    ...
    fn test_error_consistency_across_similar_patterns() {
        ...
        for error_group in [bracket_errors, quote_errors, dot_errors] {
            for expr in error_group {
                let result = JsonPathParser::compile(expr);
                assert!(result.is_err(), 
                    "Consistently malformed expression '{}' should fail", expr);
            }
        }
    }
}
...
mod nodelist_semantics_tests {
    ...
    fn test_empty_nodelist_conditions() {
        ...
        for (expr, description) in empty_result_tests {
            let mut stream = JsonArrayStream::<serde_json::Value>::new(expr);
            let chunk = Bytes::from(json_data);
            let results: Vec<_> = stream.process_chunk(chunk).collect();
            
            assert_eq!(results.len(), 0, 
                "Empty nodelist test '{}' should return 0 results: {}", expr, description);
        }
    }
...
mod path_evaluation_semantics_tests {
    ...
    fn test_root_value_semantics() {
        ...
        for (expr, expected_count, description) in root_tests {
            let mut stream = JsonArrayStream::<serde_json::Value>::new(expr);
            let chunk = Bytes::from(json_data);
            let results: Vec<_> = stream.process_chunk(chunk).collect();
    ...
    fn test_selector_evaluation_order() {
        ...
        let multiple_selector_tests = vec![
            ...
            ("$.items[1,0,3]", 3, "Multiple index selectors in specific order"),
            ("$.items[*]", 4, "Wildcard selector"),
        ];

        for (expr, expected_count, description) in multiple_selector_tests {
            let mut stream = JsonArrayStream::<serde_json::Value>::new(expr);
            let chunk = Bytes::from(json_data);
            let results: Vec<_> = stream.process_chunk(chunk).collect();
    ...
    fn test_descendant_operator_semantics() {
        ...
        let descendant_tests = vec![
            ...
            ("$.level1.level2..target", 2, "Target properties under level2"),
            ("$..level3.target", 1, "Specific path with descendant"),
        ];

        for (expr, expected_count, description) in descendant_tests {
            let mut stream = JsonArrayStream::<serde_json::Value>::new(expr);
            let chunk = Bytes::from(json_data);
            let results: Vec<_> = stream.process_chunk(chunk).collect();
    ...
    fn test_array_index_semantics() {
        ...
        for (expr, expected_count, description) in index_tests {
            let mut stream = JsonArrayStream::<serde_json::Value>::new(expr);
            let chunk = Bytes::from(json_data);
            let results: Vec<_> = stream.process_chunk(chunk).collect();
