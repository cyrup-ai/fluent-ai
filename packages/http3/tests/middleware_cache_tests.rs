...
fn test_cache_directive_parsing() {
    ...
    assert_eq!(middleware.parse_max_age_directive("max-age=3600"), Some(3600));
    assert_eq!(middleware.parse_max_age_directive("public, max-age=7200, must-revalidate"), Some(7200));
    assert_eq!(middleware.parse_max_age_directive("no-cache, no-store"), None);
...
fn test_cache_response_processing() {
    ...
    assert!(processed_response.headers().contains_key("x-computed-expires"));
}