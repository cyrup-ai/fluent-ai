...
mod ijson_boundary_tests {
    ...
    fn test_array_index_ijson_boundaries() {
        ...
        let boundary_tests = vec![
            ...
            ("$[1e16]".to_string(), false, "Scientific notation beyond safe range"),
        ...
        for (expr, should_be_valid, description) in boundary_tests {
            ...
            if should_be_valid {
                ...
                if let Err(JsonPathError::InvalidExpression { reason, .. }) = result {
                    assert!(
                        reason.contains("range") || reason.contains("I-JSON") || reason.contains("boundary"),
    ...
    fn test_array_slice_ijson_boundaries() {
        ...
        let slice_boundary_tests = vec![
            ...
            (format!("$[{}:]", MIN_SAFE_INTEGER - 1), false, "Start beyond min safe"),
            (format!("$[:{}]", MIN_SAFE_INTEGER - 1), false, "End beyond min safe"),
            (format!("$[::{}]", MIN_SAFE_INTEGER - 1), false, "Step beyond min safe"),
    ...
    fn test_comparison_operand_ijson_boundaries() {
        ...
        let comparison_tests = vec![
            ...
            (format!("$[?@.value == {}]", MAX_SAFE_INTEGER), true, "Max safe comparison"),
            (format!("$[?@.value == {}]", MIN_SAFE_INTEGER), true, "Min safe comparison"),
            ...
            (format!("$[?@.value < {}]", MIN_SAFE_INTEGER - 1), false, "Less than beyond min"),
            
            // Complex logical expressions with boundary violations
            (format!("$[?@.value > {} && @.other < {}]", MAX_SAFE_INTEGER + 1, 100), false, "Logical AND with violation"),
    ...
    fn test_edge_cases_at_ijson_boundaries() {
        ...
        let edge_cases = vec![
            ...
            ("$[9007199254740991.0]".to_string(), false, "Floating point max safe"),
            ("$[-9007199254740991.0]".to_string(), false, "Floating point min safe"),
            
            // String representations (should be valid if in I-JSON range)
            ("$['9007199254740991']".to_string(), true, "String representation of max safe"),
    ...
    fn test_execution_with_ijson_boundary_values() {
        ...
        let execution_tests = vec![
            ...
            (format!("$.items[?@.id == {}]", MAX_SAFE_INTEGER), 1, "Filter by max safe ID"),
            (format!("$.items[?@.id == {}]", MIN_SAFE_INTEGER), 1, "Filter by min safe ID"),
            (format!("$.items[?@.id > {}]", MIN_SAFE_INTEGER), 1, "Greater than min safe"),
...
mod precision_loss_tests {
    ...
    fn test_precision_loss_detection() {
        ...
        let precision_tests = vec![
            ...
            ("$[9007199254740992]".to_string(), false, "First integer with precision loss"),
            ("$[18014398509481984]".to_string(), false, "Large integer with precision loss"),
            ("$[-9007199254740992]".to_string(), false, "Negative integer with precision loss"),
            
            // Scientific notation that exceeds safe range
            ("$[9.007199254740992e15]".to_string(), false, "Scientific notation exceeding safe range"),
