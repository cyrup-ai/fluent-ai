...
mod unicode_escape_sequence_tests {
    ...
    fn test_basic_escape_sequences() {
        ...
        let test_cases = vec![
            ("$.strings[?@ =~ '.*\\\\b.*']", 1),    // Backspace
            ("$.strings[?@ =~ '.*\\\\t.*']", 1),    // Tab
            ("$.strings[?@ =~ '.*\\\\n.*']", 1),    // Newline
            ("$.strings[?@ =~ '.*\\\\f.*']", 1),    // Form feed
            ("$.strings[?@ =~ '.*\\\\r.*']", 1),    // Carriage return
            ("$.strings[?@ =~ '.*\\\\\".*']", 1),   // Quote
            ("$.strings[?@ =~ '.*\\\\\\'.*']", 1),  // Apostrophe
            ("$.strings[?@ =~ '.*\\\\/.*']", 1),    // Solidus
            ("$.strings[?@ =~ '.*\\\\\\\\.*']", 1), // Backslash
        ];
...
mod unicode_character_handling_tests {
    ...
    fn test_unicode_character_length() {
        ...
        let test_cases = vec![
            ("$.text[?length(@) == 5]", 2), // "ascii" and "café" (5 chars each)
            ("$.text[?length(@) == 3]", 3), // emoji string, Japanese, Arabic (3 chars each)
        ];
    ...
    fn test_invalid_surrogate_pairs() {
        ...
        for json_data in invalid_surrogates {
