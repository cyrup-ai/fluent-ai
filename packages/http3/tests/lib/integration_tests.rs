//! Integration tests for fluent_ai_http3_client library
//!
...
use fluent_ai_http3_client::prelude::*;
...
fn test_global_client_initialization() {
    let client = fluent_ai_http3_client::global_client();
    assert!(!client.is_closed());
}

#[test]
fn test_connection_stats() {
    let stats = fluent_ai_http3_client::connection_stats();
...
fn test_config_validation_timeout() {
    use fluent_ai_http3_client::config::HttpConfig;
    ...
    fluent_ai_http3_client::init_global_client(config);
