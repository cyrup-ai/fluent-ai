use fluent_ai_http3_client::proxy::into_proxy::{IntoProxy, validate_proxy_url, parse_proxy_url};
...
fn test_url_into_proxy() {
    let original_url = fluent_ai_http3_client::Url::parse("http://proxy.test").unwrap();
...
fn test_validate_proxy_url_valid() {
    let url = fluent_ai_http3_client::Url::parse("http://proxy.example.com:8080").unwrap();
    assert!(validate_proxy_url(&url).is_ok());
}

#[test]
fn test_validate_proxy_url_invalid_scheme() {
    let url = fluent_ai_http3_client::Url::parse("ftp://proxy.example.com").unwrap();
...
fn test_validate_proxy_url_no_host() {
    let url = fluent_ai_http3_client::Url::parse("http:///path").unwrap();
    let result = validate_proxy_url(&url);
    assert!(result.is_err());
