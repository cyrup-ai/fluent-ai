...
use fluent_ai_http3_client::proxy::types::{ProxyUrl, Intercept, Via, Extra};

#[test]
fn test_proxy_url_creation() {
    let url = fluent_ai_http3_client::Url::parse("http://proxy.example.com:8080")
...
fn test_intercept_basic_auth() {
    let url = fluent_ai_http3_client::Url::parse("http://user:pass@proxy.example.com:8080")
