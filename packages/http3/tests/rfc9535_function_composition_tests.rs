...
mod function_composition_tests {
    ...
    fn test_function_argument_type_conversion() {
        ...
        let type_conversion_tests = vec![
            ...
            ("$.library.books[?length(@.title)]", true, "ValueType result as LogicalType"),
            ("$.library.books[?count(@.tags)]", true, "Number result as LogicalType"),
            ("$.library.books[?@.metadata.pages]", true, "Property as LogicalType"),
            
            // NodesType to ValueType conversion (single node only)
            ("$.library.books[?value(@.title)]", true, "NodesType to ValueType conversion"),
            ("$.library.books[?length(value(@.title))]", true, "Chained conversion"),
            
            // Invalid type conversions should be caught
            ("$.library.books[?length(count(@.tags))]", false, "Number to ValueType invalid"),
            ("$.library.books[?value(@.tags[*])]", false, "Multi-node to ValueType invalid"),
    ...
    fn test_nested_function_validation() {
        ...
        let nested_tests = vec![
            ...
            ("$.library.books[?length(@.title) > 0]", true, "Single function in comparison"),
            ("$.library.books[?count(@.tags) + length(@.title) > 10]", true, "Functions in arithmetic"),
            ("$.library.books[?length(@.title) == count(@.tags)]", true, "Function to function comparison"),
            
            // Complex valid nesting
            ("$.library.books[?count(@.metadata.authors) > 0 && length(@.title) > count(@.tags)]", true, "Complex logical with functions"),
            
            // Invalid nesting - type mismatches
            ("$.library.books[?length(count(@.tags))]", false, "Number as string argument"),
            ("$.library.books[?count(@.title)]", false, "String as nodelist argument"),
            ("$.library.books[?match(count(@.tags), 'pattern')]", false, "Number as string argument"),
            ("$.library.books[?search(length(@.title), @.title)]", false, "Number as string argument"),
            
            // Invalid function names in composition
            ("$.library.books[?unknown(length(@.title))]", false, "Unknown function in composition"),
    ...
    fn test_function_argument_count_validation() {
        ...
        let argument_count_tests = vec![
            ...
            ("$.library.books[?length(@.title) > 0]", true, "length() with 1 arg"),
            ("$.library.books[?count(@.tags) > 0]", true, "count() with 1 arg"),
            ("$.library.books[?match(@.title, 'pattern')]", true, "match() with 2 args"),
            ("$.library.books[?search(@.title, 'pattern')]", true, "search() with 2 args"),
            ("$.library.books[?value(@.title)]", true, "value() with 1 arg"),
            
            // Incorrect argument counts should be rejected
            ("$.library.books[?length()]", false, "length() with 0 args"),
            ("$.library.books[?length(@.title, @.tags)]", false, "length() with 2 args"),
            ("$.library.books[?count()]", false, "count() with 0 args"),
            ("$.library.books[?count(@.tags, @.items)]", false, "count() with 2 args"),
            ("$.library.books[?match(@.title)]", false, "match() with 1 arg"),
            ("$.library.books[?match(@.title, 'pattern', 'extra')]", false, "match() with 3 args"),
            ("$.library.books[?search(@.title)]", false, "search() with 1 arg"),
            ("$.library.books[?value()]", false, "value() with 0 args"),
            ("$.library.books[?value(@.title, @.tags)]", false, "value() with 2 args"),
            
            // Nested function argument count validation
            ("$.library.books[?length(match(@.title, 'pattern'))]", false, "Nested with wrong types"),
    ...
    fn test_function_execution_with_composition() {
        ...
        let execution_tests = vec![
            ("$.library.books[?length(@.title) > 10]", 2, "Books with long titles"),
            ("$.library.books[?count(@.tags) > 1]", 2, "Books with multiple tags"),
            ("$.library.books[?length(@.title) > count(@.tags)]", 1, "Title longer than tag count"),
            ("$.library.books[?count(@.items) > 0]", 2, "Books with items"),
...
mod function_type_system_tests {
    ...
    fn test_valuetype_to_logicaltype_conversion() {
        ...
        let conversion_tests = vec![
            ...
            ("$.library.books[?@.metadata]", true, "Object as LogicalType"),
            ("$.library.books[?@.tags]", true, "Array as LogicalType"),
            ("$.library.books[?@.nested]", true, "Nested object as LogicalType"),
            ("$.library.books[?length(@.title)]", true, "Function result as LogicalType"),
    ...
    fn test_nodestype_to_valuetype_conversion() {
        ...
        let conversion_tests = vec![
            ...
            ("$.library.books[?value(@.title)]", true, "Single node to ValueType"),
            ("$.library.books[?length(value(@.title))]", true, "Chained conversion"),
            
            // Invalid conversions (multi-node or zero-node)
            ("$.library.books[?value(@.tags[*])]", false, "Multi-node to ValueType"),
            ("$.library.books[?value(@.nonexistent)]", false, "Zero-node to ValueType"),
    ...
    fn test_function_parameter_type_matching() {
        ...
        let type_matching_tests = vec![
            ...
            ("$.library.books[?length(@.title)]", true, "String to length()"),
            ("$.library.books[?length(@.tags)]", true, "Array to length()"),
            ("$.library.books[?length(@.metadata)]", true, "Object to length()"),
            
            // count() expects NodesType  
            ("$.library.books[?count(@.tags[*])]", true, "Nodelist to count()"),
            ("$.library.books[?count(@..title)]", true, "Descendant nodelist to count()"),
            
            // match() expects two ValueType arguments
            ("$.library.books[?match(@.title, 'pattern')]", true, "Two strings to match()"),
            
            // search() expects two ValueType arguments  
            ("$.library.books[?search(@.title, 'pattern')]", true, "Two strings to search()"),
            
            // value() expects NodesType
            ("$.library.books[?value(@.title)]", true, "Nodelist to value()"),
            
            // Type mismatches should be rejected
            ("$.library.books[?count(@.title)]", false, "String to count() - expects NodesType"),
            ("$.library.books[?length(@.tags[*])]", false, "Multi-node to length() - expects single ValueType"),
            ("$.library.books[?match(@.tags, 'pattern')]", false, "Array to match() first arg"),
...
mod complex_composition_tests {
    ...
    fn test_deeply_nested_function_calls() {
        ...
        let deep_nesting_tests = vec![
            ...
            ("$.library.books[?length(@.title) > count(@.tags)]", true, "Two function comparison"),
            
            // Three levels would require careful type management
            // Note: These may not be valid depending on type system
            ("$.library.books[?count(@.tags) > 0 && length(@.title) > 0]", true, "Parallel functions"),
            
            // Complex logical combinations
            ("$.library.books[?length(@.title) > 5 && count(@.tags) > 1 && @.metadata.pages > 100]", true, "Mixed functions and properties"),
            
            // Function results in arithmetic
            ("$.library.books[?length(@.title) * count(@.tags) > 20]", true, "Arithmetic with functions"),
    ...
    fn test_function_composition_error_messages() {
        ...
        let error_cases = vec![
            ("$.library.books[?length(count(@.tags))]", "Number as string argument"),
            ("$.library.books[?count(@.title)]", "String as nodelist argument"),
            ("$.library.books[?unknown(length(@.title))]", "Unknown function"),
            ("$.library.books[?length()]", "Missing required argument"),
            ("$.library.books[?match(@.title)]", "Missing required second argument"),
        ];
    ...
    fn test_function_composition_with_current_node() {
        ...
        let current_node_tests = vec![
            ("$.library.books[?length(@.title) > @.id]", true, "Function result vs @ property"),
            ("$.library.books[?count(@.tags) == @.metadata.pages / 100]", true, "Function vs @ calculation"),
            ("$.library.books[?@.id > 0 && length(@.title) > 5]", true, "@ property and function"),
            ("$.library.books[?length(@.title) > length(@.nested.title)]", true, "Function with nested @"),
