...
mod function_type_system_tests {
    ...
    fn test_valuetype_function_parameters() {
        ...
        let valuetype_tests = vec![
            ...
            ("$.library.books[?length(@.title) > 0]", true, "length() with string ValueType"),
            ("$.library.books[?length(@.authors) > 0]", true, "length() with array ValueType"),
            ("$.library.books[?length(@.metadata) > 0]", true, "length() with object ValueType"),
            
            // value() function - converts NodesType to ValueType  
            ("$.library.books[?length(value(@.title)) > 10]", true, "value() to ValueType conversion"),
            ("$.library.books[?value(@.id) > 0]", true, "value() with numeric result"),
            
            // String functions expecting ValueType
            ("$.library.books[?match(@.title, 'Great')]", true, "match() with string ValueType"),
            ("$.library.books[?search(@.title, 'Kill')]", true, "search() with string ValueType"),
            
            // Invalid ValueType usage
            ("$.library.books[?length(@.authors[*])]", false, "length() with multi-node (NodesType)"),
            ("$.library.books[?length(@..title)]", false, "length() with recursive descent"),
    ...
    fn test_nodestype_function_parameters() {
        ...
        let nodestype_tests = vec![
            ...
            ("$.library.books[?count(@.authors) > 0]", true, "count() with single node"),
            ("$.library.books[?count(@.authors[*]) > 0]", true, "count() with multi-node list"),
            ("$.library.books[?count(@..genres) > 5]", true, "count() with recursive descent"),
            ("$.library.books[?count(@.metadata.ratings[*]) > 3]", true, "count() with array elements"),
            
            // value() function - expects NodesType, converts to ValueType
            ("$.library.books[?value(@.id)]", true, "value() with single node"),
            ("$.library.books[?value(@.availability.inStock)]", true, "value() with boolean node"),
            
            // Invalid NodesType usage
            ("$.library.books[?count(@.title)]", false, "count() with single ValueType property"),
            ("$.library.books[?count('literal string')]", false, "count() with literal string"),
            ("$.library.books[?count(123)]", false, "count() with literal number"),
    ...
    fn test_logicaltype_function_results() {
        ...
        let logicaltype_tests = vec![
            ...
            ("$.library.books[?length(@.title)]", true, "length() result as LogicalType"),
            ("$.library.books[?count(@.authors)]", true, "count() result as LogicalType"),
            ("$.library.books[?match(@.title, 'Great')]", true, "match() result as LogicalType"),
            ("$.library.books[?search(@.title, 'Kill')]", true, "search() result as LogicalType"),
            
            // Function results in logical expressions
            ("$.library.books[?length(@.title) && count(@.authors)]", true, "Functions in AND expression"),
            ("$.library.books[?length(@.title) || count(@.metadata.genres)]", true, "Functions in OR expression"),
            ("$.library.books[?!length(@.availability.locations)]", true, "Function in NOT expression"),
            
            // Function results in comparisons (converted to ValueType)
            ("$.library.books[?length(@.title) > 10]", true, "Function result compared to number"),
            ("$.library.books[?count(@.authors) == 1]", true, "Function count comparison"),
...
mod function_argument_type_tests {
    ...
    fn test_function_argument_count_validation() {
        ...
        let argument_count_tests = vec![
            ...
            ("$.library.books[?length(@.title)]", true, "length() with 1 argument"),
            ("$.library.books[?count(@.authors)]", true, "count() with 1 argument"),
            ("$.library.books[?value(@.id)]", true, "value() with 1 argument"),
            ("$.library.books[?match(@.title, 'pattern')]", true, "match() with 2 arguments"),
            ("$.library.books[?search(@.title, 'pattern')]", true, "search() with 2 arguments"),
            
            // Incorrect argument counts
            ("$.library.books[?length()]", false, "length() with 0 arguments"),
            ("$.library.books[?length(@.title, @.authors)]", false, "length() with 2 arguments"),
            ("$.library.books[?count()]", false, "count() with 0 arguments"),
            ("$.library.books[?count(@.authors, @.metadata)]", false, "count() with 2 arguments"),
            ("$.library.books[?value()]", false, "value() with 0 arguments"),
            ("$.library.books[?value(@.id, @.title)]", false, "value() with 2 arguments"),
            ("$.library.books[?match(@.title)]", false, "match() with 1 argument"),
            ("$.library.books[?match(@.title, 'pattern', 'extra')]", false, "match() with 3 arguments"),
            ("$.library.books[?search(@.title)]", false, "search() with 1 argument"),
    ...
    fn test_function_argument_type_conversion() {
        ...
        let type_conversion_tests = vec![
            ...
            ("$.library.books[?length(@.title)]", true, "String to ValueType"),
            ("$.library.books[?length(@.authors)]", true, "Array to ValueType"),
            ("$.library.books[?length(@.metadata)]", true, "Object to ValueType"),
            ("$.library.books[?count(@.authors[*])]", true, "Multi-node to NodesType"),
            ("$.library.books[?count(@..genres)]", true, "Recursive descent to NodesType"),
            ("$.library.books[?value(@.id)]", true, "Single node to ValueType"),
            
            // String function type conversions
            ("$.library.books[?match(@.title, 'Great')]", true, "String literals in match()"),
            ("$.library.books[?search(@.authors[0], 'Lee')]", true, "Array element to string"),
            
            // Invalid type conversions
            ("$.library.books[?length(123)]", false, "Number literal to ValueType"),
            ("$.library.books[?length(true)]", false, "Boolean literal to ValueType"),
            ("$.library.books[?count(@.title)]", false, "Single property to NodesType"),
            ("$.library.books[?count('literal')]", false, "String literal to NodesType"),
            ("$.library.books[?value(@.authors[*])]", false, "Multi-node to ValueType (ambiguous)"),
            ("$.library.books[?match(@.authors, 'pattern')]", false, "Array to string in match()"),
    ...
    fn test_nested_function_argument_types() {
        ...
        let nested_function_tests = vec![
            ...
            ("$.library.books[?length(@.title) > count(@.authors)]", true, "ValueType vs NodesType result"),
            ("$.library.books[?count(@.authors) == count(@.metadata.genres)]", true, "NodesType vs NodesType result"),
            ("$.library.books[?length(@.title) == length(@.authors[0])]", true, "ValueType vs ValueType result"),
            
            // Valid function composition with type conversion
            ("$.library.books[?length(value(@.title)) > 10]", true, "NodesType->ValueType->ValueType"),
            ("$.library.books[?count(@.authors) > length(@.title) / 10]", true, "Mixed types in arithmetic"),
            
            // Invalid nested function types
            ("$.library.books[?length(count(@.authors))]", false, "Number result to string function"),
            ("$.library.books[?count(length(@.title))]", false, "Number result to nodelist function"),
            ("$.library.books[?match(count(@.authors), 'pattern')]", false, "Number to string in match()"),
            ("$.library.books[?search(length(@.title), 'pattern')]", false, "Number to string in search()"),
...
mod function_result_type_tests {
    ...
    fn test_function_result_type_consistency() {
        ...
        let result_type_tests = vec![
            ...
            ("$.library.books[?length(@.title) + 1 > 10]", true, "length() numeric result"),
            ("$.library.books[?count(@.authors) * 2 > 1]", true, "count() numeric result"),
            ("$.library.books[?length(@.title) - count(@.authors) > 5]", true, "Numeric arithmetic"),
            
            // Functions returning boolean values (LogicalType)
            ("$.library.books[?match(@.title, 'Great') == true]", true, "match() boolean result"),
            ("$.library.books[?search(@.title, 'Kill') != false]", true, "search() boolean result"),
            ("$.library.books[?match(@.title, 'Great') && search(@.title, 'Gatsby')]", true, "Boolean combination"),
            
            // Functions returning values for comparison
            ("$.library.books[?value(@.id) == 1]", true, "value() result comparison"),
            ("$.library.books[?value(@.availability.inStock) == true]", true, "value() boolean extraction"),
            ("$.library.books[?value(@.metadata.year) > 1950]", true, "value() numeric extraction"),
            
            // Mixed function result types
            ("$.library.books[?length(@.title) > 0 && match(@.title, 'Great')]", true, "Numeric and boolean mix"),
    ...
    fn test_function_execution_with_type_validation() {
        ...
        let execution_tests = vec![
            ("$.library.books[?length(@.title) > 10]", 2, "String length filtering"),
            ("$.library.books[?count(@.authors) == 1]", 3, "Author count filtering"),
            ("$.library.books[?length(@.metadata.genres) > 2]", 3, "Array length filtering"),
            ("$.library.books[?count(@.metadata.ratings[*]) > 4]", 1, "Rating count filtering"),
            ("$.library.books[?value(@.id) > 1]", 2, "ID value filtering"),
            ("$.library.books[?value(@.availability.inStock) == true]", 2, "Boolean value filtering"),
            ("$.library.books[?match(@.title, 'Great')]", 1, "Title pattern matching"),
    ...
    fn test_type_coercion_edge_cases() {
        ...
        let coercion_tests = vec![
            ...
            ("$.library.books[?value(@.missing_property)]", true, "value() with missing property"),
            ("$.library.books[?length(@.missing_property)]", true, "length() with missing property"),
            ("$.library.books[?count(@.missing_property)]", true, "count() with missing property"),
            
            // Edge cases with empty collections
            ("$.library.books[?length(@.availability.locations)]", true, "length() with empty array"),
            ("$.library.books[?count(@.availability.locations[*])]", true, "count() with empty array elements"),
            
            // Edge cases with numeric vs string types
            ("$.library.books[?value(@.id) == '1']", true, "Numeric ID compared to string"),
            ("$.library.books[?length(@.id)]", false, "length() with numeric value"),
            ("$.library.books[?match(@.id, '1')]", false, "match() with numeric value"),
            
            // Edge cases with boolean types
            ("$.library.books[?value(@.availability.inStock)]", true, "Boolean value extraction"),
            ("$.library.books[?length(@.availability.inStock)]", false, "length() with boolean"),
...
mod complex_function_type_tests {
    ...
    fn test_complex_type_scenarios() {
        ...
        let complex_scenarios = vec![
            ...
            ("$.library.books[?length(@.title) > count(@.metadata.genres) + count(@.authors)]", true, "Complex arithmetic with functions"),
            ("$.library.books[?match(@.title, 'Great') && length(@.authors[0]) > 5]", true, "Boolean and length combination"),
            ("$.library.books[?count(@.metadata.ratings[*]) > length(@.title) / 5]", true, "Count vs length ratio"),
            
            // Nested property access with functions
            ("$.library.books[?value(@.metadata.year) > 1900 && length(@.metadata.genres) > 2]", true, "Nested value and length"),
            ("$.library.books[?count(@.availability.locations[*]) == value(@.availability.quantity)]", true, "Count vs value comparison"),
            
            // Complex logical expressions with type mixing
            ("$.library.books[?length(@.title) > 10 || (count(@.authors) == 1 && match(@.title, 'Kill'))]", true, "Complex logical with mixed types"),
    ...
    fn test_function_type_error_messages() {
        ...
        let error_message_tests = vec![
            ("$.library.books[?length(count(@.authors))]", "Number argument to string function"),
            ("$.library.books[?count(@.title)]", "String argument to nodelist function"),
            ("$.library.books[?match(@.authors, 'pattern')]", "Array argument to string function"),
            ("$.library.books[?value(@.authors[*])]", "Multi-node argument to single-value function"),
            ("$.library.books[?length()]", "Missing required argument"),
            ("$.library.books[?count(@.title, @.authors)]", "Too many arguments"),
        ];
        ...
        for (expr, expected_error_type) in error_message_tests {
            ...
            if let Err(JsonPathError::InvalidExpression { reason, .. }) = result {
                ...
                println!("Function type error for '{}' ({}): {}", 
