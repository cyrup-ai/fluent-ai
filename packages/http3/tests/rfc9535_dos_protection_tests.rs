...
mod dos_protection_tests {
    ...
    fn test_recursive_descent_depth_limits() {
        ...
        for (depth, should_succeed, description) in depth_tests {
            ...
            if should_succeed {
                ...
                assert!(
                    ...
                    depth, description, elapsed.as_millis()
                );
                ...
                if elapsed > Duration::from_secs(5) {
                    println!("DoS protection: Depth {} timed out as expected ({})", depth, description);
                } else if results.is_empty() {
                    println!("DoS protection: Depth {} returned no results ({})", depth, description);
                } else {
                    // Acceptable if it completes but with protection warnings
                    println!("DoS protection: Depth {} completed with {} results in {}ms ({})", 
                        depth, results.len(), elapsed.as_millis(), description);
                }
            }
        }
    }
    ...
    fn test_recursive_descent_breadth_limits() {
        ...
        for (width, items_per_level, should_succeed, description) in breadth_tests {
            ...
            if should_succeed {
                ...
                assert_eq!(
                    ...
                    expected_matches, description
                );
            } else {
                // Should have DoS protection
                if elapsed > Duration::from_secs(10) {
                    println!("DoS protection: Breadth test timed out as expected ({})", description);
                } else {
                    println!("DoS protection: Breadth test completed in {}ms with {} results ({})",
                        elapsed.as_millis(), results.len(), description);
                }
            }
        }
    }
    ...
    fn test_pathological_recursive_patterns() {
        ...
        for (expr, description) in pathological_tests {
            ...
            match compile_result {
                ...
                Err(_) => {
                    ...
                    println!("DoS protection: Parser rejected pathological pattern '{}' ({})", expr, description);
                }
            }
        }
    }
    ...
    fn test_memory_exhaustion_protection() {
        ...
        for (expr, data_size, description) in memory_tests {
            ...
            println!("Memory test '{}' with {} data elements: {} results in {}ms ({})",
                expr, data_size, results.len(), elapsed.as_millis(), description);
...
mod recursive_descent_performance_tests {
    ...
    fn test_linear_vs_exponential_growth() {
        ...
        for size in size_tests {
            ...
            println!("Performance test size {}: {} results in {}ms (expected: {})",
                size, results.len(), elapsed.as_millis(), expected_results);
            
            // Performance should scale reasonably
            let ms_per_result = elapsed.as_millis() as f64 / results.len() as f64;
            assert!(
                ms_per_result < 10.0,
    ...
    fn test_recursive_descent_termination() {
        ...
        for (expr, description) in termination_tests {
            ...
            assert!(
                ...
                expr, description, elapsed.as_millis()
            );
...
mod dos_attack_simulation_tests {
    ...
    fn test_fork_bomb_pattern_protection() {
        ...
        for (expr, description) in fork_bomb_patterns {
            ...
            match JsonPathParser::compile(expr) {
                ...
                Err(_) => {
                    println!("Fork bomb protection: Parser rejected '{}' ({})", expr, description);
                }
            }
        }
    }
    ...
    fn test_resource_consumption_limits() {
        ...
        for (expr, data_multiplier, description) in resource_tests {
            // Create large dataset
            ...
            println!("Resource test '{}' with {} scale: {} results in {}ms ({})",
                expr, data_multiplier, results.len(), elapsed.as_millis(), description);
