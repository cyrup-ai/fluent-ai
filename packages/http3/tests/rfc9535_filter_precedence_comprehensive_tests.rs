...
mod logical_truth_table_tests {
    ...
    fn test_and_operator_truth_table() {
        ...
        let and_tests = vec![
            ("$.items[?@.flag_a && @.flag_b]", vec![3, 5], "true && true cases"),
            ("$.items[?@.flag_a && !@.flag_b]", vec![1], "true && false cases"),  
            ("$.items[?!@.flag_a && @.flag_b]", vec![2], "false && true cases"),
    ...
    fn test_or_operator_truth_table() {
        ...
        let or_tests = vec![
            ("$.items[?@.flag_a || @.flag_b]", vec![1, 2, 3, 5], "Any true cases"),
            ("$.items[?!@.flag_a || !@.flag_b]", vec![1, 2, 4], "Any false cases"),
    ...
    fn test_not_operator_truth_table() {
        ...
        let not_tests = vec![
            ...
            ("$.items[?!!@.active]", vec![1, 3, 5], "Double NOT (identity) cases"),
...
mod complex_logical_tests {
    ...
    fn test_short_circuit_behavior() {
        ...
        let short_circuit_tests = vec![
            ...
            ("$.items[?@.active || @.nonexistent.property]", "OR short-circuit with error"),
            
            // AND short-circuit: if first is false, second shouldn't be evaluated  
            ("$.items[?!@.active && @.nonexistent.property]", "AND short-circuit with error"),
    ...
    fn test_logical_operator_syntax_validation() {
        ...
        let syntax_tests = vec![
            ...
            ("$.items[?@.active && @.flag_a || @.flag_b]", true, "Mixed operators"),
            
            // Invalid operator syntax
            ("$.items[?@.active & @.flag_a]", false, "Single & invalid"),
            ("$.items[?@.active | @.flag_a]", false, "Single | invalid"),
            ("$.items[?@.active and @.flag_a]", false, "Word 'and' invalid"),
...
mod logical_error_tests {
    ...
    fn test_parentheses_error_handling() {
        ...
        let paren_errors = vec![
            ...
            ("$.items[?@.active && @.flag_a)]", "Unmatched closing parenthesis"),
            ("$.items[?(@.active && (@.flag_a]", "Nested unclosed parenthesis"),
            ("$.items[?@.active && @.flag_a))]", "Extra closing parenthesis"),
        ...
        for (expr, error_type) in paren_errors {
            ...
            match result {
                ...
                Err(_) => {
                    println!("Parentheses error correctly caught: {} ({})", expr, error_type);
                }
            }
        }
    }
