...
use bytes::Bytes;
use fluent_ai_http3::json_path::{CoreJsonPathEvaluator, JsonArrayStream, JsonPathParser};
...
fn test_jsonpath_filter_on_string_performance() {
    ...
    assert!(
        eval_exec_time.as_millis() < 100,
        "Core evaluation took too long: {:?}",
        eval_exec_time
    );
    assert!(
        stream_process_time.as_millis() < 1000,
        "Stream processing took too long: {:?}",
        stream_process_time
    );
}
...
fn test_simplified_jsonpath_patterns() {
    ...
    for pattern in test_patterns {
        ...
        match CoreJsonPathEvaluator::new(pattern) {
            Ok(evaluator) => match evaluator.evaluate(&json_value) {
                Ok(results) => {
