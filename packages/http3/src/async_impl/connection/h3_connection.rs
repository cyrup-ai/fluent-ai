use crate::types::h3_chunks::{H3ConnectionChunk, H3BiStreamChunk, H3DataChunk, H3SendResult};
    ...
    T: h3::quic::Connection<BufRecvStream>,
{
    ...
    pub fn accept_recv_stream(mut self, mut context: Context<'_>) -> AsyncStream<H3ConnectionChunk, 1024> {
    ...
    pub fn accept_bidi_stream(mut self, mut context: Context<'_>) -> AsyncStream<H3BiStreamChunk, 1024> {
...
impl H3RecvStreamHandler {
    ...
    pub fn poll_data_stream<T>(&self, mut buf_recv_stream: BufRecvStream<T>, mut context: Context<'_>) -> AsyncStream<H3DataChunk, 1024>
    where
...
impl H3SendStreamHandler {
    ...
    pub fn send_data_stream<T>(&self, mut send_stream: SendStream<T>, data_chunks: Vec<bytes::Bytes>, mut context: Context<'_>) -> AsyncStream<H3SendResult, 1024>
    where
    ...
    pub fn poll_ready_stream<T>(&self, mut send_stream: SendStream<T>, mut context: Context<'_>) -> AsyncStream<H3SendResult, 1024>
    where
        T: h3::quic::SendStream,
    {
        ...
        AsyncStream::<H3SendResult, 1024>::with_channel(move |sender| {
            match send_stream.poll_ready(&mut context) {
                Poll::Ready(Ok(())) => { emit!(sender, H3SendResult::send_ready(stream_id)); }
                Poll::Ready(Err(e)) => { emit!(sender, H3SendResult::bad_chunk(format!("Poll ready error: {}", e))); }
                Poll::Pending => {} // AsyncStream elite polling loop handles this
            }
        })
    }
    ...
    T: h3::quic::Connection<BufRecvStream>,
{
    ...
    pub fn get_connection(&self) -> Option<Arc<H3Connection<T>>> {
        ...
        let index = self.next_connection.fetch_add(1, Ordering::Relaxed) as usize % self.connections.len();
