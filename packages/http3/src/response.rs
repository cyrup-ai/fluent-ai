...
impl HttpResponse {
    ...
    pub fn from_cache(status: StatusCode, headers: HashMap<String, String>, body: impl Into<Vec<u8>>) -> Self {
        Self {
            status,
            headers,
            body: body.into(),
    ...
    pub fn json<T: serde::de::DeserializeOwned>(&self) -> crate::HttpResult<T> {
        serde_json::from_slice(&self.body).map_err(|e| {
            crate::HttpError::DeserializationError {
    ...
    pub fn error_for_status(self) -> crate::HttpResult<Self> {
        if self.is_success() {
            ...
            Err(crate::HttpError::HttpStatus {
                ...
                message: format!("HTTP {} {}", self.status.as_u16(), 
