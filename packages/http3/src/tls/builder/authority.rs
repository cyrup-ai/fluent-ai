...
impl AuthorityRemoteBuilder {
    ...
    pub async fn load(self) -> super::responses::CertificateAuthorityResponse {
        ...
        // Download certificate from remote URL with timeout
        let mut http_stream = crate::builder::Http3Builder::new().get(&self.url);
        
        let cert_content = match tokio::time::timeout(
            self.timeout,
            async {
                let mut chunks = Vec::new();
                let mut http_stream_pin = std::pin::Pin::new(&mut http_stream);
                while let Some(chunk) = fluent_ai_async::AsyncStream::next(&mut http_stream_pin).await {
                    chunks.push(chunk);
                }
                
                // Convert chunks to HttpResponse
                if chunks.is_empty() {
                    return Err(crate::error::network_error("No response received"));
                }
                
                // For simplicity, reconstruct response from chunks
                // This is a basic implementation - production code might need more sophisticated parsing
                let body_bytes: Vec<u8> = chunks.into_iter()
                    .filter_map(|chunk| {
                        match chunk {
                            crate::prelude::HttpChunk::Body(bytes) => Some(bytes.to_vec()),
                            crate::prelude::HttpChunk::Data(bytes) => Some(bytes.to_vec()),
                            crate::prelude::HttpChunk::Chunk(bytes) => Some(bytes.to_vec()),
                            _ => None,
                        }
                    })
                    .flatten()
