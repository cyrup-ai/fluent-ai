...
impl Form {
    ...
    ///
    /// ```no_run
    /// let mut file_stream = crate::hyper::multipart::Form::new()
    ///     .file("key", "/path/to/file");
    /// if let Some(Ok(form)) = file_stream.try_next() {
    ///     println!("Form created with file");
    /// }
    /// ```
    ///
    /// # Errors
    ...
    pub fn file<T, U>(self, name: T, path: U) -> AsyncStream<io::Result<Form>>
    where
        T: Into<Cow<'static, str>>,
        U: AsRef<Path>,
    {
        ...
        AsyncStream::with_channel(move |sender| {
            let task = spawn_task(move || -> Result<Form, std::io::Error> {
                // Create Part from file using streams-first pattern
                let mut part_stream = Part::file(path);
                match part_stream.try_next() {
                    Some(Ok(part)) => Ok(self.part(name, part)),
                    Some(Err(e)) => Err(e),
                    None => Err(std::io::Error::new(std::io::ErrorKind::UnexpectedEof, "Part::file stream ended without producing a part")),
                }
            });
            
            match task.collect() {
                Ok(form) => emit!(sender, Ok(form)),
        ...
        AsyncStream::with_channel(move |sender| {
            let task = spawn_task(move || -> Result<(), crate::Error> {
                ...
                for (name, part) in fields {
                    // Generate part stream and process all its chunks
                    let mut part_stream = self.part_stream(name, part);
            ...
            match task.collect() {
                Ok(_) => {}, // All items already emitted
                Err(e) => handle_error!(e, "multipart form streaming"),
            }
        })
    }
        ...
        T: Into<Cow<'static, str>>,
    {
        ...
        let percent_encoding = self.inner.percent_encoding.clone();
        ...
        AsyncStream::with_channel(move |sender| {
            ...
            match task.collect() {
                Ok(_) => {}, // All items already emitted
                Err(e) => handle_error!(e, "multipart part streaming"),
            }
        })
    }
...
impl Part {
    ...
    #[cfg(feature = "stream")]
    #[cfg_attr(docsrs, doc(cfg(feature = "stream")))]
    pub fn file<T: AsRef<Path>>(path: T) -> fluent_ai_async::AsyncStream<io::Result<Part>> {
        use fluent_ai_async::{AsyncStream, emit, handle_error, spawn_task};
        
        AsyncStream::with_channel(move |sender| {
            let task = spawn_task(move || -> io::Result<Part> {
                ...
                // Use standard synchronous file operations for streams-first architecture
...
mod tests {
    use super::*;
    use fluent_ai_async::{spawn_task, AsyncStream, emit};
    ...
    fn stream_to_end() {
        ...
        let stream1 = AsyncStream::with_channel(move |sender| {
            let task = spawn_task(move || -> Result<(), crate::Error> {
                emit!(sender, Ok("part1".to_owned()));
                Ok(())
            });
            let _ = task.collect();
        });
        
        let stream2 = AsyncStream::with_channel(move |sender| {
            let task = spawn_task(move || -> Result<(), crate::Error> {
                emit!(sender, Ok("part2".to_owned()));
                Ok(())
            });
            let _ = task.collect();
        });
        ...
        let mut form = Form::new()
            .part("reader1", Part::stream(Body::stream(stream1)))
            .part("key1", Part::text("value1"))
            .part("key2", Part::text("value2").mime(mime::IMAGE_BMP))
            .part("reader2", Part::stream(Body::stream(stream2)))
