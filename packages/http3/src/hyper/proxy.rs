...
//
// The user creates a builder (`crate::hyper::Proxy`), and configures any extras.
...
impl Proxy {
    ...
    ///     .proxy(crate::hyper::Proxy::http("https://my.prox")?)
    ///     .build()?;
    /// # Ok(())
    /// # }
    /// # fn main() {}
    ...
    /// headers.insert(USER_AGENT, "http3".parse().expect("valid header value"));
    /// let proxy = crate::hyper::Proxy::https("http://localhost:1234")?
    ///     .headers(headers);
    /// # Ok(())
    /// # }
    ...
    /// # fn run() -> Result<(), Box<dyn std::error::Error>> {
    /// let proxy = crate::hyper::Proxy::https("http://localhost:1234")?
    ///     .no_proxy(crate::hyper::NoProxy::from_string("direct.tld, sub.direct2.tld"));
    /// # Ok(())
    /// # }
    /// # fn main() {}
    /// ```
...
mod tests {
    ...
    fn intercepted_uri(p: &Matcher, s: &str) -> Uri {
        p.intercept(&s.parse().expect("test URI should parse")).expect("should intercept").uri().clone()
    }

    #[test]
    fn test_http() {
        let target = "http://example.domain/";
        let p = Proxy::http(target).expect("test proxy should create").into_matcher();
    ...
    fn test_https() {
        let target = "http://example.domain/";
        let p = Proxy::https(target).expect("test proxy should create").into_matcher();
    ...
    fn test_all() {
        let target = "http://example.domain/";
        let p = Proxy::all(target).expect("test proxy should create").into_matcher();
    ...
    fn test_standard_with_custom_auth_header() {
        ...
        let got = p.intercept(&url("http://anywhere.local")).expect("should intercept");
    ...
    fn test_socks_proxy_default_port() {
        {
            let m = Proxy::all("socks5://example.com").expect("test proxy should create").into_matcher();
