...
fn fetch(req: Request) -> AsyncStream<Result<Response, crate::Error>> {
    ...
    let http_response = resp.body(js_resp)
        .map_err(crate::error::builder)?;
...
mod tests {
    ...
    fn default_headers_clone() {
        ...
        assert_eq!(
            headers1.get(CONTENT_TYPE).expect("content-type header should exist"),
            "text/plain",
        ...
        assert_eq!(
            headers2.get(CONTENT_TYPE).expect("content-type header should exist"),
            "application/json",
    ...
    fn user_agent_header() {
        ...
        assert_eq!(
            headers1.get(USER_AGENT).expect("user-agent header should exist"),
            "FooBar/1.2.3",
