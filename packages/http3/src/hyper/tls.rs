...
impl Identity {
    ...
    ///     .read_to_end(&mut buf)?;
    /// let pkcs12 = crate::hyper::Identity::from_pkcs12_der(&buf, "my-privkey-password")?;
    /// # drop(pkcs12);
    /// # Ok(())
    /// # }
    /// ```
    ...
    /// let pkcs8 = crate::hyper::Identity::from_pkcs8_pem(&cert, &key)?;
    /// # drop(pkcs8);
    /// # Ok(())
    /// # }
    /// ```
    ...
    ///     .read_to_end(&mut buf)?;
    /// let id = crate::hyper::Identity::from_pem(&buf)?;
    /// # drop(id);
    /// # Ok(())
    /// # }
    /// ```
