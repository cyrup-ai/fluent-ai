...
//! # http3
//!
//! The `http3` crate provides a convenient, higher-level HTTP
//! [`Client`][client].
//!
...
//! - Cookies
//!
//! The [`crate::hyper::Client`][client] is asynchronous (requiring Tokio). For
//! applications wishing  to only make a few HTTP requests, the
//! [`crate::hyper::blocking`](blocking) API may be more convenient.
//!
//! Additional learning resources include:
//!
//! - [The Rust Cookbook](https://rust-lang-nursery.github.io/rust-cookbook/web/clients.html)
//! - [http3 Repository Examples](https://github.com/seanmonstar/http3/tree/master/examples)
//!
//! ## Commercial Support
//!
...
//!
//! ```rust
//! # fn run() -> Result<(), crate::hyper::Error> {
//! use crate::hyper::HttpStream;
//! let mut response_stream = crate::hyper::get("https://www.rust-lang.org");
//! let response = response_stream.try_next()?;
//! let mut text_stream = response.text();
//! let body = text_stream.try_next()?;
//!
//! println!("body = {body:?}");
//! # Ok(())
//! # }
...
//! type, you can use the `crate::hyper::Body` constructors.
//!
//! ```rust
...
//! let mut response_stream = client.post("http://httpbin.org/post")
...
//! i.e. `let proxy = crate::hyper::Proxy::http("https://secure.example")?;`
...
//!
//! TLS and cookies are provided through the browser environment, so http3 can issue TLS requests with cookies,
...
//!
//! ```notrust
//! RUSTFLAGS="--cfg http3_unstable" cargo build
//! ```
//!
//! ## Sponsors
//!
...
#[cfg(all(feature = "http3", not(feature = "http3_unstable")))]
compile_error!(
    "\
    The `http3` feature is unstable, and requires the \
    `RUSTFLAGS='--cfg http3_unstable'` environment variable to be set.\
"
);
...
// universal mods
#[macro_use]
/// Error types and utilities for HTTP operations
pub mod error;

/// AsyncStream-native service and layer traits for 100% tower feature retention
...
///
/// See also the methods on the [`crate::hyper::Response`](./struct.Response.html)
/// type.
///
