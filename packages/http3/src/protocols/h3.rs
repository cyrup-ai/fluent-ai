//! Direct H3 protocol implementation using fluent_ai_async AsyncStream
//! 
    ...
    AsyncStream::with_channel(move |sender| {
        std::thread::spawn(move || {
            ...
            let mut config = match quiche::Config::new(quiche::PROTOCOL_VERSION) {
                Ok(config) => config,
                Err(e) => {
                    emit!(sender, H3Chunk::bad_chunk(format!("Config creation failed: {}", e)));
                    return;
                }
            };
            
            if let Err(e) = config.set_application_protos(&[b"h3"]) {
                emit!(sender, H3Chunk::bad_chunk(format!("Failed to set H3 protocol: {}", e)));
                return;
            }
            ...
            match quiche::connect(Some(host), &scid, addr, addr, &mut config) {
                Ok(mut connection) => {
                    ...
                    let stream_id = match connection.stream_writable_next() {
                        Some(id) => id,
                        None => {
                            emit!(sender, H3Chunk::bad_chunk("No writable stream available".to_string()));
                            return;
                        }
                    };
                    ...
                    if let Err(e) = connection.stream_send(stream_id, request_data.as_bytes(), body.is_empty()) {
                        emit!(sender, H3Chunk::bad_chunk(format!("Failed to send headers: {}", e)));
                        return;
                    }
                    ...
                    if !body.is_empty() {
                        if let Err(e) = connection.stream_send(stream_id, &body, true) {
                            emit!(sender, H3Chunk::bad_chunk(format!("Failed to send body: {}", e)));
                            return;
                        }
                    }
                    ...
                    loop {
                        for readable_stream_id in connection.readable() {
                            ...
                            match connection.stream_recv(readable_stream_id, &mut buf) {
                                ...
                                Err(e) => {
                                    emit!(sender, H3Chunk::bad_chunk(format!("Stream read error: {}", e)));
                                    return;
                                }
                            }
                        }
                ...
                Err(e) => {
                    emit!(sender, H3Chunk::bad_chunk(format!("H3 connection failed: {}", e)));
                }
            }
        });
    })
