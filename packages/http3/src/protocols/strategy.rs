...
use http::{Uri, HeaderMap, StatusCode};
...
impl HttpProtocolStrategy {
    ...
    pub fn build(&self) -> Box<dyn crate::protocols::strategy_trait::ProtocolStrategy> {
        ...
        match self {
            Self::Http2(config) => Box::new(H2Strategy::new(crate::protocols::h2::strategy::H2Config {
                enable_push: config.enable_push,
                max_concurrent_streams: config.max_concurrent_streams,
                initial_window_size: config.initial_window_size,
                max_frame_size: config.max_frame_size,
                max_header_list_size: 16384, // Default value since it's not in the source config
            })),
