...
impl QuicheConnectionEvent {
    ...
    pub fn established(local: std::net::SocketAddr, peer: std::net::SocketAddr) -> Self {
        let connection_id = QUICHE_COUNTER.fetch_add(1, Ordering::Relaxed);
...
impl QuicheStreamChunk {
    pub fn stream_opened(stream_id: u64, bidirectional: bool) -> Self {
...
impl QuicheWriteResult {
    ...
    pub fn bytes_written(stream_id: u64, bytes_written: usize, fin: bool) -> Self {
        let packet_id = stream_id; // Use stream_id as packet_id for traceability
        Self::Sent {
            packet_id,
            size: bytes_written,
