//! Direct Quiche QUIC protocol implementation using fluent_ai_async AsyncStream
//! 
    ...
    AsyncStream::with_channel(move |sender| {
        std::thread::spawn(move || {
            ...
            let mut config = match quiche::Config::new(quiche::PROTOCOL_VERSION) {
                Ok(config) => config,
                Err(e) => {
                    emit!(sender, QuicheChunk::bad_chunk(format!("Config creation failed: {}", e)));
                    return;
                }
            };
            ...
            if let Err(e) = config.set_application_protos(&[b"h3"]) {
                emit!(sender, QuicheChunk::bad_chunk(format!("Failed to set protocols: {}", e)));
                return;
            }
            ...
            match quiche::connect(Some(&server_name), &scid, addr, addr, &mut config) {
                    ...
                    loop {
                        ...
                        for stream_id in connection.readable() {
                            ...
                            match connection.stream_recv(stream_id, &mut buf) {
                                ...
                                Err(e) => {
                                    emit!(sender, QuicheChunk::bad_chunk(format!("Stream read error: {}", e)));
                                    break;
                                }
                            }
                        }
                ...
                Err(e) => {
                    emit!(sender, QuicheChunk::bad_chunk(format!("Connection failed: {}", e)));
                }
            }
        });
    })
    ...
    AsyncStream::with_channel(move |sender| {
        std::thread::spawn(move || {
            match connection.stream_send(stream_id, &data, true) {
                ...
                Err(e) => {
                    emit!(sender, QuicheChunk::bad_chunk(format!("Send failed: {}", e)));
                }
            }
        });
    })
