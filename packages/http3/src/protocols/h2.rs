    ...
    AsyncStream::with_channel(move |sender| {
        std::thread::spawn(move || {
            ...
            let tcp = match TcpStream::connect(format!("{}:{}", host, port)) {
                Ok(tcp) => tcp,
                Err(e) => {
                    emit!(sender, H2Chunk::bad_chunk(format!("TCP connection failed: {}", e)));
                    return;
                }
            };
            ...
            match h2::client::handshake(tcp) {
                                ...
                                match h2.send_request(req, false) {
                                    Ok((response_future, mut stream)) => {
                                        ...
                                        match response_future {
                                            ...
                                            Err(e) => {
                                                emit!(sender, H2Chunk::bad_chunk(format!("H2 response error: {}", e)));
                                                break;
                                            }
                                        }
                                    }
                                    Err(e) => {
                                        emit!(sender, H2Chunk::bad_chunk(format!("H2 send error: {}", e)));
                                        break;
                                    }
                                }
                            }
                ...
                Err(e) => {
                    emit!(sender, H2Chunk::bad_chunk(format!("H2 handshake failed: {}", e)));
                }
            }
        });
    })
