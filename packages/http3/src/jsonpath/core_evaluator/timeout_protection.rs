...
mod tests {
    ...
    fn test_recommended_timeout() {
        assert_eq!(
            TimeoutProtectedEvaluator::recommended_timeout_ms("$..*"),
            5000
        );
        assert_eq!(
            TimeoutProtectedEvaluator::recommended_timeout_ms("$..book"),
            2000
        );
        assert_eq!(
            TimeoutProtectedEvaluator::recommended_timeout_ms("$[?@.price]"),
            1500
        );
        assert_eq!(
            TimeoutProtectedEvaluator::recommended_timeout_ms("$.store"),
            500
        );
    }
}
