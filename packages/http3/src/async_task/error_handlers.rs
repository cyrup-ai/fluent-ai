...
/// Zero-allocation error handler for async tasks
/// Performance: Inlined, no heap allocation, static dispatch
#[inline(always)]
pub fn default_task_error_handler<T: BadTraitImpl>(error: &str) -> T {
    eprintln!("Task error (BadTraitImpl): {}", error);
    T::bad_impl(error)
}
...
impl<T: BadTraitImpl> ErrorHandler<T> for DefaultErrorHandler<T> {
    #[inline(always)]
    fn handle_error(&self, error: &str) -> T {
        default_task_error_handler(error)
    }
}
...
where
    F: Fn(&str) -> T + Send + Sync,
{
    /// Create a new closure-based error handler - zero allocation
    #[inline(always)]
...
where
    F: Fn(T) -> T + Send + Sync,
{
    /// Create a new closure-based result handler - zero allocation
    #[inline(always)]
    pub const fn new(closure: F) -> Self {
        Self {
            closure,
...
where
    F: Fn(T) -> T + Send + Sync,
{
    /// Create a new closure-based chunk handler - zero allocation
    #[inline(always)]
    pub const fn new(closure: F) -> Self {
        Self {
            closure,
