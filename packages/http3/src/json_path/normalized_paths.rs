...
mod tests {
    ...
    fn test_member_path() {
        let root = NormalizedPath::root();
        let member_path = root.child_member("store").unwrap();
        assert_eq!(member_path.as_str(), "$['store']");
        assert!(!member_path.is_root());
        assert_eq!(member_path.depth(), 1);
        assert_eq!(member_path.parent().unwrap().as_str(), "$");
    }

    #[test]
    fn test_index_path() {
        let root = NormalizedPath::root();
        let member_path = root.child_member("items").unwrap();
        let index_path = member_path.child_index(0).unwrap();
    ...
    fn test_complex_path() {
        let root = NormalizedPath::root();
        let complex = root
            .child_member("store").unwrap()
            .child_member("book").unwrap()
            .child_index(0).unwrap()
            .child_member("title").unwrap();
    ...
    fn test_parse_normalized_path() {
        let parsed = NormalizedPathProcessor::parse_normalized_path("$['store']['book'][0]['title']").unwrap();
    ...
    fn test_path_relationships() {
        let parent = NormalizedPath::root().child_member("store").unwrap();
        let child = parent.child_member("book").unwrap();
