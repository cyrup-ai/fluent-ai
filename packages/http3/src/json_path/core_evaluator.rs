...
impl CoreJsonPathEvaluator {
            ...
            JsonSelector::Filter { expression } => {
                ...
                match value {
                    Value::Array(arr) => {
                        ...
                        println!(
                            "DEBUG: Array filter - collected {} existing properties: {:?}",
                    ...
                    Value::Object(_) => {
                        ...
                        let filter_result = FilterEvaluator::evaluate_predicate_with_context(
                            value,
                            expression,
                            &empty_context,
                        )?;
                        println!(
                            "DEBUG: Filter evaluation for object {:?} = {}",
    ...
    fn apply_descendant_segment_recursive(
        ...
        println!(
            "DEBUG: apply_descendant_segment_recursive called with value: {:?}",
        ...
        for (i, descendant) in all_descendants.iter().enumerate() {
            ...
            if !temp_results.is_empty() {
                println!(
                    "DEBUG: descendant {} matched, adding {} results",
                    i,
                    temp_results.len()
                );
                ...
                let has_filter = child_selectors
                    .iter()
                    .any(|s| matches!(s, JsonSelector::Filter { .. }));
                if has_filter {
    ...
    fn apply_wildcard_to_node_with_limits(
        &self,
        value: &Value,
        results: &mut Vec<Value>,
        max_results: usize,
    ...
    fn apply_slice_selector_with_params<'a>(
        &self,
        node: &'a Value,
        start: Option<i64>,
        end: Option<i64>,
    ...
    fn evaluate_property_with_array_wildcards(
        &self,
        json: &Value,
        expr: &str,
    ) -> JsonPathResult<Vec<Value>> {
