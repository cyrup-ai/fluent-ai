...
pub fn debug_infinite_loop_patterns() {
    ...
    let failing_patterns = vec!["$..book[2]", "$.store.bicycle"];
            ...
            Ok(parsed_expr) => {
                ...
                match CoreJsonPathEvaluator::new(pattern) {
                    Ok(evaluator) => {
                        ...
                        match std::panic::catch_unwind(|| evaluator.evaluate(&bookstore_json)) {
                            Ok(result) => {
    ...
    let working_patterns = vec!["$", "$.store", "$.store.book", "$.store.book[0]"];

    for pattern in working_patterns {
        println!("\n--- Testing working pattern: {} ---", pattern);

        match CoreJsonPathEvaluator::new(pattern) {
            Ok(evaluator) => match evaluator.evaluate(&bookstore_json) {
                Ok(values) => {
