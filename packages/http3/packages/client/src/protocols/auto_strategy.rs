...
impl AutoStrategy {
    ...
    fn build_strategies(
        ...
        let primary = match iter.next() {
            ...
            None => {
                // This should never happen due to the logic above, but we handle it gracefully
                // by providing a safe default H3 strategy
                tracing::error!(
                    target: "fluent_ai_http3::protocols", 
                    "No strategies available, falling back to default H3 strategy"
                );
                Box::new(H3Strategy::new(configs.h3.clone()))
            }
        };
