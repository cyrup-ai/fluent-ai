...
use crate::providers::embedding::EmbeddingBatch;
    ...
    fn process_embeddings(&self, mut embeddings: Vec<Vec<f32>>, normalize: bool) -> Vec<Vec<f32>> {
        if normalize {
            for embedding in &mut embeddings {
                crate::providers::embedding::normalize_vector(embedding);
            }
        }
        embeddings
    }
            ...
            match provider.make_embedding_request(request).await {
                Ok(response) => {
                    if let Some(data) = response.data.first() {
                        ...
                        if config.normalize {
                            crate::providers::embedding::normalize_vector(&mut embedding);
                        }
                            ...
                            .map(|mut embedding| {
                                match &mut embedding {
                                    ZeroOneOrMany::Many(vec) => {
                                        crate::providers::embedding::normalize_vector(vec);
                                    }
                                    _ => {}
                                }
                                embedding
