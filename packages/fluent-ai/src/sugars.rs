...
/// BadTraitImpl for String - returns error message as the bad implementation
impl BadTraitImpl for String {
    fn bad_impl(error: &str) -> Self {
        eprintln!("String BadTraitImpl: {}", error);
        format!("Error: {}", error)
    }
}
...
impl<S, F, U> futures::Stream for StreamMap<S, F>
where
    ...
    F: FnMut(S::Item) -> U + Unpin,
{
    ...
    fn poll_next(
        ...
        match futures::StreamExt::poll_next_unpin(&mut self.stream, cx) {
            std::task::Poll::Ready(Some(item)) => {
                let mapped = (self.f)(item);
                std::task::Poll::Ready(Some(mapped))
            }
...
impl<S, F> futures::Stream for StreamFilter<S, F>
where
    S: futures::Stream + Unpin,
