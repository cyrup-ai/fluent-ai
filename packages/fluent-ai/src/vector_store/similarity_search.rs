...
impl SimilaritySearchEngine {
    ...
    pub fn search_enhanced(
        ...
        crate::async_task::spawn_async(async move {
            ...
            if config.enable_reranking {
                results = engine
                    .apply_reranking(&query_vector, results, &config)
                    .await;
            }
    ...
    pub fn search_multi_vector(
        ...
        crate::async_task::spawn_async(async move {
            ...
            let primary_results = engine
                .store
                .search_vector(query.primary_vector.clone(), Some(config.max_results * 2))
                .await;
            ...
            for (vector, weight) in &query.secondary_vectors {
                let secondary_results = engine
                    .store
                    .search_vector(vector.clone(), Some(config.max_results * 2))
                    .await;
            ...
            for (i, query_vector) in batch_query.queries.into_iter().enumerate() {
                let config = if let Some(ref configs) = batch_query.configs {
                    configs
                        .get(i)
                        .cloned()
                        .unwrap_or_else(|| batch_query.global_config.clone())
                } else {
        ...
        tokio::spawn(async move {
            ...
            for (index, result) in results.into_iter().enumerate() {
                ...
                metadata.insert(
                    ...
                    serde_json::json!(result.result.score),
                );
                ...
                for (metric, score) in result.metric_scores {
                    metadata.insert(
                        format!("{:?}_score", metric).to_lowercase(),
    ...
    async fn perform_primary_search(
        ...
        if let Some(candidates) = candidates {
            ...
            self.store
                .search_vector(query_vector.to_vec(), Some(config.max_results * 2))
                .await
        }
    }
    ...
    fn apply_diversification(
        ...
        while diversified.len() < results.len() && diversified.len() < 50 {
            ...
            for (i, candidate) in results.iter().enumerate() {
                ...
                for selected in &diversified {
                    ...
                    let similarity = if candidate.id == selected.id {
                        1.0
                    } else {
                        0.0
                    };
    ...
    fn combine_scores(&self, scores: &[(f32, f32)], strategy: CombinationStrategy) -> f32 {
        ...
        match strategy {
            ...
            CombinationStrategy::Maximum => scores
                .iter()
                .map(|(score, _)| *score)
                .fold(f32::NEG_INFINITY, f32::max),
            CombinationStrategy::Minimum => scores
                .iter()
                .map(|(score, _)| *score)
                .fold(f32::INFINITY, f32::min),
            CombinationStrategy::Product => scores.iter().map(|(score, _)| *score).product(),
    ...
    fn update_search_stats(&self, search_time_us: f64) {
        ...
        if stats.total_searches == 1 {
            ...
            stats.avg_search_time_us =
                alpha * search_time_us + (1.0 - alpha) * stats.avg_search_time_us;
        }
    }
