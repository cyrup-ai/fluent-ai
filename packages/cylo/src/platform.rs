...
impl PlatformInfo {
    ...
    fn detect_available_backends(
        ...
        backends.push(Self::detect_firecracker_backend_availability(os, capabilities));
    ...
    fn detect_kernel_version() -> Option<String> {
        ...
        fs::read_to_string("/proc/version")
            .ok()
            .and_then(|content| {
                content.split_whitespace().nth(2).map(|v| v.to_string())
            })
    }
    ...
    fn detect_container_support(os: &OperatingSystem) -> ContainerSupport {
        ContainerSupport {
            ...
            apple_containers: Self::is_command_available("container") && matches!(os, OperatingSystem::MacOS { .. }),
    ...
    fn detect_apple_backend_availability(os: &OperatingSystem, arch: &Architecture) -> BackendAvailability {
        ...
        let (reason, performance_rating) = if available {
            ("Apple Silicon macOS with containerization support".to_string(), 95)
        } else if matches!(os, OperatingSystem::MacOS { .. }) {
            ("macOS detected but not Apple Silicon or containerization not available".to_string(), 0)
        } else {
            ("Apple containerization only available on macOS".to_string(), 0)
        };
    ...
    fn detect_landlock_backend_availability(os: &OperatingSystem, capabilities: &PlatformCapabilities) -> BackendAvailability {
        ...
        let (reason, performance_rating) = if available {
            ...
            ("Linux detected but LandLock or bubblewrap not available".to_string(), 0)
        } else {
    ...
    fn detect_firecracker_backend_availability(os: &OperatingSystem, capabilities: &PlatformCapabilities) -> BackendAvailability {
        let available = matches!(os, OperatingSystem::Linux { .. }) && 
        ...
        let (reason, performance_rating) = if available {
            ...
            ("Linux detected but KVM or FireCracker not available".to_string(), 0)
        } else {
    ...
    fn has_secure_boot() -> bool {
        std::path::Path::new("/sys/firmware/efi/efivars/SecureBoot-8be4df61-93ca-11d2-aa0d-00e098032b8c").exists()
    }
...
pub fn is_apple_silicon() -> bool {
    ...
    matches!(info.os, OperatingSystem::MacOS { .. }) && 
    matches!(info.arch, Architecture::Arm64)
}
...
mod tests {
    ...
    fn platform_specific_detection() {
        ...
        #[cfg(target_os = "linux")]
        {
            ...
            assert!(info.available_backends.iter().any(|b| b.name == "FireCracker"));
        }
    }
}