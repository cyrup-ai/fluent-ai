...
impl AppleBackend {
    ...
    fn execute_in_container(image: String, request: ExecutionRequest) -> AsyncTask<BackendResult<ExecutionResult>> {
        AsyncTaskBuilder::new()
            .spawn(move || async move {
                let start_time = Instant::now();
                
                // Create unique container name
                let container_name = format!("cylo-{}-{}", 
                ...
                let resource_usage = Self::parse_resource_usage(&container_name).await.unwrap_or_default();
    ...
    fn prepare_execution_command(language: &str, code: &str) -> BackendResult<Vec<String>> {
        match language.to_lowercase().as_str() {
            "python" | "python3" => {
                Ok(vec!["python3".to_string(), "-c".to_string(), code.to_string()])
            },
            "javascript" | "js" | "node" => {
            ...
            "bash" | "sh" => {
                Ok(vec!["sh".to_string(), "-c".to_string(), code.to_string()])
            },
            "go" => {
                Ok(vec![
                    "sh".to_string(),
                ...
                match Self::ensure_image_available(image.clone()).await {
                    ...
                    Err(e) => {
                        return ExecutionResult::failure(
                            -1,
                            format!("Failed to prepare image: {}", e)
                        );
                    }
                }
        ...
        AsyncTaskBuilder::new()
            .spawn(|| async move {
                ...
                let cleanup_result = Command::new("container")
                    .args(&["ps", "-a", "--filter", "name=cylo-", "--format", "{{.Names}}"])
                    .output();
...
mod tests {
    ...
    fn image_format_validation() {
        ...
        assert!(AppleBackend::is_valid_image_format("registry.io/user/image:tag"));
    ...
    fn execution_command_preparation() {
        let python_cmd = AppleBackend::prepare_execution_command("python", "print('hello')").unwrap();
        assert_eq!(python_cmd, vec!["python3", "-c", "print('hello')"]);
        
        let js_cmd = AppleBackend::prepare_execution_command("javascript", "console.log('hello')").unwrap();
