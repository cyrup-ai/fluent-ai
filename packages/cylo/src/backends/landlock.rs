...
impl LandLockBackend {
    ...
    fn validate_jail_path(jail_path: &Path) -> BackendResult<()> {
        ...
        if !jail_path.exists() {
            if let Err(e) = fs::create_dir_all(jail_path) {
                return Err(BackendError::InvalidConfig {
                    backend: "LandLock",
                    details: format!("Cannot create jail directory {}: {}", jail_path.display(), e),
                });
            }
        }
    ...
    fn setup_jail_environment(&self, request: &ExecutionRequest) -> BackendResult<PathBuf> {
        ...
        fs::set_permissions(&exec_dir, fs::Permissions::from_mode(0o700))
        ...
        match request.language.as_str() {
            ...
            _ => {
                ...
                if matches!(request.language.as_str(), "bash" | "sh") {
                    fs::set_permissions(&code_file, fs::Permissions::from_mode(0o755))
                        .map_err(|e| BackendError::FileSystemFailed {
        ...
        AsyncTaskBuilder::new()
            .spawn(move || async move {
                ...
                cmd.args(&[
                    ...
                    "--ro-bind", "/lib", "/lib",              // Read-only system libraries
                    "--ro-bind", "/lib64", "/lib64",          // Read-only system libraries
                    "--ro-bind", "/bin", "/bin",              // Read-only system binaries
                    ...
                    "--bind", exec_dir.to_str().unwrap_or(""), "/workspace", // Writable workspace
                    "--chdir", "/workspace",                   // Change to workspace
                ...
                if let Some(input) = &request.input {
                    if let Some(stdin) = child.stdin.take() {
                        use std::io::Write;
                        let mut stdin = stdin;
                        stdin.write_all(input.as_bytes()).map_err(|e| BackendError::ProcessFailed {
    ...
    fn prepare_execution_command(language: &str, exec_dir: &Path) -> BackendResult<(String, Vec<String>)> {
        match language.to_lowercase().as_str() {
            "python" | "python3" => {
                Ok(("python3".to_string(), vec!["main.py".to_string()]))
            },
            "javascript" | "js" | "node" => {
                Ok(("node".to_string(), vec!["main.js".to_string()]))
            },
            "rust" => {
            ...
            "bash" | "sh" => {
                Ok(("bash".to_string(), vec!["code".to_string()]))
            },
            "go" => {
                Ok(("bash".to_string(), vec![
                    "-c".to_string(),
            ...
            _ => {
                Err(BackendError::UnsupportedLanguage {
                    backend: "LandLock",
                ...
                match backend.setup_jail_environment(&test_request) {
                    Ok(exec_dir) => {
                        ...
                        HealthStatus::healthy("LandLock backend operational")
                            ...
                            .with_metric("access_fs", &format!("0x{:x}", features.supported_access_fs))
                    },
                    Err(e) => {
                        HealthStatus::unhealthy(format!("Test environment setup failed: {}", e))
