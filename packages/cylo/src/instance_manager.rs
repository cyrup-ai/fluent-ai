...
impl InstanceManager {
    ...
    pub fn get_instance(&self, instance_id: &str) -> AsyncTask<CyloResult<Arc<dyn ExecutionBackend>>> {
        let instances_lock = Arc::new(&self.instances);
                ...
                if needs_health_check {
                    ...
                    if !health_result.is_healthy {
                        return Err(CyloError::backend_unavailable(
                            ...
                            format!("Instance {} is unhealthy: {}", instance_id, health_result.message)
                        ));
                    }
    ...
    pub fn get_instance_health(&self, instance_id: &str) -> CyloResult<Option<HealthStatus>> {
        ...
        Ok(instances.get(instance_id).and_then(|managed| managed.last_health.clone()))
    }
        ...
        AsyncTaskBuilder::new()
            .spawn(move || async move {
                ...
                for (instance_id, backend) in instance_list {
                    ...
                    let health_task = AsyncTaskBuilder::new()
                        .spawn(move || async move {
            ...
            .spawn(move || async move {
                ...
                // Identify idle instances
                {
                    ...
                    for (instance_id, managed) in instances.iter() {
                        let idle_time = now.duration_since(managed.last_accessed)
        ...
        AsyncTaskBuilder::new()
            .spawn(move || async move {
                ...
                for (instance_id, managed) in all_instances {
                    let id = instance_id.clone();
                    let cleanup_task = AsyncTaskBuilder::new()
                        .spawn(move || async move {
                            if let Err(e) = managed.backend.cleanup().await {
                                eprintln!("Warning: Failed to cleanup instance {} during shutdown: {}", id, e);
                            }
                        });
...
pub fn init_global_instance_manager(
    ...
    GLOBAL_INSTANCE_MANAGER.set(manager)
