...
use termcolor::{
    colored_println, success_check, warning_triangle, error_x, info_i,
    ColoredMessage, ThemeConfig, CyrupTheme, set_global_theme,
};
...
fn main() {
    ...
    ColoredMessage::new()
        ...
        .newline()
        .accent("  1. ").text_secondary("Initialize the model with your API key")
        .newline()
        .accent("  2. ").text_secondary("Process the user's request through the completion pipeline")
        .newline()
        .accent("  3. ").text_secondary("Stream the response with proper error handling")
        .newline()
        .println().unwrap();
    ...
    ColoredMessage::new()
        ...
        .text_primary("bold text").bold()
        .text_primary(", ")
        .text_primary("italic text").italic()
        .text_primary(", and ")
