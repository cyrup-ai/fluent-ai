//! Document builder implementations
//!
//! All document construction logic and builder patterns.

use fluent_ai_domain::{AsyncTask, spawn_async, AsyncStream};
use fluent_ai_domain::document::{Document, ContentFormat, DocumentMediaType};
...
pub struct DocumentBuilderWithHandler {
    ...
    chunk_handler: Option<Box<dyn FnMut(DocumentChunk) -> DocumentChunk + Send + 'static>>,
}
...
impl DocumentBuilder {
    ...
    where
        F: FnMut(DocumentChunk) -> DocumentChunk + Send + 'static,
    {
        DocumentBuilderWithHandler {
            data: self.data,
            format: self.format,
