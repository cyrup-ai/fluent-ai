...
use fluent_ai_provider::{
    completion_provider::{ModelPrompt, CompletionProvider, CompletionError},
...
async fn example_direct_prompting() -> Result<(), CompletionError> {
    ...
    while let Some(chunk) = stream.next().await {
        match chunk {
            ...
            CompletionChunk::Complete { text, finish_reason, usage } => {
                println!("\n✅ Complete: {}", text);
...
async fn example_advanced_builder() -> Result<(), CompletionError> {
    ...
    let documents = ZeroOneOrMany::One(
        Document::new("rust_guide.md", "Ownership is Rust's unique feature for memory safety...")
    );
    ...
    let final_builder = builder_with_explicit_key.on_chunk(|chunk_result| {
        match chunk_result {
            Ok(chunk) => {
                match chunk {
                    CompletionChunk::Text { content, .. } => {
                    ...
                    CompletionChunk::ToolPartial { id, name, arguments, .. } => {
    ...
    while let Some(chunk) = stream.next().await {
        match chunk {
            ...
            CompletionChunk::ToolComplete { id, name, result, .. } => {
                println!("✅ Tool {} ({}) completed: {}", name, id, result);
            }
            CompletionChunk::Complete { text, finish_reason, usage } => {
                println!("\n\n🎯 Final completion summary:");
                println!("📝 Total accumulated text length: {} characters", accumulated_text.len());
                ...
                if let Some(usage_info) = usage {
                    ...
                    let efficiency = usage_info.completion_tokens as f64 / usage_info.total_tokens as f64;
...
async fn example_performance_demo() -> Result<(), CompletionError> {
    ...
    println!("🏗️ Builder creation: {:?} (compile-time defaults)", builder_time);
...
async fn example_multi_model() -> Result<(), CompletionError> {
    ...
    let models = vec![
        ("GPT-4o", Box::new(Gpt4o) as Box<dyn ModelPrompt<Provider = OpenAICompletionBuilder>>),
        ("GPT-4o Mini", Box::new(Gpt4oMini) as Box<dyn ModelPrompt<Provider = OpenAICompletionBuilder>>),
    ];
