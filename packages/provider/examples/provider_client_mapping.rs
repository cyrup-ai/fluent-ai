...
use fluent_ai_provider::{
    ClientConfig, ClientFactoryResult, Providers, UnifiedClient,
};
use std::sync::Arc;
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let provider_names = vec![
        "openai", "anthropic", "claude", "gemini", "mistral", "groq", "perplexity", "xai"
    ];
    ...
    match Providers::openai_client("sk-test-key".to_string()).await {
        Ok(client) => {
            ...
            println!("   Provider: {}", client.provider_name());
            println!("   Available models: {:?}", client.get_models().await.unwrap_or_default());
        }
    ...
    for client in clients {
        println!("🔧 Testing unified interface for {}", client.provider_name());
    ...
    for provider in all_providers {
        let supported = if provider.is_supported() { "✅" } else { "❌" };
        let env_vars = provider.required_env_vars().join(", ");
        println!("| {:10} | {:8} | {:18} |", provider.name(), supported, env_vars);
    }
