...
use fluent_ai_provider::clients::anthropic::{AnthropicError, AnthropicResult};
...
fn test_typestate_builder_chain() {
    ...
    let tool = ToolBuilder::named("calculator")
        ...
        .on_invocation(|_conv, _emit, req: CalculatorRequest, dep: &TestDependency| {
            Box::pin(async move {
...
fn test_tool_registration_and_storage() {
    ...
    let tool = ToolBuilder::named("test_tool")
        ...
        .request_schema::<CalculatorRequest>(SchemaType::Serde)
        .result_schema::<CalculatorResult>(SchemaType::Serde)
        .on_invocation(|_conv, _emit, _req: CalculatorRequest, _dep: &TestDependency| {
...
fn test_duplicate_tool_registration() {
    ...
    if let Err(AnthropicError::ToolError(msg)) = result2 {
        assert!(msg.contains("already exists"), "Error should mention tool already exists");
    } else {
...
fn test_tool_error_handling() {
    ...
    let tool = ToolBuilder::named("error_tool")
        ...
        .request_schema::<CalculatorRequest>(SchemaType::Serde)
        .result_schema::<CalculatorResult>(SchemaType::Serde)
        .on_invocation(|_conv, _emit, req: CalculatorRequest, _dep: &TestDependency| {
            Box::pin(async move {
                // Simulate error condition
                if req.expression == "error" {
                    return Err(AnthropicError::InvalidInput("Invalid expression".to_string()));
                }
                Ok(())
            })
        })
