...
impl Default for CredentialConfig {
    fn default() -> Self {
        ...
        environment_mappings.insert("openai".to_string(), vec![
            "OPENAI_API_KEY".to_string(),
            "OPENAI_API_TOKEN".to_string(),
        ]);
        
        // Anthropic environment variables
        environment_mappings.insert("anthropic".to_string(), vec![
            "ANTHROPIC_API_KEY".to_string(),
            "CLAUDE_API_KEY".to_string(),
        ]);
        
        // AI21 environment variables
        environment_mappings.insert("ai21".to_string(), vec![
        ...
        environment_mappings.insert("google".to_string(), vec![
            "GOOGLE_API_KEY".to_string(),
            "GEMINI_API_KEY".to_string(),
        ]);
        
        environment_mappings.insert("mistral".to_string(), vec![
            "MISTRAL_API_KEY".to_string(),
        ]);
        
        environment_mappings.insert("groq".to_string(), vec![
...
impl CredentialManager {
    ...
    pub async fn new(config: CredentialConfig) -> SecurityResult<Self> {
        let encryption = if let Some(storage_config) = &config.encrypted_storage {
            Some(Arc::new(
                EncryptionEngine::new(&storage_config.encryption_key_path)
        ...
        let audit_logger = Arc::new(
            AuditLogger::new(&config.audit_config)
                .await
    ...
    pub async fn get_credential(&self, provider: &str) -> SecurityResult<Arc<SecureCredential>> {
        ...
        self.credentials.insert(provider.to_string(), credential.clone());
    ...
    async fn is_credential_valid(&self, credential: &SecureCredential) -> SecurityResult<bool> {
        ...
        if self.config.validation.validate_on_load {
            let is_valid = self.validate_credential_against_provider(credential).await?;
    ...
    pub async fn update_credential(
        ...
        // Update cache
        self.credentials.insert(provider.to_string(), Arc::new(credential.clone()));
