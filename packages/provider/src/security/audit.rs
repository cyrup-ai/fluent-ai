...
impl AuditLogger {
    ...
    pub async fn new(config: &AuditConfig) -> SecurityResult<Self> {
        ...
        let statistics = AuditStatistics {
            ...
            log_file_size: log_file.metadata()
                .map(|m| m.len())
                .unwrap_or(0),
    ...
    async fn create_metadata(&self, event: &AuditEvent) -> HashMap<String, String> {
        ...
        metadata.insert("thread_id".to_string(), 
        ...
        match event {
            ...
            AuditEvent::Security(sec_event) => {
                ...
                match sec_event {
                    ...
                    SecurityEvent::PolicyViolation { policy, severity, .. } => {
    ...
    pub async fn verify_log_integrity(&self) -> SecurityResult<bool> {
        ...
        for line in content.lines() {
            ...
            if entry.previous_hash != previous_hash {
                error!("Audit log integrity violation: hash chain broken at sequence {}", entry.sequence);
                return Ok(false);
            }
            ...
            if entry.entry_hash != calculated_hash {
                error!("Audit log integrity violation: entry hash mismatch at sequence {}", entry.sequence);
                return Ok(false);
            }
...
mod tests {
    ...
    async fn test_log_integrity() {
        ...
        // Verify integrity
        assert!(logger.verify_log_integrity().await.unwrap());
