...
impl EncryptionEngine {
    ...
    pub fn encrypt_credential(&self, credential_value: &str) -> SecurityResult<String> {
        ...
        let serialized = bincode::serialize(&encrypted_data).map_err(|e| {
    ...
    pub fn decrypt_credential(&self, encrypted_string: &str) -> SecurityResult<String> {
        // Decode from base64
        let serialized = base64::decode(encrypted_string).map_err(|e| {
            SecurityError::EncryptionError {
...
impl Drop for EncryptionEngine {
    fn drop(&mut self) {
        // The ChaCha20Poly1305 cipher will be zeroized due to ZeroizeOnDrop derive
    }
}
