...
pub trait ProviderModelDiscovery: Send + Sync + 'static {
        ...
        M: Model + 'static,
    {
        ...
        registry.register(self.provider_name(), model)
...
static PROVIDER_REGISTRY: Lazy<DashMap<&'static str, Arc<dyn ProviderModelDiscovery>>> = Lazy::new(DashMap::new);
...
pub fn get_provider(provider_name: &str) -> Option<Arc<dyn ProviderModelDiscovery>> {
    PROVIDER_REGISTRY.get(provider_name).map(|p| p.value().clone())
}

/// Discover and register all models from all registered providers
pub async fn discover_all_models() -> DiscoveryResult<()> {
    let providers: Vec<_> = PROVIDER_REGISTRY.iter().map(|p| p.value().clone()).collect();
    ...
    for provider in providers {
        ...
        if let Err(e) = provider.discover_and_register().await {
            error!("Failed to discover models for provider {}: {}", provider_name, e);
            return Err(e);
        }
    }
