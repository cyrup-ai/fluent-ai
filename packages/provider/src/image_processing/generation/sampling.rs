...
pub fn euler_sample(
    ...
    let mut x = flux::sampling::get_noise(1, height, width, y.device())
        .map_err(|e| {
            SamplingError::NoiseGenerationError(format!("Failed to generate noise: {}", e))
        })?
        .to_dtype(DType::F16)
    ...
    for (step, window) in sigmas.windows(2).enumerate() {
        ...
        let noise_pred = mmdit
            .forward(
                &Tensor::cat(&[&x, &x], 0).map_err(|e| SamplingError::TensorError(e))?,
        ...
        if let Some(slg_config) = slg_config.as_ref() {
            ...
            if step_fraction >= slg_config.start && step_fraction <= slg_config.end {
                let slg_noise_pred = mmdit
                    .forward(
                        ...
                        &Tensor::full(timestep as f32, (1,), x.device())
                            ...
                            .contiguous()
                            .map_err(|e| SamplingError::TensorError(e))?,
                        &y.i(..1).map_err(|e| SamplingError::TensorError(e))?,
                        &context.i(..1).map_err(|e| SamplingError::TensorError(e))?,
                        Some(&slg_config.layers),
                    )
                    .map_err(|e| {
                        SamplingError::MMDiTError(format!("SLG forward pass failed: {}", e))
                    })?;

                // Apply skip layer guidance
                let slg_adjustment = (noise_pred
                    .i(..1)
                    .map_err(|e| SamplingError::TensorError(e))?
                    - slg_noise_pred
                        .i(..1)
                ...
                guidance =
                    (guidance + slg_adjustment).map_err(|e| SamplingError::TensorError(e))?;
            }
        }

        // Euler integration step
        x = (x + (guidance * (*s_prev - *s_curr)).map_err(|e| SamplingError::TensorError(e))?)
            .map_err(|e| SamplingError::TensorError(e))?;
    }
...
fn apply_cfg(cfg_scale: f64, noise_pred: &Tensor) -> SamplingResult<Tensor> {
    ...
    let cond_pred = noise_pred
        .narrow(0, 0, 1)
        .map_err(|e| SamplingError::TensorError(e))?;

    let uncond_pred = noise_pred
        .narrow(0, 1, 1)
        .map_err(|e| SamplingError::TensorError(e))?;

    let guidance = ((cond_pred * cfg_scale).map_err(|e| SamplingError::TensorError(e))?
        - (uncond_pred * (cfg_scale - 1.0)).map_err(|e| SamplingError::TensorError(e))?)
    .map_err(|e| SamplingError::TensorError(e))?;
...
pub mod noise {
    ...
    pub fn generate_noise(
        ...
        flux::sampling::get_noise(batch_size, height, width, device).map_err(|e| {
            SamplingError::NoiseGenerationError(format!("Failed to generate noise: {}", e))
        })
    }
    ...
    pub fn generate_seeded_noise(
        ...
        if let Err(e) = device.set_seed(seed) {
            return Err(SamplingError::NoiseGenerationError(format!(
                "Failed to set seed: {}",
                e
            )));
        }
...
pub mod schedule {
    ...
    pub fn apply_time_shift(schedule: &[f64], alpha: f64) -> Vec<f64> {
        schedule.iter().map(|&t| time_snr_shift(alpha, t)).collect()
    }
}
