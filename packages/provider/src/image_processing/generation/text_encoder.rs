...
impl ClipWithTokenizer {
    ...
    pub fn new(
        ...
        let clip = stable_diffusion::clip::ClipTextTransformer::new(vb, &config)
            .map_err(|e| TextEncodingError::ModelLoadingError(format!("CLIP model loading failed: {}", e)))?;
        ...
        let tokenizer = Tokenizer::from_file(path_buf.to_str().ok_or_else(|| {
            TextEncodingError::TokenizerLoadingError("Failed to serialize HuggingFace PathBuf".to_string())
        })?)
        .map_err(|e| TextEncodingError::TokenizerLoadingError(format!("Tokenizer loading failed: {}", e)))?;
    ...
    pub fn encode_text_to_embedding(
        ...
        let pad_id = match &self.config.pad_with {
            Some(padding) => *self
                ...
                .get_vocab(true)
                .get(padding.as_str())
                .ok_or_else(|| TextEncodingError::EncodingError("Failed to tokenize padding".to_string()))?,
            None => *self
                .tokenizer
                .get_vocab(true)
                .get("<|endoftext|>")
                .ok_or_else(|| TextEncodingError::EncodingError("Failed to tokenize end-of-text".to_string()))?,
        };
        ...
        let (text_embeddings, text_embeddings_penultimate) = self
            ...
            .map_err(|e| TextEncodingError::EncodingError(format!("CLIP forward pass failed: {}", e)))?;
...
impl T5WithTokenizer {
    ...
    pub fn new(
        ...
        let t5 = t5::T5EncoderModel::load(vb, config)
            .map_err(|e| TextEncodingError::ModelLoadingError(format!("T5 model loading failed: {}", e)))?;
    ...
    pub fn encode_text_to_embedding(
        ...
        let pad_id = *self
            ...
            .get_vocab(true)
            .get("</s>")
            .ok_or_else(|| TextEncodingError::EncodingError("Failed to tokenize T5 end token".to_string()))?;
        
        let mut tokens = self
            .tokenizer
            .encode(prompt, true)
            .map_err(|e| TextEncodingError::EncodingError(format!("T5 token encoding failed: {}", e)))?
            .get_ids()
            .to_vec();
        ...
        let text_embeddings = self
            .t5
            .forward(&tokens)
            .map_err(|e| TextEncodingError::EncodingError(format!("T5 forward pass failed: {}", e)))?;
...
impl StableDiffusion3TripleClipWithTokenizer {
    ...
    pub fn encode_text_to_embedding(
        ...
        // CLIP-L encoding
        let (clip_l_text_embeddings, clip_l_text_embeddings_pooled) = self
            .clip_l
            .encode_text_to_embedding(prompt, device)?;
        
        // CLIP-G encoding
        let (clip_g_text_embeddings, clip_g_text_embeddings_pooled) = self
            .clip_g
            .encode_text_to_embedding(prompt, device)?;
        
        // T5-XXL encoding
        let t5_text_embeddings = self
            .t5
            .encode_text_to_embedding(prompt, device)?;
        ...
        let y = Tensor::cat(&[
            ...
            t5_text_embeddings.mean(1).map_err(|e| TextEncodingError::TensorError(e))?,
        ], D::Minus1)
        .map_err(|e| TextEncodingError::TensorError(e))?;
...
pub fn get_context_embedder_config() -> candle_transformers::models::mmdit::model::ContextEmbedderConfig {
