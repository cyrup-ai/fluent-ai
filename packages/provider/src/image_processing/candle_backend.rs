...
impl CandleImageProcessor {
    ...
    ) -> ImageProcessingResult<Tensor> {
        #[cfg(feature = "image")]
        {
            // Decode image using the standard image crate (matching CLIP example)
            let dynamic_image = image::load_from_memory(&image.data).map_err(|e| {
                ImageProcessingError::PreprocessingError(format!("Failed to decode image: {}", e))
            })?;
            ...
            .map_err(|e| {
                ImageProcessingError::PreprocessingError(format!("Failed to create tensor: {}", e))
            })?;

            // Permute to channels-first format (3, H, W) - exact match with CLIP example
            let image_tensor = image_tensor.permute((2, 0, 1)).map_err(|e| {
                ImageProcessingError::PreprocessingError(format!("Failed to permute tensor: {}", e))
            })?;

            // Convert to F32 for neural network processing - exact match with CLIP example
            let image_tensor = image_tensor.to_dtype(DType::F32).map_err(|e| {
                ImageProcessingError::PreprocessingError(format!("Failed to convert to F32: {}", e))
            })?;
            ...
            let normalized_tensor = match config.pixel_normalization {
                PixelNormalization::ZeroToOne => {
                    ...
                    image_tensor.affine(1.0 / 255.0, 0.0).map_err(|e| {
                        ImageProcessingError::PreprocessingError(format!(
                            "Failed to normalize to [0,1]: {}",
                            e
                        ))
                    })?
                }
                PixelNormalization::MinusOneToOne => {
                    ...
                    image_tensor.affine(2.0 / 255.0, -1.0).map_err(|e| {
                        ImageProcessingError::PreprocessingError(format!(
                            "Failed to normalize to [-1,1]: {}",
                            e
                        ))
                    })?
                }
                PixelNormalization::ImageNet => {
                    ...
                    let normalized = image_tensor.affine(1.0 / 255.0, 0.0).map_err(|e| {
                        ImageProcessingError::PreprocessingError(format!(
                            "Failed to scale for ImageNet: {}",
                            e
                        ))
                    })?;
                    ...
                    for c in 0..3 {
                        let channel = normalized.get(c).map_err(|e| {
                            ImageProcessingError::PreprocessingError(format!(
                    ...
                    Tensor::stack(&channels, 0).map_err(|e| {
                        ImageProcessingError::PreprocessingError(format!(
                            "Failed to stack normalized channels: {}",
                            e
                        ))
                    })?
                }
    ...
    fn extract_spatial_statistics(&self, tensor: &Tensor) -> ImageProcessingResult<Vec<f32>> {
        ...
        for c in 0..3 {
            let channel = tensor.get(c).map_err(|e| {
                ImageProcessingError::FeatureExtractionError(format!(
                    "Failed to get channel {}: {}",
                    c, e
                ))
            })?;
            ...
            let variance = channel
                ...
                .map_err(|e| {
                    ImageProcessingError::FeatureExtractionError(format!(
                        "Failed to compute variance mean: {}",
                        e
                    ))
                })?
                .to_scalar::<f32>()
                .map_err(|e| {
                    ImageProcessingError::FeatureExtractionError(format!(
                        "Failed to extract variance scalar: {}",
                        e
                    ))
                })?;
            ...
            let max_val = channel
                ...
                .map_err(|e| {
                    ImageProcessingError::FeatureExtractionError(format!(
                        "Failed to compute max reduction: {}",
                        e
                    ))
                })?
                .to_scalar::<f32>()
            ...
            let min_val = channel
                ...
                .map_err(|e| {
                    ImageProcessingError::FeatureExtractionError(format!(
                        "Failed to compute min reduction: {}",
                        e
                    ))
                })?
                .to_scalar::<f32>()
    ...
    fn extract_texture_gradients(&self, tensor: &Tensor) -> ImageProcessingResult<Vec<f32>> {
        ...
        .map_err(|e| {
            ImageProcessingError::FeatureExtractionError(format!(
                "Failed to create Sobel X kernel: {}",
                e
            ))
        })?;
        ...
        .map_err(|e| {
            ImageProcessingError::FeatureExtractionError(format!(
                "Failed to create Sobel Y kernel: {}",
                e
            ))
        })?;
        ...
        for c in 0..3 {
            ...
            let channel_4d = channel
                ...
                .map_err(|e| {
                    ImageProcessingError::FeatureExtractionError(format!(
                        "Failed to add channel dimension: {}",
                        e
                    ))
                })?;

            // Compute gradients using convolution
            let grad_x = channel_4d.conv2d(&sobel_x, 1, 1, 1, 1).map_err(|e| {
                ImageProcessingError::FeatureExtractionError(format!(
                    "Failed to compute X gradient: {}",
                    e
                ))
            })?;

            let grad_y = channel_4d.conv2d(&sobel_y, 1, 1, 1, 1).map_err(|e| {
                ImageProcessingError::FeatureExtractionError(format!(
                    "Failed to compute Y gradient: {}",
                    e
                ))
            })?;

            // Compute gradient magnitudes
            let grad_x_mean = grad_x
                .abs()
                .map_err(|e| {
                    ImageProcessingError::FeatureExtractionError(format!(
                        "Failed to compute gradient X abs: {}",
                        e
                    ))
                })?
                .mean_all()
                .map_err(|e| {
                    ImageProcessingError::FeatureExtractionError(format!(
                        "Failed to compute gradient X mean: {}",
                        e
                    ))
                })?
                .to_scalar::<f32>()
                .map_err(|e| {
                    ImageProcessingError::FeatureExtractionError(format!(
                        "Failed to extract gradient X scalar: {}",
                        e
                    ))
                })?;

            let grad_y_mean = grad_y
                .abs()
                .map_err(|e| {
                    ImageProcessingError::FeatureExtractionError(format!(
                        "Failed to compute gradient Y abs: {}",
                        e
                    ))
                })?
                .mean_all()
                .map_err(|e| {
                    ImageProcessingError::FeatureExtractionError(format!(
                        "Failed to compute gradient Y mean: {}",
                        e
                    ))
                })?
                .to_scalar::<f32>()
                .map_err(|e| {
                    ImageProcessingError::FeatureExtractionError(format!(
                        "Failed to extract gradient Y scalar: {}",
                        e
                    ))
                })?;
    ...
    fn extract_color_statistics(&self, tensor: &Tensor) -> ImageProcessingResult<Vec<f32>> {
        ...
        for c in 0..3 {
            ...
            let mean = channel
                ...
                .map_err(|e| {
                    ImageProcessingError::FeatureExtractionError(format!(
                        "Failed to extract color mean scalar: {}",
                        e
                    ))
                })?;
            ...
            let variance = channel
                ...
                .map_err(|e| {
                    ImageProcessingError::FeatureExtractionError(format!(
                        "Failed to compute color variance: {}",
                        e
                    ))
                })?
                .mean_all()
                .map_err(|e| {
                    ImageProcessingError::FeatureExtractionError(format!(
                        "Failed to compute color variance mean: {}",
                        e
                    ))
                })?
                .to_scalar::<f32>()
                .map_err(|e| {
                    ImageProcessingError::FeatureExtractionError(format!(
                        "Failed to extract color variance scalar: {}",
                        e
                    ))
                })?;
            ...
            let max_val = channel
                ...
                .map_err(|e| {
                    ImageProcessingError::FeatureExtractionError(format!(
                        "Failed to compute color max reduction: {}",
                        e
                    ))
                })?
                .to_scalar::<f32>()
                .map_err(|e| {
                    ImageProcessingError::FeatureExtractionError(format!(
                        "Failed to extract color max scalar: {}",
                        e
                    ))
                })?;
            ...
            let min_val = channel
                ...
                .map_err(|e| {
                    ImageProcessingError::FeatureExtractionError(format!(
                        "Failed to compute color min reduction: {}",
                        e
                    ))
                })?
                .to_scalar::<f32>()
                .map_err(|e| {
                    ImageProcessingError::FeatureExtractionError(format!(
                        "Failed to extract color min scalar: {}",
                        e
                    ))
                })?;
    ...
    fn extract_edge_responses(&self, tensor: &Tensor) -> ImageProcessingResult<Vec<f32>> {
        ...
        .map_err(|e| {
            ImageProcessingError::FeatureExtractionError(format!(
                "Failed to create Laplacian kernel: {}",
                e
            ))
        })?;
        ...
        for c in 0..3 {
            ...
            let channel_4d = channel
                ...
                .map_err(|e| {
                    ImageProcessingError::FeatureExtractionError(format!(
                        "Failed to add edge batch dimension: {}",
                        e
                    ))
                })?
                .unsqueeze(0)
                .map_err(|e| {
                    ImageProcessingError::FeatureExtractionError(format!(
                        "Failed to add edge channel dimension: {}",
                        e
                    ))
                })?;

            // Apply Laplacian filter
            let edges = channel_4d.conv2d(&laplacian, 1, 1, 1, 1).map_err(|e| {
                ImageProcessingError::FeatureExtractionError(format!(
                    "Failed to compute edges: {}",
                    e
                ))
            })?;

            // Compute edge strength (mean absolute response)
            let edge_strength = edges
                .abs()
                .map_err(|e| {
                    ImageProcessingError::FeatureExtractionError(format!(
                        "Failed to compute edge abs: {}",
                        e
                    ))
                })?
                .mean_all()
    ...
    fn map_features_to_embedding(&self, vision_features: &[f32]) -> Vec<f32> {
        ...
        if !vision_features.is_empty() {
            ...
            let feature_window_size =
                (vision_features.len() as f32 / self.embedding_dims as f32).ceil() as usize;
        ...
        if let Some(device_config) = config.get("device_config") {
            if let Ok(device_type) = serde_json::from_value::<DeviceType>(device_config.clone()) {
                self.device = match device_type {
                    ...
                    DeviceType::Cuda => Device::cuda_if_available(0).map_err(|e| {
                        ImageProcessingError::DeviceSelectionError(format!(
                            "CUDA not available: {}",
                            e
                        ))
                    })?,
                    DeviceType::Metal => Device::metal_if_available(0).map_err(|e| {
                        ImageProcessingError::DeviceSelectionError(format!(
                            "Metal not available: {}",
                            e
                        ))
            ...
            metadata: ImageEmbeddingMetadata {
                ...
                backend_metadata: {
                    ...
                    metadata.insert(
                        ...
                        serde_json::Value::Number(serde_json::Number::from(vision_features.len())),
                    );
                    metadata.insert(
                        "device".to_string(),
