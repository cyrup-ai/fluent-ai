...
use super::error::{BedrockError, Result, CredentialsSource};
...
impl SigV4Signer {
    ...
    fn get_timestamp(&self) -> Result<ArrayString<16>> {
        ...
        if timestamp.try_push_str(&format!(
            ...
            return Err(BedrockError::signature_error("Timestamp formatting failed", "timestamp"));
        }
    ...
    fn build_canonical_request(
        ...
        if canonical_request.try_push_str(method).is_err() {
            return Err(BedrockError::signature_error("Method too long", "canonical_request"));
        }
        if canonical_request.try_push('\n').is_err() {
            return Err(BedrockError::signature_error("Buffer overflow", "canonical_request"));
        }
        
        // Canonical URI (already URL encoded)
        if canonical_request.try_push_str(uri).is_err() {
            return Err(BedrockError::signature_error("URI too long", "canonical_request"));
        }
        if canonical_request.try_push('\n').is_err() {
            return Err(BedrockError::signature_error("Buffer overflow", "canonical_request"));
        }
        ...
        if canonical_request.try_push_str(query_string).is_err() {
            return Err(BedrockError::signature_error("Query string too long", "canonical_request"));
        }
        if canonical_request.try_push('\n').is_err() {
            return Err(BedrockError::signature_error("Buffer overflow", "canonical_request"));
        }
        ...
        let mut sorted_headers: arrayvec::ArrayVec<(&str, &str), 32> = headers.iter().copied().collect();
        sorted_headers.sort_by(|a, b| a.0.cmp(b.0));
        
        for (name, value) in sorted_headers.iter() {
            if canonical_request.try_push_str(&format!("{}:{}\n", name.to_lowercase(), value.trim())).is_err() {
                return Err(BedrockError::signature_error("Headers too long", "canonical_request"));
            }
        }
        if canonical_request.try_push('\n').is_err() {
            return Err(BedrockError::signature_error("Buffer overflow", "canonical_request"));
        ...
        for (i, (name, _)) in sorted_headers.iter().enumerate() {
            ...
            if canonical_request.try_push_str(&name.to_lowercase()).is_err() {
                return Err(BedrockError::signature_error("Header names too long", "canonical_request"));
            }
        }
        if canonical_request.try_push('\n').is_err() {
            return Err(BedrockError::signature_error("Buffer overflow", "canonical_request"));
        ...
        if canonical_request.try_push_str(&format!("{:x}", payload_hash)).is_err() {
            return Err(BedrockError::signature_error("Payload hash too long", "canonical_request"));
        }
    ...
    fn build_string_to_sign(
        ...
        if string_to_sign.try_push_str("AWS4-HMAC-SHA256\n").is_err() {
            return Err(BedrockError::signature_error("Buffer overflow", "string_to_sign"));
        }
        
        // Timestamp
        if string_to_sign.try_push_str(timestamp).is_err() {
            return Err(BedrockError::signature_error("Timestamp too long", "string_to_sign"));
        }
        ...
        if string_to_sign.try_push_str(&format!("{}/{}/{}/aws4_request\n", date, region, self.service)).is_err() {
            return Err(BedrockError::signature_error("Credential scope too long", "string_to_sign"));
        }
        
        // Canonical request hash
        let canonical_hash = Sha256::digest(canonical_request.as_bytes());
        if string_to_sign.try_push_str(&format!("{:x}", canonical_hash)).is_err() {
            return Err(BedrockError::signature_error("Canonical hash too long", "string_to_sign"));
        }
    ...
    fn calculate_signature(
        ...
        if signature.try_push_str(&format!("{:x}", signature_bytes)).is_err() {
            return Err(BedrockError::signature_error("Signature formatting failed", "signature"));
        }
    ...
    fn build_authorization_header(
        ...
        if auth_header.try_push_str(&format!(
            "AWS4-HMAC-SHA256 Credential={}/{}/{}/{}/aws4_request",
            access_key_id, date, region, self.service
        )).is_err() {
            return Err(BedrockError::signature_error("Credential too long", "authorization"));
        }
        
        // Signed headers
        if auth_header.try_push_str(", SignedHeaders=").is_err() {
            return Err(BedrockError::signature_error("Buffer overflow", "authorization"));
        }
        ...
        for (i, name) in sorted_header_names.iter().enumerate() {
            ...
            if auth_header.try_push_str(&name.to_lowercase()).is_err() {
                return Err(BedrockError::signature_error("Header names too long", "authorization"));
            }
        }
        
        // Signature
        if auth_header.try_push_str(&format!(", Signature={}", signature)).is_err() {
            return Err(BedrockError::signature_error("Signature too long", "authorization"));
        }
