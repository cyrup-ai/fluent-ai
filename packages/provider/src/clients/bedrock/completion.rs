...
impl BedrockCompletionBuilder {
    ...
    fn build_request_body(&self, messages: &[Message]) -> Result<Vec<u8>> {
        ...
        request.insert("messages".to_string(), serde_json::Value::Array(bedrock_messages));
        ...
        if !self.stop_sequences.is_empty() {
            let stop_seqs: Vec<serde_json::Value> = self.stop_sequences.iter()
                .map(|s| serde_json::Value::String(s.clone()))
                .collect();
            inference_config.insert("stopSequences".to_string(), serde_json::Value::Array(stop_seqs));
        }
        
        if !inference_config.is_empty() {
            request.insert("inferenceConfig".to_string(), serde_json::Value::Object(inference_config));
        }
        ...
        if let Some(ref system) = self.system_prompt {
            ...
            system_prompt.insert("text".to_string(), serde_json::Value::String(system.clone()));
            system_prompts.push(serde_json::Value::Object(system_prompt));
            request.insert("system".to_string(), serde_json::Value::Array(system_prompts));
        }
        
        // Add tools if provided and model supports them
        if !self.tools.is_empty() {
            if let Err(_) = validate_model_capability(self.model, "tools") {
                return Err(BedrockError::config_error("tools", "Model does not support tools"));
            }
            
            let bedrock_tools = self.convert_tools()?;
            request.insert("toolConfig".to_string(), serde_json::Value::Object(bedrock_tools));
        }
    ...
    fn convert_messages(&self, messages: &[Message]) -> Result<Vec<serde_json::Value>> {
        ...
        for message in messages {
            ...
            bedrock_msg.insert("role".to_string(), serde_json::Value::String(role_str.to_string()));
            
            // Set content
            let mut content = Vec::new();
            let mut text_content = serde_json::Map::new();
            text_content.insert("text".to_string(), serde_json::Value::String(message.content.clone()));
    ...
    fn convert_tools(&self) -> Result<serde_json::Map<String, serde_json::Value>> {
        ...
        for tool in &self.tools {
            ...
            tool_spec.insert("name".to_string(), serde_json::Value::String(tool.name.clone()));
            tool_spec.insert("description".to_string(), serde_json::Value::String(tool.description.clone()));
    ...
    fn build_endpoint_url(&self) -> ArrayString<128> {
        ...
        let _ = url.try_push_str(&format!(
            ...
            self.region,
            self.model,
            if self.stream { "invoke-with-response-stream" } else { "invoke" }
        ));
        url
    }
    ...
    async fn execute_completion(&self, messages: &[Message]) -> Result<CompletionResponse> {
        ...
        // Prepare headers
        let headers = [
            ("host", &format!("bedrock-runtime.{}.amazonaws.com", self.region)),
...
impl CompletionProvider for BedrockCompletionBuilder {
    ...
    fn stream(&self, request: CompletionRequest) -> AsyncTask<Result<AsyncStream<Self::StreamingResponse>, Self::Error>> {
        ...
        AsyncTask::spawn(async move {
            let stream = builder.stream_completion_internal(&request.messages).await
                .map_err(|e| CompletionError::from(e))?;
...
impl BedrockCompletionBuilder {
    ...
    async fn stream_completion_internal(&self, messages: &[Message]) -> Result<AsyncStream<CompletionChunk>> {
        let request_body = self.build_request_body(messages)?;
        let endpoint = self.build_endpoint_url();
