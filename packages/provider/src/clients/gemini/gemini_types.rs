...
impl TryFrom<Value> for Schema {
    ...
    fn try_from(value: Value) -> GeminiResult<Self> {
        ...
        let properties = obj
            ...
            .map(|map| -> GeminiResult<HashMap<String, Schema>> {
                let mut result = HashMap::new();
                for (k, v) in map {
                    let schema = v.clone().try_into()?;
                    result.insert(k.clone(), schema);
                }
                Ok(result)
            })
            .transpose()?;
        ...
        let items = obj
            ...
            .map(|v| -> GeminiResult<Box<Schema>> {
                let schema: Schema = v.clone().try_into()?;
                Ok(Box::new(schema))
            })
            .transpose()?;
...
impl TryFrom<completion::ToolDefinition> for Tool {
    ...
    fn try_from(tool: completion::ToolDefinition) -> GeminiResult<Self> {
...
impl TryFrom<GenerateContentResponse> for completion::CompletionResponse<GenerateContentResponse> {
    ...
    fn try_from(response: GenerateContentResponse) -> GeminiResult<Self> {
        ...
        let content = candidate
            ...
            .parts
            .iter()
            .map(|part| -> GeminiResult<completion::AssistantContent> {
                Ok(match part {
