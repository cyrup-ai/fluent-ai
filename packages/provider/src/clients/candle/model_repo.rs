...
impl ModelMetadata {
    ...
    pub fn new(
        ...
        Ok(Self {
            hf_repo_id: ArrayString::from(hf_repo_id)
                .map_err(|_| ModelRepoError::RepoIdTooLong)?,
            architecture,
...
impl ModelRepository {
    ...
    fn initialize_default_models(&self) -> Result<(), ModelRepoError> {
        ...
        self.metadata.insert(CandleModel::Llama2_7B, Arc::new(llama2_7b));
        ...
        self.metadata.insert(CandleModel::Llama2_13B, Arc::new(llama2_13b));
        ...
        self.metadata.insert(CandleModel::Mistral_7B, Arc::new(mistral_7b));
        ...
        self.metadata.insert(CandleModel::CodeLlama_7B, Arc::new(codellama_7b));
        ...
        self.metadata.insert(CandleModel::Phi3_Mini, Arc::new(phi3_mini));
        ...
        self.metadata.insert(CandleModel::Gemma_2B, Arc::new(gemma_2b));
        ...
        self.metadata.insert(CandleModel::Gemma_7B, Arc::new(gemma_7b));
    ...
    pub async fn download_model(&self, model: CandleModel) -> Result<SmallVec<[ModelFileInfo; 8]>, ModelRepoError> {
        let metadata = self.get_metadata(model)
    ...
    async fn download_model_files(
        ...
        let hf_api = hf_client_guard.as_ref()
        ...
        let config_path = self.download_file(&repo, &metadata.config_file, model).await?;
        file_infos.push(ModelFileInfo {
            file_path: config_path.clone(),
            file_size: tokio::fs::metadata(&config_path).await
        ...
        let tokenizer_path = self.download_file(&repo, &metadata.tokenizer_file, model).await?;
        file_infos.push(ModelFileInfo {
            file_path: tokenizer_path.clone(),
            file_size: tokio::fs::metadata(&tokenizer_path).await
                .map_err(|e| ModelRepoError::IoError(e))?
                .len(),
        ...
        if metadata.model_files.ends_with(".index.json") {
            ...
            let index_path = self.download_file(&repo, &metadata.model_files, model).await?;
            ...
            // Single model file
            let model_path = self.download_file(&repo, &metadata.model_files, model).await?;
            let file_size = tokio::fs::metadata(&model_path).await
                .map_err(|e| ModelRepoError::IoError(e))?
                .len();
    ...
    async fn download_file(
        ...
        let downloaded_path = repo.get(filename).await
            .map_err(|e| ModelRepoError::DownloadError(format!("Failed to download {}: {}", filename, e)))?;

        // Move downloaded file to cache location
        tokio::fs::copy(&downloaded_path, &local_path).await
            .map_err(|e| ModelRepoError::IoError(e))?;
    ...
    async fn parse_safetensors_index(&self, index_path: &Path) -> Result<SmallVec<[ArrayString<64>; 16]>, ModelRepoError> {
    ...
    fn create_memory_map(&self, file_path: &Path) -> Result<Mmap, ModelRepoError> {
        ...
        unsafe {
            Mmap::map(&file)
                .map_err(|e| ModelRepoError::MemoryMapError(format!("Failed to create memory map: {}", e)))
        }
    }
    ...
    pub async fn get_cache_size(&self) -> Result<u64, ModelRepoError> {
        ...
        let mut dir_entries = tokio::fs::read_dir(&self.cache_dir).await
            .map_err(|e| ModelRepoError::IoError(e))?;
