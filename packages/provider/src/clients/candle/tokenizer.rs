...
impl SpecialTokens {
    ...
    pub fn is_special_token(&self, token_id: u32) -> bool {
        ...
        self.additional_special_tokens.values().any(|&id| id == token_id)
    }
...
impl CandleTokenizer {
    ...
    async fn initialize_basic_vocabulary(&self) -> CandleResult<()> {
        ...
        let common_tokens = [
            " ", "a", "the", "and", "or", "but", "in", "on", "at", "to", "for", "of", "with",
    ...
    pub fn encode(&self, text: &str) -> CandleResult<TokenizationResult> {
        ...
        // Update statistics
        self.stats.total_chars_processed.store(
            self.stats.total_chars_processed.load() + original_length as u64
        );
        self.stats.total_tokens_generated.store(
            self.stats.total_tokens_generated.load() + tokens.len() as u64
        );
        ...
        if total_chars > 0 {
            self.stats.avg_tokens_per_char.store(total_tokens as f32 / total_chars as f32);
        }
    ...
    pub fn decode_stream(&self, token_id: u32, text_buffer: &mut TextBuffer) -> CandleResult<()> {
        ...
        self.stats.total_decode_ops.store(self.stats.total_decode_ops.load() + 1);
