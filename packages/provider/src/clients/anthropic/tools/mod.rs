...
pub use function_calling::{
    ToolBuilder, NamedTool, DescribedTool, ToolWithDeps, ToolWithRequestSchema, 
    ToolWithSchemas, ToolWithInvocation, TypedToolTrait, TypedToolStorage,
    TypedTool, ToolExecutor, ToolRegistry, ToolExecutionContext, ToolResult, 
    ToolOutput, Conversation
};

// Re-export built-in tools
pub use calculator::{CalculatorTool, ExpressionEvaluator, ExpressionError};
...
impl ToolRegistryBuilder {
    ...
    pub fn with_calculator(mut self) -> AnthropicResult<Self> {
        let calculator = Box::new(CalculatorTool);
        self.registry.register_tool("calculator".to_string(), calculator)?;
        Ok(self)
    }
    ...
    pub fn with_file_operations(mut self) -> AnthropicResult<Self> {
        let file_ops = Box::new(FileOperationsTool);
        self.registry.register_tool("file_operations".to_string(), file_ops)?;
        Ok(self)
    }
    
    /// Add custom tool executor
    #[inline(always)]
    pub fn with_tool(mut self, name: String, executor: Box<dyn ToolExecutor + Send + Sync>) -> AnthropicResult<Self> {
    ...
    #[inline(always)]
    pub fn with_typed_tool<D, Req, Res>(mut self, tool: TypedTool<D, Req, Res>) -> AnthropicResult<Self>
    where
        D: Send + Sync + 'static,
