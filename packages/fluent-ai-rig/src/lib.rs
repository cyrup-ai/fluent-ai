...
use fluent_ai_provider::{Models, Provider as ProviderTrait, Providers};
                ...
                "gpt" => {
                    ...
                    match agent.stream_prompt(&prompt_text).await {
                        Ok(mut stream) => {
                            ...
                            while let Some(chunk) = stream.next().await {
                                match chunk {
                                    Ok(AssistantContent::Text(text)) => {
                                        ...
                                        if tx.send(CompletionChunk::text(text.text)).is_err() {
                                    ...
                                    Err(e) => {
                                        ...
                                        let _ = tx.send(
                                            CompletionChunk::error(format!("Error: {}", e))
                                        );
                                        break;
                                    }
                                }
                            ...
                            let _ = tx.send(CompletionChunk::complete("", Some(FinishReason::Stop), None));
                        }
                        Err(e) => {
...
impl CompletionBackend for RigCompletionBackend {
    fn submit_completion(&self, prompt: &str, _tools: &[String]) -> AsyncTask<String> {
        let fluent_prompt = FluentPrompt::new(prompt);
        let stream = self.prompt(fluent_prompt);

        use fluent_ai::async_task::spawn_async;
        spawn_async(async move {
            let chunks: Vec<CompletionChunk> = stream.collect().await;
            chunks
                .into_iter()
                .filter_map(|chunk| chunk.text_content().map(|s| s.to_string()))
                .collect::<Vec<_>>()
                .join("")
        })
    }
