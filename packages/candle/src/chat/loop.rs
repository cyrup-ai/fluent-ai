//! Chat loop control flow types for the Candle framework.

use std::fmt;

/// Controls the flow of a chat conversation.
///
/// This enum is used to control the flow of a chat conversation, allowing for breaking out of loops,
/// sending responses, or prompting the user for more input.
...
impl fmt::Display for CandleChatLoop {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            CandleChatLoop::UserPrompt(prompt) => write!(f, "CandleChatLoop::UserPrompt({:?})", prompt),
        }
    }
}
...
mod tests {
    ...
    fn test_chat_loop_display() {
        assert_eq!(
            CandleChatLoop::Break.to_string(),
