...
impl ChatSearchIndex {
    ...
    pub async fn add_message(&self, message: ChatMessage) -> Result<(), SearchError> {
        ...
        for (term, count) in term_counts {
            ...
            self.term_frequencies.insert(term, TermFrequency {
                tf,
                df,
                total_docs,
            });
    ...
    pub async fn search(&self, query: &SearchQuery) -> Result<Vec<SearchResult>, SearchError> {
        ...
        match query.operator {
            ...
            QueryOperator::Phrase => {
                results = self.search_phrase(&query.terms, query.fuzzy_matching).await?;
            }
            QueryOperator::Proximity { distance } => {
                results = self.search_proximity(&query.terms, distance, query.fuzzy_matching).await?;
            }
        }
        ...
        stats.average_query_time = (stats.average_query_time * (stats.total_queries - 1) as f64 + query_time) / stats.total_queries as f64;
    ...
    async fn search_and(&self, terms: &[Arc<str>], fuzzy: bool) -> Result<Vec<SearchResult>, SearchError> {
        if terms.is_empty() {
            return Ok(vec![]);
        }
    ...
    async fn search_or(&self, terms: &[Arc<str>], fuzzy: bool) -> Result<Vec<SearchResult>, SearchError> {
        ...
        for result in all_results {
            ...
            if let Some(existing) = unique_results.get_mut(&key) {
                ...
                existing_result.relevance_score = (existing_result.relevance_score + result.relevance_score) / 2.0;
            } else {
    ...
    async fn search_not(&self, terms: &[Arc<str>], fuzzy: bool) -> Result<Vec<SearchResult>, SearchError> {
    ...
    async fn search_phrase(&self, terms: &[Arc<str>], fuzzy: bool) -> Result<Vec<SearchResult>, SearchError> {
        let mut results = Vec::new();
        
        for entry in self.document_store.iter() {
            let message = entry.value();
            let content = message.content.to_lowercase();
            let phrase = terms.iter()
                .map(|t| t.as_ref())
                .collect::<Vec<_>>()
                .join(" ");
    ...
    async fn search_proximity(&self, terms: &[Arc<str>], distance: u32, fuzzy: bool) -> Result<Vec<SearchResult>, SearchError> {
        let mut results = Vec::new();
            ...
            for entry in entries.value() {
                if let Some(message) = self.document_store.get(&entry.doc_id) {
                    ...
                    results.push(SearchResult {
                        ...
                        highlighted_content: Arc::from(self.highlight_text(&message.value().content, term)),
                        tags: vec![],
                        context: vec![],
                    });
                }
    ...
    fn intersect_results(&self, results1: Vec<SearchResult>, results2: Vec<SearchResult>) -> Vec<SearchResult> {
        let mut intersection = Vec::new();
        let ids2: std::collections::HashSet<_> = results2.iter()
    ...
    /// Apply query filters
    async fn apply_filters(&self, mut results: Vec<SearchResult>, query: &SearchQuery) -> Result<Vec<SearchResult>, SearchError> {
        // Date range filter
...
impl ConversationTagger {
    ...
    /// Create a new tag
    pub async fn create_tag(&self, name: Arc<str>, description: Arc<str>, category: Arc<str>) -> Result<Arc<str>, SearchError> {
    ...
    /// Create a child tag
    pub async fn create_child_tag(&self, parent_id: Arc<str>, name: Arc<str>, description: Arc<str>, category: Arc<str>) -> Result<Arc<str>, SearchError> {
    ...
    /// Tag a message
    pub async fn tag_message(&self, message_id: Arc<str>, tag_ids: Vec<Arc<str>>) -> Result<(), SearchError> {
        ...
        for tag_id in &tag_ids {
            if let Some(messages) = self.tag_messages.get(tag_id) {
                ...
                self.tag_messages.insert(tag_id.clone(), vec![message_id.clone()]);
            }
    ...
    pub async fn auto_tag_message(&self, message: &ChatMessage) -> Result<Vec<Arc<str>>, SearchError> {
    ...
    pub fn get_message_tags(&self, message_id: &Arc<str>) -> Vec<Arc<str>> {
        self.message_tags.get(message_id)
    ...
    pub async fn add_auto_tagging_rule(&self, pattern: Arc<str>, tag_ids: Vec<Arc<str>>) -> Result<(), SearchError> {
...
impl HistoryExporter {
    ...
    pub async fn export_history(&self, messages: Vec<ChatMessage>, options: &ExportOptions) -> Result<String, SearchError> {
        let start_time = Instant::now();
        ...
        stats.average_export_time = (stats.average_export_time * (stats.total_exports - 1) as f64 + export_time) / stats.total_exports as f64;
    ...
    async fn filter_messages(&self, mut messages: Vec<ChatMessage>, options: &ExportOptions) -> Result<Vec<ChatMessage>, SearchError> {
        // Date range filter
    ...
    async fn export_json(&self, messages: &[ChatMessage], options: &ExportOptions) -> Result<String, SearchError> {
        ...
        serde_json::to_string_pretty(&export_obj)
            .map_err(|e| SearchError::ExportError { reason: Arc::from(e.to_string()) })
    }
    
    /// Export to CSV format
    async fn export_csv(&self, messages: &[ChatMessage], options: &ExportOptions) -> Result<String, SearchError> {
    ...
    async fn export_markdown(&self, messages: &[ChatMessage], options: &ExportOptions) -> Result<String, SearchError> {
        let mut markdown_output = String::new();
        
        markdown_output.push_str("# Conversation History\n\n");
        
        if options.include_timestamps {
            markdown_output.push_str(&format!("Exported at: {}\n\n", 
        ...
        for message in messages {
            ...
            if options.include_metadata && message.metadata.is_some() {
                markdown_output.push_str(&format!("*Metadata: {}*\n\n", 
    ...
    async fn export_html(&self, messages: &[ChatMessage], options: &ExportOptions) -> Result<String, SearchError> {
        ...
        for message in messages {
            ...
            html_output.push_str(&format!("<div class=\"content\">{}</div>\n", 
                message.content.replace('<', "&lt;").replace('>', "&gt;")));
            
            if options.include_timestamps {
                html_output.push_str(&format!("<div class=\"timestamp\">Timestamp: {}</div>\n", message.timestamp));
            }
            
            if options.include_metadata && message.metadata.is_some() {
                html_output.push_str(&format!("<div class=\"metadata\">Metadata: {}</div>\n", 
                    message.metadata.as_ref().unwrap()));
            }
    ...
    async fn export_xml(&self, messages: &[ChatMessage], options: &ExportOptions) -> Result<String, SearchError> {
        ...
        if options.include_timestamps {
            xml_output.push_str(&format!("  <export_info exported_at=\"{}\"/>\n", 
                std::time::SystemTime::now()
                    .duration_since(std::time::UNIX_EPOCH)
                    .unwrap_or_default()
                    .as_secs()));
        ...
        for message in messages {
            ...
            xml_output.push_str(&format!("    <content>{}</content>\n", 
                message.content.replace('<', "&lt;").replace('>', "&gt;")));
            ...
            if options.include_metadata && message.metadata.is_some() {
                xml_output.push_str(&format!("    <metadata>{}</metadata>\n", 
                    message.metadata.as_ref().unwrap()));
            }
    ...
    async fn export_plain_text(&self, messages: &[ChatMessage], options: &ExportOptions) -> Result<String, SearchError> {
        ...
        for message in messages {
            ...
            if options.include_metadata && message.metadata.is_some() {
                text_output.push_str(&format!("Metadata: {}\n", message.metadata.as_ref().unwrap()));
            }
...
impl EnhancedHistoryManager {
    ...
    /// Search messages
    pub async fn search_messages(&self, query: &SearchQuery) -> Result<Vec<SearchResult>, SearchError> {
...
impl HistoryManagerBuilder {
    ...
    pub fn build(self) -> EnhancedHistoryManager {
        ...
        search_index.simd_threshold.store(self.simd_threshold, Ordering::Relaxed);
