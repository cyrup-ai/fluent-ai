...
use std::sync::Arc;
use crate::chat::templates::core::{TemplateValue, TemplateResult, TemplateError};

/// Template filter function type
pub type FilterFunction = Arc<dyn Fn(&TemplateValue, &[TemplateValue]) -> TemplateResult<TemplateValue> + Send + Sync>;
...
impl FilterRegistry {
    ...
    pub fn apply(&self, name: &str, value: &TemplateValue, args: &[TemplateValue]) -> TemplateResult<TemplateValue> {
        match self.get(name) {
    ...
    fn register_default_filters(&mut self) {
        // uppercase filter
        self.register("uppercase", Arc::new(|value, _args| {
            match value {
        ...
        // lowercase filter
        self.register("lowercase", Arc::new(|value, _args| {
            match value {
        ...
        // trim filter
        self.register("trim", Arc::new(|value, _args| {
            match value {
        ...
        // length filter
        self.register("length", Arc::new(|value, _args| {
            match value {
        ...
        // default filter
        self.register("default", Arc::new(|value, args| {
