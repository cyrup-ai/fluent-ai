...
impl LLMConfig {
    ...
    pub fn new(provider: LLMProvider, model: impl Into<String>) -> Result<Self, LLMConfigError> {
        ...
        if model.is_empty() {
            return Err(LLMConfigError::InvalidModel("Model name cannot be empty".to_string()));
        }
    ...
    pub fn validate(&self) -> Result<(), LLMConfigError> {
        ...
        if self.max_tokens == 0 {
            return Err(LLMConfigError::InvalidParameter("max_tokens must be greater than 0".to_string()));
        }
        
        if !(0.0..=2.0).contains(&self.temperature) {
            return Err(LLMConfigError::InvalidParameter("temperature must be between 0.0 and 2.0".to_string()));
        }
        
        if self.timeout_ms == 0 {
            return Err(LLMConfigError::InvalidParameter("timeout_ms must be greater than 0".to_string()));
        }
