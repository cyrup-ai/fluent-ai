...
use crate::memory::{MemoryError, MemoryConfig};
...
impl<const TOOLS_CAPACITY: usize> AgentBuilder<TOOLS_CAPACITY> {
    ...
    pub fn tool(mut self, tool: McpToolData) -> AgentResult<Self> {
        if self.tools.try_push(tool).is_err() {
            return Err(AgentError::ToolCapacityExceeded { max: TOOLS_CAPACITY });
        }
        Ok(self)
    }
    ...
    pub fn temperature(mut self, temperature: f64) -> AgentResult<Self> {
        if !(0.0..=2.0).contains(&temperature) {
            return Err(AgentError::InvalidModel {
                reason: format!("Temperature {} is outside valid range [0.0, 2.0]", temperature)
            });
        }
        self.temperature = Some(temperature);
        Ok(self)
    ...
    pub fn max_tokens(mut self, max_tokens: u64) -> AgentResult<Self> {
        if !(1..=100_000).contains(&max_tokens) {
            return Err(AgentError::InvalidModel {
                reason: format!("Max tokens {} is outside valid range [1, 100000]", max_tokens)
            });
        }
        self.max_tokens = Some(max_tokens);
        Ok(self)
    ...
    pub async fn build(self) -> AgentResult<Agent> {
        ...
        let model = self.model.ok_or_else(|| AgentError::Config("Model is required".to_string()))?;
        let system_prompt = self.system_prompt.unwrap_or_else(|| "You are a helpful AI assistant.".to_string());
        ...
        let memory_arc = if let Some(shared_memory) = self.shared_memory {
            ...
            let memory_config = self.memory_config.unwrap_or_else(|| self.config.memory_config.clone());
