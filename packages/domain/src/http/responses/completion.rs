...
use crate::http::common::{
    BaseMessage, CommonUsage, FinishReason, ToolCall, ValidationError, MAX_MESSAGES, MAX_TOOLS,
    MAX_IDENTIFIER_LEN,
};
use crate::Provider;
...
impl CompletionResponse {
    ...
    pub fn new(
        ...
        let response_id = ArrayString::from(id)
            .map_err(|_| CompletionResponseError::IdTooLong(id.len()))?;
        ...
        Ok(Self {
            id: response_id,
            object: ArrayString::from("chat.completion").map_err(|_| CompletionResponseError::InternalError)?,
            model: model_name,
            created: now,
    ...
    #[inline]
    pub fn from_provider_json(provider: Provider, json: &Value) -> Result<Self, CompletionResponseError> {
        match provider {
            ...
            Provider::Groq | Provider::OpenRouter | Provider::Together => Self::from_openai_compatible_json(json),
    ...
    fn from_anthropic_json(json: &Value) -> Result<Self, CompletionResponseError> {
        ...
        if let Some(content_array) = json.get("content").and_then(|v| v.as_array()) {
            ...
            let finish_reason = json.get("stop_reason")
                .and_then(|v| v.as_str())
            ...
            let choice = CompletionChoice {
                ...
                message: BaseMessage {
                    ...
                    name: None,
                    tool_call_id: None,
                    tool_calls: tool_calls.try_into().unwrap_or_default(),
                },
    ...
    fn from_bedrock_json(json: &Value) -> Result<Self, CompletionResponseError> {
        ...
        let usage = if let Some(usage_obj) = json.get("usage") {
            let input_tokens = usage_obj.get("inputTokens")
                .and_then(|v| v.as_u64()).unwrap_or(0) as u32;
            let output_tokens = usage_obj.get("outputTokens")
                .and_then(|v| v.as_u64()).unwrap_or(0) as u32;
    ...
    fn from_cohere_json(json: &Value) -> Result<Self, CompletionResponseError> {
        ...
        let usage = if let Some(meta) = json.get("meta") {
            if let Some(billed_units) = meta.get("billed_units") {
                let input_tokens = billed_units.get("input_tokens")
                    .and_then(|v| v.as_u64()).unwrap_or(0) as u32;
                let output_tokens = billed_units.get("output_tokens")
                    .and_then(|v| v.as_u64()).unwrap_or(0) as u32;
                CommonUsage::new(input_tokens, output_tokens)
            } else {
                CommonUsage::new(0, 0)
        ...
        if let Some(text) = json.get("text").and_then(|v| v.as_str()) {
            let finish_reason = json.get("finish_reason")
                .and_then(|v| v.as_str())
    ...
    fn from_ollama_json(json: &Value) -> Result<Self, CompletionResponseError> {
        ...
        let usage = if let Some(prompt_eval_count) = json.get("prompt_eval_count").and_then(|v| v.as_u64()) {
    ...
    fn from_ai21_json(json: &Value) -> Result<Self, CompletionResponseError> {
        ...
        if let Some(completions) = json.get("completions").and_then(|v| v.as_array()) {
            for (index, completion) in completions.iter().enumerate() {
                if let Some(text) = completion.get("data").and_then(|d| d.get("text")).and_then(|v| v.as_str()) {
                    let choice = CompletionChoice {
                        index: index as u32,
                        message: BaseMessage::assistant(text),
                        finish_reason: Some(FinishReason::Stop),
    ...
    #[inline]
    fn parse_anthropic_tool_use(content_block: &Value) -> Result<Option<ToolCall>, CompletionResponseError> {
        let tool_id = content_block.get("id")
            .and_then(|v| v.as_str())
            .unwrap_or("unknown");
        let function_name = content_block.get("name")
            .and_then(|v| v.as_str())
            .unwrap_or("unknown");
    ...
    pub fn primary_text(&self) -> Option<&str> {
        self.choices.first().map(|choice| choice.message.content_str())
    }

    /// Get all completion texts
    #[inline]
    pub fn all_texts(&self) -> Vec<&str> {
        self.choices.iter().map(|choice| choice.message.content_str()).collect()
    }
    ...
    pub fn has_tool_calls(&self) -> bool {
        self.choices.iter().any(|choice| choice.message.has_tool_calls())
    }
...
impl CompletionChoice {
    ...
    #[inline]
    fn from_openai_choice(choice_obj: &Value, index: usize) -> Result<Self, CompletionResponseError> {
        let message_obj = choice_obj.get("message")
        ...
        let message = BaseMessage {
            role: crate::http::common::MessageRole::from_str(role),
        ...
        let finish_reason = choice_obj.get("finish_reason")
            .and_then(|v| v.as_str())
            .map(FinishReason::from_str);
    ...
    #[inline]
    fn from_google_candidate(candidate: &Value, index: usize) -> Result<Self, CompletionResponseError> {
    ...
    fn from_bedrock_output(output: &Value, index: usize) -> Result<Self, CompletionResponseError> {
        let message_obj = output.get("message")
            .ok_or(CompletionResponseError::MissingField("message"))?;
    ...
    #[inline]
    fn parse_openai_tool_call(call_obj: &Value) -> Result<Option<ToolCall>, CompletionResponseError> {
        let id = call_obj.get("id")
            .and_then(|v| v.as_str())
            .unwrap_or("unknown");
        let function_obj = call_obj.get("function")
            .ok_or(CompletionResponseError::MissingField("function"))?;
        let name = function_obj.get("name")
            .and_then(|v| v.as_str())
            .unwrap_or("unknown");
        let arguments = function_obj.get("arguments")
            .and_then(|v| v.as_str())
            .unwrap_or("{}");
...
impl CompletionChunk {
    ...
    #[inline]
    pub fn new(id: &str, model: &str, choices: Vec<ChunkChoice>) -> Result<Self, CompletionResponseError> {
        let chunk_id = ArrayString::from(id)
            .map_err(|_| CompletionResponseError::IdTooLong(id.len()))?;
        let model_name = ArrayString::from(model)
            .map_err(|_| CompletionResponseError::ModelNameTooLong(model.len()))?;
        ...
        Ok(Self {
            id: chunk_id,
            object: ArrayString::from("chat.completion.chunk").map_err(|_| CompletionResponseError::InternalError)?,
            model: model_name,
            created: now,
            choices: choice_array,
            usage: None,
    ...
    #[inline]
    pub fn from_provider_chunk(provider: Provider, data: &[u8]) -> Result<Self, CompletionResponseError> {
        match provider {
            ...
            Provider::Groq | Provider::OpenRouter | Provider::Together => Self::from_openai_chunk(data),
    ...
    fn from_openai_chunk(data: &[u8]) -> Result<Self, CompletionResponseError> {
        let json: Value = serde_json::from_slice(data)
            .map_err(|_| CompletionResponseError::InvalidJson)?;
        ...
        if let Some(usage_obj) = json.get("usage") {
            ...
            let total_tokens = usage_obj.get("total_tokens")
                .and_then(|v| v.as_u64()).unwrap_or(0) as u32;
            chunk.usage = Some(CommonUsage::from_openai(prompt_tokens, completion_tokens, total_tokens));
        }
    ...
    fn from_bedrock_chunk(data: &[u8]) -> Result<Self, CompletionResponseError> {
        ...
        if let Some(content_block_delta) = json.get("contentBlockDelta") {
            let text = content_block_delta.get("delta")
...
impl ChunkChoice {
    ...
    #[inline]
    fn from_openai_delta(choice_obj: &Value, index: usize) -> Result<Self, CompletionResponseError> {
        let delta_obj = choice_obj.get("delta")
            .ok_or(CompletionResponseError::MissingField("delta"))?;
        ...
        let delta = ChunkDelta {
            role: role.map(|r| crate::http::common::MessageRole::from_str(r)),
    ...
    #[inline]
    fn parse_openai_tool_call_delta(call_obj: &Value) -> Result<Option<ToolCall>, CompletionResponseError> {
...
impl StreamingResponse {
    ...
    #[inline]
    pub fn new(provider: Provider, model: &str, request_id: &str) -> Result<Self, CompletionResponseError> {
        let model_name = ArrayString::from(model)
            .map_err(|_| CompletionResponseError::ModelNameTooLong(model.len()))?;
...
impl fmt::Display for CompletionResponseError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::ProviderError { provider, message, status_code } => {
...
mod tests {
    ...
    fn test_openai_response_parsing() {
        ...
        assert_eq!(response.model.as_str(), "gpt-4");
        assert_eq!(response.choices.len(), 1);
        assert_eq!(response.choices[0].text(), "Hello! How can I help you today?");
    ...
    fn test_anthropic_response_parsing() {
        ...
        assert_eq!(response.choices.len(), 1);
        assert_eq!(response.choices[0].text(), "Hello! I'm Claude, an AI assistant created by Anthropic.");
        assert_eq!(response.usage.prompt_tokens, 10);
    ...
    fn test_completion_chunk_creation() {
        ...
        let chunk = CompletionChunk::new("chunk-123", "gpt-4", vec![choice])
    ...
    fn test_streaming_response() {
        ...
        let chunk = CompletionChunk::new("chunk-1", "gpt-4", vec![])
            .expect("Should create chunk");
    ...
    fn test_error_display() {
        ...
        assert_eq!(format!("{}", error), "Provider openai error (429): Rate limited");
    }
    ...
    fn test_response_helpers() {
        ...
        assert_eq!(response.all_texts(), vec!["First response", "Second response"]);
