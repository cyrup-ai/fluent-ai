...
impl SIMDTokenizer {
    ...
    pub fn tokenize_simd_avx2(
        ...
        unsafe {
            self.tokenize_simd_avx2_unsafe(text)
        }
    }
        ...
        while pos + 32 <= text.len() {
            ...
            for &boundary in &boundaries {
                if boundary > last_boundary {
                    ...
                    if !token_content.is_empty() && !token_content.iter().all(|&b| b.is_ascii_whitespace()) {
    ...
    unsafe fn find_word_boundaries_simd_avx2(&self, data: __m256i) -> SmallVec<[usize; 8]> {
        ...
        let boundary_mask = _mm256_or_si256(
            ...
            _mm256_or_si256(
                newline_cmp,
                _mm256_or_si256(punct_cmp_1, punct_cmp_2)
            )
        );
    ...
    fn determine_token_type(&self, content: &[u8]) -> TokenType {
        ...
        if content.iter().all(|&b| self.classify_character(b) == first_type) {
            first_type
    ...
    pub fn get_stats(&self) -> PerformanceStats {
        ...
        PerformanceStats {
            ...
            total_processing_time_nanos: self.tokenization_time_nanos.get(),
            ...
            simd_operations_count: self.simd_operations.get(),
            cache_hits: 0,
            cache_misses: 0,
        }
    }
