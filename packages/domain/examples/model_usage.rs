...
use fluent_ai_domain::model::{
    ...
    CacheConfig, RealModelInfo, ValidationResult, BatchValidationResult,
};
...
async fn example_basic_model_discovery() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("Registry contains {} models across {} providers", 
    ...
    println!("Found {} models with thinking capabilities", thinking_models.len());
    ...
    if let Some(cheapest) = registry.find_cheapest_model(Some(8000), false) {
        println!("Most affordable model with 8K+ context: {} (${:.2} + ${:.2})",
                 cheapest.name,
                 cheapest.pricing_input,
                 cheapest.pricing_output);
    }
...
async fn example_advanced_filtering() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("Found {} models suitable for chatbot use:", chatbot_models.len());
    for model in chatbot_models.iter().take(3) {
        println!("  - {} (context: {}, cost: ${:.2}/${:.2})",
                 model.name, model.max_context, 
    ...
    println!("\nFound {} models suitable for complex reasoning:", reasoning_models.len());
    for model in reasoning_models.iter().take(3) {
        println!("  - {} (context: {}, thinking: {}, cost: ${:.2}/${:.2})",
                 model.name, model.max_context, model.is_thinking,
                 model.pricing_input, model.pricing_output);
    }
    ...
    println!("\nAnthropic models within budget: {}", anthropic_budget_models.len());
...
async fn example_caching_strategies() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("Average lookup time: {}ns", final_stats.average_lookup_time_nanos);
    ...
    println!("Memory usage: {} bytes -> {} bytes", memory_before, memory_after);
...
async fn example_validation_and_health_monitoring() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if !failed_models.is_empty() {
        ...
        for failed in failed_models {
            println!("  - {}/{}: {}", 
                     failed.provider, failed.model_name, 
...
async fn example_application_integration() -> Result<(), Box<dyn std::error::Error>> {
    ...
    async fn select_model_for_task(
        ...
        for model in candidates {
            ...
            let provider = if model.name.starts_with("gpt") { "openai" }
                          else if model.name.starts_with("claude") { "anthropic" }
                          else if model.name.starts_with("grok") { "xai" }
                          else { "unknown" };
            ...
            match validator.validate_model_exists(provider, &model.name).await {
                ...
                Ok(validation) => {
                    println!("Model {}/{} failed validation: {}", 
                             provider, model.name, validation.status_description());
                }
                Err(e) => {
...
async fn example_performance_monitoring() -> Result<(), Box<dyn std::error::Error>> {
    ...
    impl PerformanceMetrics {
        ...
        fn report_performance(&self) {
            ...
            println!("    Thinking models: {}", self.registry_stats.thinking_models);
            println!("    High-context models: {}", self.registry_stats.high_context_models);
            ...
            println!("    Avg lookup: {}ns", self.cache_stats.average_lookup_time_nanos);
            
            // Health metrics
            println!("  Provider Health:");
            let healthy_providers = self.validation_health
                .values()
                .filter(|v| v.is_valid())
                .count();
            
            println!("    Healthy providers: {}/{}", 
                     healthy_providers, self.validation_health.len());
        ...
        fn identify_issues(&self) -> Vec<String> {
            ...
            if self.cache_stats.average_lookup_time_nanos > 1_000_000 {
                issues.push(format!("Slow cache lookups: {}ms avg", 
                                   self.cache_stats.average_lookup_time_nanos / 1_000_000));
            }
            
            // Check provider health
            let unhealthy_providers: Vec<_> = self.validation_health
                .iter()
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if available_providers.is_empty() {
        ...
        println!("✅ Detected API keys for: {}", available_providers.join(", "));
    }
    
    // Run all examples
    let examples = vec![
        ("Basic Model Discovery", example_basic_model_discovery as fn() -> _),
        ("Advanced Filtering", example_advanced_filtering),
        ("Caching Strategies", example_caching_strategies),
        ("Validation & Health Monitoring", example_validation_and_health_monitoring),
