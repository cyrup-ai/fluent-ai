...
impl ToolSet {
    ...
    pub fn execute(&self, name: &str, _args: Value) -> AsyncStream<ChatMessageChunk> {
        for tool in &self.tools {
            let tool_name = match tool {
                ToolType::Simple(t) => t.name(),
...
impl ToolSetBuilder {
    ...
    pub fn on_call<F>(self, name: &str, _args: Value, handler: F) -> AsyncTask<Value>
    where
        F: FnOnce(Result<Value, String>) -> Value + Send + 'static,
        Value: crate::async_task::NotResult,
    {
        let _toolset = self.register();
