...
impl Form {
    ...
    pub fn into_stream(mut self) -> impl Stream<Item = Result<Bytes, crate::Error>> + Send + Sync {
        ...
        let boundary = self.inner.boundary.clone();
        let percent_encoding = self.inner.percent_encoding;

        let fields = self.inner.take_fields();

        let mut streams: Vec<
            Pin<Box<dyn Stream<Item = Result<Bytes, crate::Error>> + Send + Sync>>,
        > = Vec::with_capacity(fields.len() + 1);

        for (name, part) in fields {
            let boundary_stream =
                stream::once(future::ready(Ok(format!("--{}\r\n", boundary).into())));
            let header = stream::once(future::ready(Ok({
                let mut h = percent_encoding.encode_headers(name.as_ref(), &part.meta);
                h.extend_from_slice(b"\r\n\r\n");
                h.into()
            })));
            let part_stream = boundary_stream
                .chain(header)
                .chain(part.value.into_stream())
                .chain(stream::once(future::ready(Ok("\r\n".into()))));
            streams.push(Box::pin(part_stream)
                as Pin<
                    Box<dyn Stream<Item = Result<Bytes, crate::Error>> + Send + Sync>,
                >);
        }
